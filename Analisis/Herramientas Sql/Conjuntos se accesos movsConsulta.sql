IF OBJECT_ID('mg.UsuarioConjuntosMovConsulta') IS NOT NULL
	DROP TABLE mg.UsuarioConjuntosMovConsulta

CREATE TABLE mg.UsuarioConjuntosMovConsulta(
	AccesoA VARCHAR(10),
	NumAccesoA INT,
	AccesoB VARCHAR(10),
	NumAccesoB INT,
...
IF EXISTS (SELECT NAME FROM TempDB.SYS.SYSOBJECTS WHERE Id = OBJECT_ID('Tempdb.dbo.#UsuarioMovsConsulta') AND TYPE = 'U')
	DROP TABLE #UsuarioMovsConsulta

SELECT Usuario,item
INTO #UsuarioMovsConsulta
FROM UsuarioMovsConsulta
UNION
SELECT Usuario,item
FROM UsuarioMovsEdicion
...
BEGIN
	...
	BEGIN
		...
		INSERT INTO mg.UsuarioConjuntosMovConsulta(AccesoA,NumAccesoA,AccesoB,NumAccesoB,Intercepcion,NumIntercepcion,AmenosB,NumAmenosB,BmenosA,NumBmenosA)
...
GO
SELECT * FROM mg.UsuarioConjuntosMovConsulta

SELECT *
FROM mg.UsuarioConjuntosMovConsulta WITH(NOLOCK)
...
SELECT *
FROM mg.UsuarioConjuntosMovConsulta
WHERE AccesoA LIKE 'AUDCC%'
