IF OBJECT_ID('mg.UsuarioConjuntosReportes') IS NOT NULL
	DROP TABLE mg.UsuarioConjuntosReportes

CREATE TABLE mg.UsuarioConjuntosReportes(
	AccesoA VARCHAR(10),
	NumAccesoA INT,
	AccesoB VARCHAR(10),
	NumAccesoB INT,
...
BEGIN
	...
	BEGIN
		...
		INSERT INTO mg.UsuarioConjuntosReportes(AccesoA,NumAccesoA,AccesoB,NumAccesoB,Intercepcion,NumIntercepcion,AmenosB,NumAmenosB,BmenosA,NumBmenosA)
...
GO
SELECT * FROM mg.UsuarioConjuntosReportes

SELECT *
FROM mg.UsuarioConjuntosReportes WITH(NOLOCK)
...
SELECT Porcentaje = 100.00*NumIntercepcion/IIF(NumAccesoA=0,1,NumAccesoA)
	...
	,NumIntercepcion,NumAmenosB,NumBmenosA
FROM mg.UsuarioConjuntosReportes
WHERE AccesoA LIKE 'VENTP%'
	AND AccesoB NOT LIKE 'ADMIN%'
	AND AccesoB NOT LIKE 'SISIN%'
	--AND 100.00*NumIntercepcion/IIF(NumAccesoA=0,1,NumAccesoA) > 99.00
	AND AccesoA='VENTP_GERA'
ORDER BY 2,1 DESC,3
...
FROM (
	...
	LEFT JOIN UsuarioReportes P ON M.item=P.item
