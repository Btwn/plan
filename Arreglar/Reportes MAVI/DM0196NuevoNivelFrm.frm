[Forma]
Clave=DM0196NuevoNivelFrm
Nombre=DM0196NuevoNivelFrm
Icono=0
Modulos=(Todos)
...
Visible=S
ConCondicion=S
Expresion=EjecutarSQL(<T>EXEC SP_DM0196EliminarCrearDivision :tDiv, :tNom, :nOpc, :tUsr<T>, MAvi.DM0207Division, MAvi.DM0207Nivel,3,Usuario )
EjecucionCondicion=Si Vacio(MAvi.DM0207Nivel)<BR>Entonces<BR>    Error(<T>Debe llenar el campo Nivel<T>)<BR>Sino<BR>    Si SQL(<T>Select COUNT(Nombre) From NivelCobranzaMavi Where Nombre = :tNom<T>,MAvi.DM0207Nivel) > 0<BR>    Entonces<BR>        Si SQL(<T>Select COUNT(Nombre) From DM0196NivelCobranzaMaviDivAvales Where Nombre = :tNom AND Division = :tDiv<T>,MAvi.DM0207Nivel,MAvi.DM0207Division)  > 0<BR>        Entonces<BR>            Error(<T>Ese Nivel ya existe en la division seleccionada, elija otro<T>)<BR>        Sino<BR>            Verdadero<BR>        Fin<BR>    Sino<BR>      Error(<T>El nivel escrito no existe en la tabla NivelCobranzaMavi<T>)<BR>    Fin<BR>Fin
