...
AccionesDerecha=S
ListaAcciones=Seleccionar<BR>AutoSelecciona
VentanaBloquearAjuste=S
ExpresionesAlMostrar=Asigna(Mavi.SW,1)<BR>si vacio(Mavi.UEN)<BR>    entonces<BR>        precaucion(<T>Debe seleccionar una UEN<T>)<BR>        abortaroperacion<BR>sino<BR>si vacio(Mavi.TipoSucurAnalisis)<BR>    entonces<BR>        precaucion(<T>Debe seleccionar un Tipo de Sucursal<T>)<BR>        abortaroperacion<BR>fin<BR>fin<BR><BR> /**/<BR>/*precaucion(<T>Debe seleccionar un Tipo de Sucursal<T>,botonaceptar)=botonaceptar,abortaroperacion,abortaroperacion<BR> sino<BR>si (condatos(Mavi.TipoSucurAnalisis))<BR>entonces<BR>asigna(mavi.tiposucuranalisis,nulo)*/<BR>//sino<BR>//<T><T><BR>//fin
...
IconosNombre=MaviSucuAnadeCartVis:Num
...
[Acciones.AutoSelecciona]
Nombre=AutoSelecciona
Boton=0
NombreDesplegar=AutoSel
Multiple=S
EnBarraHerramientas=S
