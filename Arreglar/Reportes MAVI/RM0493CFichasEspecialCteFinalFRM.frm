[Forma]
Clave=RM0493CFichasEspecialCteFinalFRM
Icono=0
Modulos=(Todos)
...
Nombre=RM0493C Fichas Cobro Cliente Final
[Filtros]
Estilo=Ficha
Clave=Filtros
...
Antes=S
GuardarAntes=S
AntesExpresiones=Asigna(Mavi.Clave,0)<BR>Si SQL(<T>Select count(*) from MAVIRM0493CFICHASCOBROCTEFINAL where Ejercicio = :tejercicio and Quincena = :tquince<T>,Mavi.Ejercicio,Mavi.quincena) > 0<BR>entonces<BR>    si Confirmacion( <T>Existen datos en esta quincena, ¿Desea sobreescribir?<T>,BotonAceptar,BotonCancelar) = 1<BR>     entonces<BR>          Asigna(Mavi.Clave,0)<BR>     sino<BR>         Asigna(Mavi.Clave,1)<BR>    fin<BR>fin
[Acciones.Preliminar]
Nombre=Preliminar
Boton=56
NombreEnBoton=S
...
ClaveAccion=RM0493CFichasEspecialCteFinalRepImp
Activo=S
Visible=S
...
ClaveAccion=RM0493CFichasEspecialCteFinalRepXLS
Activo=S
Visible=S
...
Boton=0
TipoAccion=Reportes Impresora
ClaveAccion=RM0493CFichasEspecialCteFinalRepTXT
Activo=S
Visible=S
