...
[Forma]
Clave=TarjetaSerieMovMAVIRedimirVitual
Icono=67
Modulos=(Todos)
...
ExpresionesAlMostrar=//Asigna(Mavi.SaldoVirtual,0)<BR>Asigna(Mavi.ImporteVirtual,0)<BR><BR>Asigna(Info.ClienteNombre,SQL(<T>SELECT Cliente FROM Venta WITH(NOLOCK) WHERE ID = :nId<T>,Info.ID)) //ya no pregunto si tiene virtual lo se desde la forma de venta<BR>Si<BR>  Mavi.TipoMoneDima = <T>VIRTUAL<T><BR>Entonces<BR>  Asigna(Mavi.SerieMoneVirtual,SQL(<T>SELECT Serie FROM TarjetaMonederoMAVI WITH(NOLOCK) WHERE estatus = :tEst AND TipoMonedero = :tTip AND Cliente = :tCte <T>,<T>ACTIVA<T>,<T>VIRTUAL<T>,Info.ClienteNombre))<BR><BR>Fin<BR><BR>//informacion(Info.Cliente+<T>,<T>+Mavi.SerieMoneVirtual)     <BR>SI (SQL(<T>SELECT dbo.fnMonederoDV(:tSerie,0)<T>,Mavi.SerieMoneVirtual))=<T>1<T><BR>ENTONCES<BR>    Asigna(Mavi.SaldoVirtual, Vacio(SQL(<T>SELECT dbo.FnVTASCalcularSaldo(:tCuenta,:nUEN)<T>,Izquierda(Mavi.SerieMoneVirtual,8),Info.UEN),0.00))<BR>SINO<BR>    Asigna(Mavi.SaldoVirtual, Vacio(SQL(<T>SELECT dbo.FnVTASCalcularSaldo(:tCuenta,:nUEN)<T>,Mavi.SerieMoneVirtual,Info.UEN),0.00))<BR>FIN      <BR><BR>  //Asigna(Mavi.SerieMoneVirtual,<T>01<T>)
