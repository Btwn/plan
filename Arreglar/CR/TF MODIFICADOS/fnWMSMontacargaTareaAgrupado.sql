...
TMA.Origen,
TMA.OrigenID,
Cuenta = (CASE WHEN TMA.OrigenTipo = 'VTAS' THEN (SELECT Cliente FROM Venta WITH(NOLOCK) WHERE Mov=TMA.Origen AND MovID = TMA.OrigenID) ELSE NULL END)
FROM WMSMontacargaTarea WITH(NOLOCK)
LEFT OUTER JOIN Art WITH(NOLOCK) ON WMSMontacargaTarea.Articulo=Art.Articulo
LEFT OUTER JOIN Agente WITH(NOLOCK) ON WMSMontacargaTarea.Montacarga=Agente.Agente
JOIN TMA WITH(NOLOCK) ON WMSMontacargaTarea.ID=TMA.ID
