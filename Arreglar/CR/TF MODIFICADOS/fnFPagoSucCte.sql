...
FROM CteEmpresaCFD A WITH(NOLOCK)
JOIN CteCFDFormaPago B WITH(NOLOCK)
ON A.Empresa = B.Empresa
AND A.Cliente = B.Cliente
WHERE A.Empresa = @Empresa
AND A.Cliente = @Cte
INSERT INTO @FormaPago(Cliente, Sucursal, FormaPago, Cuenta, Tipo)
SELECT A.Cliente, NULL, A.InfoPago, B.CuentaPago, 'Informaci�n del Pago'
FROM CteCFDInfoPago A WITH(NOLOCK)
JOIN CteCFDInfoPagoD B WITH(NOLOCK)
