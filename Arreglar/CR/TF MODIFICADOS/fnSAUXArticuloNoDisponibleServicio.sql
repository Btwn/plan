...
JOIN SAUXD sd WITH(NOLOCK) ON s.ID = sd.ID
JOIN SAUX os WITH(NOLOCK) ON s.Origen = os.Mov AND s.OrigenID = os.MovID AND s.Empresa = os.Empresa
JOIN Inv i WITH(NOLOCK)   ON i.Origen = s.Mov AND i.OrigenID = s.MovID AND s.Empresa = i.Empresa
