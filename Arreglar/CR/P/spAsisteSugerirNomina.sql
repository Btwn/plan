	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				IF EXISTS (SELECT personal FROM AsisteD d WHERE d.ID = @ID AND UPPER(d.Tipo) = 'D_aa Desc. Lab.' AND ISNULL(RTRIM(d.Concepto), '') = @Concepto)
				BEGIN
					INSERT Nomina (UltimoCambio, Sucursal, SucursalOrigen, SucursalDestino, OrigenTipo, Origen, OrigenID, Empresa, Usuario, Estatus, Mov, FechaEmision, Proyecto, UEN, Moneda, TipoCambio, Concepto)
						SELECT GETDATE()
							  ,@Sucursal
					...
					INSERT NominaD (ID, Renglon, Personal, FechaD, Cantidad)
						...
						JOIN Personal p
							...
							AND p.Estatus = 'ALTA'
						WHERE d.ID = @ID
						AND UPPER(d.Tipo) = 'D_aa Desc. Lab.'
						AND ISNULL(RTRIM(d.Concepto), '') = @Concepto
						GROUP BY d.Personal
								,d.Fecha
								,d.Cantidad
				...
				IF EXISTS (SELECT personal FROM AsisteD d WHERE d.ID = @ID AND UPPER(d.Tipo) = 'D_aa Festivo Lab.' AND ISNULL(RTRIM(d.Concepto), '') = @Concepto)
				BEGIN
					INSERT Nomina (UltimoCambio, Sucursal, SucursalOrigen, SucursalDestino, OrigenTipo, Origen, OrigenID, Empresa, Usuario, Estatus, Mov, FechaEmision, Proyecto, UEN, Moneda, TipoCambio, Concepto)
						SELECT GETDATE()
							  ,@Sucursal
					...
					INSERT NominaD (ID, Renglon, Personal, FechaD, Cantidad)
						...
						JOIN Personal p
							...
							AND p.Estatus = 'ALTA'
						WHERE d.ID = @ID
						AND UPPER(d.Tipo) = 'D_aa Festivo Lab.'
						AND ISNULL(RTRIM(d.Concepto), '') = @Concepto
						GROUP BY d.Personal
								,d.Fecha
								,d.Cantidad
