...
BEGIN
SELECT @Cliente = v.Cliente, @CFDImporte = ISNULL(vtce.SubTotal,0.0), @CFDImpuesto1 = ISNULL(Impuesto1Total,0.0), @CFDImpuesto2 = ISNULL(Impuesto2Total,0.0), @TipoCambio = v.TipoCambio FROM Venta v JOIN VentaTCalcExportacion vtce ON vtce.ID = v.ID WHERE v.ID = @ID   
...
SELECT @Cliente = v.Cliente,  @CFDImporte = ISNULL(v.Importe,0.0),      @CFDImpuesto1 = ISNULL(v.Impuestos,0.0),    @CFDImpuesto2 = 0.0,                        @TipoCambio = v.TipoCambio FROM CXC v WHERE v.ID = @ID  
END
