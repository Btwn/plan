...
IF NOT EXISTS (SELECT * FROM PosicionArt WHERE Posicion = @PosicionDestino AND Articulo = @Articulo) AND @PosicionDestino IS NOT NULL 
BEGIN
...
SELECT @Zona = Zona FROM AlmPos WHERE Posicion = @PosicionDestino AND Almacen = @Almacen
INSERT TMAD (
