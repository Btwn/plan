		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						SET @Ok = 80110
						SET @OkRef = 'Movimiento bloqueado: El costo excede al m ximo indicado en la orden de Compra. Artb¡culo: ' + CAST(@Articulo AS VARCHAR(50)) + ' Costo:$' + CAST(@Costo AS VARCHAR(15))
					END
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				IF (@RFCCompleto = 1)
					SELECT @OK = 80110
						  ,@OKRef = 'El RFC del Cliente No Est_¡ Completo'

				IF (@RFCCompleto = 2)
					SELECT @OK = 80110
						  ,@OKRef = 'El RFC del Cliente Est_¡ Incorrecto'
