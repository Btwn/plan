...
GO
ALTER PROCEDURE spCapturaAfectar
@ID                		int,
@Accion			char(20),
@Base			char(20),
@Empresa	      		char(5),
...
IF @Ok IS NULL
BEGIN
INSERT CapturaD (ID, Campo, Valor, Sucursal)
SELECT @IDGenerar, Campo, Valor, @Sucursal
FROM CapturaD
WHERE ID = @ID
...
IF @Accion = 'CANCELAR'
BEGIN
DECLARE crCapturaCancelar CURSOR FOR
SELECT ID
FROM Captura
WHERE OrigenTipo = @Modulo AND Origen = @Mov AND OrigenID = @MovID AND Estatus IN ('PENDIENTE', 'CONCLUIDO')
OPEN crCapturaCancelar
FETCH NEXT FROM crCapturaCancelar INTO @CancelarID
...
FETCH NEXT FROM crCapturaCancelar INTO @CancelarID
END
CLOSE crCapturaCancelar
DEALLOCATE crCapturaCancelar
END
IF @Accion <> 'CANCELAR'
