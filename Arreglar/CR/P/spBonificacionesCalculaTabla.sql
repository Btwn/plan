...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			IF dbo.fnFechaSinHora(GETDATE()) >= dbo.fnFechaSinHora((
				...
				SELECT @Ok = 1
					  ,@OkRef = 'No cumple con el l_amite de pago posterior1'
	...
	BEGIN
		...
		IF @MaxDiasAtrazo > @DiasAtrazo
			SELECT @Ok = 1
				  ,@OkRef = 'Excede el n_ºmero de dias de atraso permitidos '
	...
	BEGIN
		...
		IF @DiasMenoresA < DATEDIFF(DAY, @FechaEmisionFact, GETDATE())
			SELECT @Ok = 1
				  ,@OkRef = 'Excede d_aas menores' + CONVERT(CHAR(30), @DiasMenoresA)
	...
	BEGIN
		...
		IF GETDATE() >= (@FechaEmisionFact + @DiasMayoresA)
			SELECT @Ok = 1
				  ,@OkRef = 'Excede d_aas mayores' + CONVERT(CHAR(30), @DiasMayoresA)
	...
	BEGIN
		...
		IF NOT EXISTS (SELECT IdBonificacion FROM MaviBonificacionCanalVta BonCan WHERE CONVERT(VARCHAR(10), BonCan.CanalVenta) = @ClienteEnviarA AND BonCan.IdBonificacion = @id)
			SELECT @Ok = 1
				  ,@OkRef = 'Venta de Canal No Configurada Para esta Bonificaci_n'
	...
	BEGIN
		...
		IF NOT EXISTS (SELECT IdBonificacion FROM MaviBonificacionCondicion WHERE COndicion = @Condicion AND IdBonificacion = @Id)
			SELECT @Ok = 1
				  ,@OkRef = 'Condicion No Configurada Para esta Bonificaci_n'
	...
	IF NOT @TipoSucursal IS NULL
		...
		SELECT @Ok = 1
			  ,@OkRef = 'Bonificaci_n No Configurada Para este tipo de Sucursal'
