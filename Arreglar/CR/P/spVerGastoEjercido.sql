...
GO
ALTER PROCEDURE [dbo].[spVerGastoEjercido]
 @Empresa CHAR(5)
...
BEGIN
	...
	IF (
		...
		= 1
		SELECT @GastoEjercido = SUM((d.Importe - ISNULL(d.Provision, 0.0)) *
		 CASE
			 WHEN mt.Clave = 'GAS.DG' THEN -1
			 ELSE 1
		 END)
		FROM Gasto e
			,GastoD d
		...
		AND mt.Modulo = 'GAS'
		AND mt.Mov = e.Mov
		AND mt.Clave IN ('GAS.P', 'GAS.C', 'GAS.DG', 'GAS.DPR', 'GAS.EST')
		AND mt.Mov NOT IN ('Complto Presupuesto')
	ELSE
		SELECT @GastoEjercido = SUM((d.Importe - ISNULL(d.Provision, 0.0)) *
		 CASE
			 WHEN mt.Clave = 'GAS.DG' THEN -1
