...
BEGIN
SELECT @Empresa = Empresa,@Editable = CASE WHEN Estatus = 'SINAFECTAR' THEN 1 ELSE 0 END FROM Campana WHERE ID = @IDCampana
...
JOIN Cte c ON a.Contacto = c.Cliente
WHERE ISNULL(a.EnviarA,0) = 0
UPDATE @AgendaEstacion
SET Nombre = e.Nombre
FROM @AgendaEstacion a
JOIN Cte c ON a.Contacto = c.Cliente
JOIN CteEnviarA e ON c.Cliente = e.Cliente AND a.EnviarA = e.ID
WHERE ISNULL(a.EnviarA,0) <> 0
END
COMMIT
