...
SELECT @Existe = Count(*) FROM FormaPago WHERE FormaPago = @FormaPago
...
BEGIN
SELECT @Moneda = Moneda FROM Mon WHERE Orden = 1
END
SET @Existe = Null
SELECT @Existe = Count(*) FROM Mon WHERE Moneda = ISNULL(@Moneda,@MonedaCfg)
IF @Existe < 1 AND @OK IS NULL
BEGIN
SET @Ok = 10060
...
END
SET @Existe = Null
SELECT @Existe = Count(*) FROM Cte WHERE Cliente = @Cliente
IF @Existe < 1 AND @OK IS NULL
BEGIN
SET @Ok = 10060
...
END
/*********************************************************************************/
SELECT @TipoCambio = TipoCambio FROM Mon WHERE Moneda = ISNULL(@Moneda,@MonedaCfg)
SELECT @MonedaCte = DefMoneda FROM Cte WHERE Cliente = @Cliente
IF @MonedaCte IS NULL
SET @MonedaCte = @Moneda
SELECT @TipoCambioCte = TipoCambio FROM Mon WHERE Moneda = @MonedaCte
IF @Referencia Like '% %'
...
AND	MovID = @MovID
AND	Cliente = @Cliente
SELECT	@TipoCambioFact = Tipocambio FROM Mon WHERE Moneda = @MonedaFact
...
END
/*********************************************************************************/
SELECT @ChecaSelec = ISNULL(COUNT(*),0) FROM WebCxCPendientesPaso
...
SELECT @Renglon = MAX(Renglon) FROM CxcD WHERE ID = @IDCxcCob
...
FROM  CxcPendiente C
JOIN  MovTipo MT ON C.Mov = MT.Mov AND MT.Modulo = 'CXC'
WHERE  MT.Clave = 'CXC.F'
AND  C.Cliente = @Cliente
AND	C.Moneda = ISNULL(@MonedaFact,@Moneda)
AND (C.Mov <> ISNULL(@Mov,'') OR C.MovID <> ISNULL(@MovID,''))
AND  C.Id in (Select IdCxC from WebCxCPendientesPaso Where Cliente = @Cliente and Seleccion = 'Si')
...
SET @MontoCobro = ROUND(ISNULL(@MontoCobro,0)+@ImporteAplica,dbo.fnRedondeoMonetarios())
END
SELECT @Renglon = MAX(Renglon)+1024 FROM CxcD WHERE ID = @IDCxcCob
SET @Renglon = ISNULL(@Renglon,1024)
INSERT INTO CxcD(ID, Renglon, RenglonSub, Importe, Aplica, AplicaID,
Ligado, Sucursal, LigadoDR, Logico1, SucursalOrigen)
...
FETCH CrDocumentos INTO @cID,@cEmpresa,@cMov,@cMovID,@cCliente,@cSaldo
END
UPDATE Cxc SET Importe = @MontoCobro WHERE ID = @IDCxcCob
CLOSE CrDocumentos
...
WHILE @@FETCH_STATUS = 0 AND @SaldoCobro > 0
BEGIN
SELECT @cTipoCambio = TipoCambio FROM Mon WHERE Moneda = @cMoneda
SET @MontoCobro = 0
INSERT INTO CXC(Empresa, Mov, Moneda, Cliente, AplicaManual, ConDesglose, GenerarPoliza, Indirecto, GenerarDinero, Sucursal,
Logico1, Logico2, Logico3, Logico4, Logico5, Logico6, ConTramites, SucursalOrigen, FechaEmision, UltimoCambio,
Concepto, TipoCambio, Usuario, Referencia, ClienteMoneda, Vencimiento, FormaCobro, Importe, Impuestos, Agente,
