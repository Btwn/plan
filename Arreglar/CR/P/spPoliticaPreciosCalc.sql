...
GO
ALTER PROCEDURE spPoliticaPreciosCalc
...
END ELSE
IF (@Nivel IN ('Siempre')) OR (@Nivel IN ('Promocion') AND @PromocionPromocionAplicada = 0) 
BEGIN
SELECT @CalcDescuentoPromocion = (@DescuentoAcum /100) * @PonderadoPromocion 
...
END ELSE
IF (@Nivel IN ('Siempre')) OR (@Nivel IN ('Particular') AND @PromocionParticularAplicada = 0)  
BEGIN
SELECT @CalcDescuentoParticular = (@DescuentoAcum /100) * @PonderadoParticular 
