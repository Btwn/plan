...
AND d.Almacen = Alm.Almacen AND Alm.ExcluirPlaneacion = 0
AND UPPER(A.Tipo) NOT IN (CASE WHEN @CfgPlanPlanearServicios = 0 THEN 'SERVICIO' END)
GROUP BY d.Articulo, ISNULL(d.SubCuenta, ''), d.Almacen, CASE WHEN d.FechaRequerida < @Hoy THEN -1 ELSE ISNULL(DATEDIFF(week, @Hoy, d.FechaRequerida), 0) END
...
UPDATE PlanArt SET Periodo = ISNULL(DATEDIFF(week, @Hoy, @FechaRequerida), 0) WHERE CURRENT OF crISDemanda
END
FETCH NEXT FROM crISDemanda INTO @TiempoEntrega, @TiempoEntregaUnidad, @TiempoEntregaSeg, @TiempoEntregaSegUnidad, @Almacen, @AlmacenROP, @RutaDistribucion
END
CLOSE crISDemanda
...
END
INSERT PlanArt (Empresa, Almacen, Articulo, SubCuenta, Acronimo, Cantidad, Periodo)
SELECT @Empresa, d.Almacen, d.Articulo, ISNULL(d.SubCuenta, ''), 'PRV', SUM(d.Cantidad*d.Factor), ISNULL(DATEDIFF(week, @Hoy, d.FechaRequerida), 0)
...
AND d.Almacen = Alm.Almacen AND Alm.ExcluirPlaneacion = 0
AND UPPER(A.Tipo) NOT IN (CASE WHEN @CfgPlanPlanearServicios = 0 THEN 'SERVICIO' END)
GROUP BY d.Articulo, ISNULL(d.SubCuenta, ''), d.Almacen, ISNULL(DATEDIFF(week, @Hoy, d.FechaRequerida), 0)
...
AND d.Almacen = Alm.Almacen AND Alm.ExcluirPlaneacion = 0
AND UPPER(A.Tipo) NOT IN (CASE WHEN @CfgPlanPlanearServicios = 0 THEN 'SERVICIO' END)
GROUP BY d.Articulo, ISNULL(d.SubCuenta, ''), d.Almacen, CASE WHEN d.FechaRequerida < @Hoy THEN -1 ELSE ISNULL(DATEDIFF(week, @Hoy, d.FechaRequerida), 0) END, CASE WHEN e.OrigenTipo = 'PLAN' THEN 1 ELSE 0 END
HAVING SUM(d.CantidadPendiente*d.Factor) > 0 /*OR SUM(d.CantidadOrdenada*d.Factor) > 0*/
INSERT PlanArt (Empresa, Almacen, Articulo, SubCuenta, Acronimo, Cantidad, Periodo, OrigenPlan)
...
CASE WHEN d.FechaEntrega < @Hoy THEN -1 ELSE ISNULL(DATEDIFF(week, @Hoy, d.FechaEntrega), 0) END
...
AND d.Almacen = Alm.Almacen AND Alm.ExcluirPlaneacion = 0
AND UPPER(A.Tipo) NOT IN (CASE WHEN @CfgPlanPlanearServicios = 0 THEN 'SERVICIO' END)
GROUP BY d.Articulo, ISNULL(d.SubCuenta, ''), d.Almacen, CASE WHEN d.FechaEntrega < @Hoy THEN -1 ELSE ISNULL(DATEDIFF(week, @Hoy, d.FechaEntrega), 0) END
...
CASE WHEN e.FechaEntrega < @Hoy THEN -1 ELSE ISNULL(DATEDIFF(week, @Hoy, e.FechaEntrega), 0) END
...
AND d.Almacen = Alm.Almacen AND Alm.ExcluirPlaneacion = 0
AND UPPER(A.Tipo) NOT IN (CASE WHEN @CfgPlanPlanearServicios = 0 THEN 'SERVICIO' END)
GROUP BY d.Articulo, ISNULL(d.SubCuenta, ''), d.Almacen, CASE WHEN e.FechaEntrega < @Hoy THEN -1 ELSE ISNULL(DATEDIFF(week, @Hoy, e.FechaEntrega), 0) END
HAVING SUM(d.CantidadPendiente*d.Factor) > 0
INSERT PlanArt (Empresa, Almacen, Articulo, SubCuenta, Acronimo, Cantidad, Periodo)
...
AND UPPER(A.Tipo) NOT IN (CASE WHEN @CfgPlanPlanearServicios = 0 THEN 'SERVICIO' END)
GROUP BY d.Articulo, ISNULL(d.SubCuenta, ''), e.AlmacenDestino, CASE WHEN d.FechaRequerida < @Hoy THEN -1 ELSE ISNULL(DATEDIFF(week, @Hoy, d.FechaRequerida), 0) END
...
CASE WHEN pa.FechaEntrega < @Hoy THEN -1 ELSE ISNULL(DATEDIFF(week, @Hoy, pa.FechaEntrega), 0) END
...
AND pa.Almacen = Alm.Almacen AND Alm.ExcluirPlaneacion = 0
AND UPPER(A.Tipo) NOT IN (CASE WHEN @CfgPlanPlanearServicios = 0 THEN 'SERVICIO' END)
GROUP BY pa.Articulo, ISNULL(pa.SubCuenta, ''), pa.Almacen, CASE WHEN pa.FechaEntrega < @Hoy THEN -1 ELSE ISNULL(DATEDIFF(week, @Hoy, pa.FechaEntrega), 0) END
...
AND UPPER(A.Tipo) NOT IN (CASE WHEN @CfgPlanPlanearServicios = 0 THEN 'SERVICIO' END)
GROUP BY pa.Articulo, ISNULL(pa.SubCuenta, ''), pa.AlmacenDestino, CASE WHEN pa.FechaEntrega < @Hoy THEN -1 ELSE ISNULL(DATEDIFF(week, @Hoy, pa.FechaEntrega), 0) END
HAVING SUM(pa.Cantidad) > 0
INSERT PlanArt (Empresa, Almacen, Articulo, SubCuenta, Acronimo, Cantidad, Periodo)
