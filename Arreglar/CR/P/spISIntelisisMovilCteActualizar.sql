...
IF (CASE WHEN LEN(@RFC) = 10 THEN ISNUMERIC(RIGHT(@RFC,6)) ELSE 1 END) = 1 AND NOT EXISTS(SELECT RFC FROM Cte WHERE RFC = @RFC AND Cliente <> @Cliente) AND ISNULL(@RFC,'') <> ''
BEGIN
...
ELSE
BEGIN
IF EXISTS(SELECT Cliente FROM Cte WHERE	Cliente = @Cliente)
BEGIN
SELECT
