...
SELECT @Modificar = ISNULL(ModificarPosicionSugeridaWMS,0) FROM Usuario WHERE Usuario = @Usuario 
...
IF @Ok IS NULL
BEGIN
IF @Ok IS NULL AND NOT EXISTS(SELECT * FROM TMA t JOIN TMAD d ON t.ID = d. ID  WHERE  d.Tarima = @Tarima  AND t.Empresa = @Empresa AND t.Sucursal = @Sucursal AND t.Mov=@Mov AND t.MovID= @MovID) SELECT @Ok = 13110 
