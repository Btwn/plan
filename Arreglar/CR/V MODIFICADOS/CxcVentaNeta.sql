...
JOIN MovTipo WITH (NOLOCK) ON Cxc.Mov = MovTipo.Mov AND MovTipo.Modulo = 'CXC'
WHERE
...
JOIN MovTipo WITH (NOLOCK) ON v.Mov = MovTipo.Mov AND MovTipo.Modulo = 'VTAS'
WHERE
...
FROM Venta v WITH (NOLOCK)
JOIN VentaCobro vc WITH (NOLOCK) ON v.ID = vc.ID
JOIN MovTipo WITH (NOLOCK) ON v.Mov = MovTipo.Mov AND MovTipo.Modulo = 'VTAS'
WHERE
