...
FROM Compra c
WITH(NOLOCK) WHERE c.ID = @ID
IF NOT EXISTS(SELECT * FROM Alm a WITH(NOLOCK) WHERE a.Almacen = @AlmacenDestino)
BEGIN
SELECT 'El Almac�n indicado no existe'
RETURN
END
IF NOT EXISTS(SELECT * FROM MovTipo mt WITH(NOLOCK) WHERE mt.Modulo = 'INV' AND mt.Mov = @GenerarMov)
BEGIN
SELECT 'El Movimiento indicado no existe'
RETURN
END
IF NOT EXISTS(SELECT * FROM MovTipo mt WITH(NOLOCK) WHERE mt.Modulo = 'INV' AND mt.Mov = @GenerarMov AND mt.Clave IN ('INV.OI', 'INV.SI'))
BEGIN
...
WITH(NOLOCK) WHERE ce.Estacion = @Estacion
AND ce.ID = @ID
AND ISNULL(ce.CantidadA, 0) > ce.Existencia
IF @Articulo IS NOT NULL
BEGIN
SELECT 'El Art�culo ' + RTRIM(@Articulo) + ' excede la cantidad indicada a la Existencia'
RETURN
END
IF @Almacen = @AlmacenDestino
BEGIN
SELECT 'El Almac�n Destino debe ser diferente al de la Compra'
RETURN
END
