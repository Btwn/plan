...
IF @Ok  IS NULL
IF NOT EXISTS (SELECT * FROM POSSaldoPMon WITH (NOLOCK) WHERE Empresa = @Empresa AND Cuenta = @Monedero)
...
IF @Ok  IS NULL
BEGIN
IF NOT EXISTS (SELECT * FROM POSAuxiliarPMon WITH (NOLOCK) WHERE Empresa = @Empresa AND Cuenta = @Monedero)
