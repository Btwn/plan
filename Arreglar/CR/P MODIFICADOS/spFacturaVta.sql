...
GO
ALTER PROCEDURE dbo.spFacturaVta
@ID INT
...
(SELECT Direccion FROM AnexoCta WITH (NOLOCK) WHERE RAMA = 'EMP' AND CUENTA = V.Empresa AND TIPO = 'Imagen' AND Nombre = 'LOGO') AS CODIGO,
(SELECT NOMBRE FROM FISCALREGIMEN WITH (NOLOCK) WHERE FiscalRegimen = E.FiscalRegimen) AS REGIMENFISCAL,
ISNULL(VC.FormaCobro1, (SELECT InfoFormaPago FROM CteCFD WITH (NOLOCK) WHERE Cliente = V.Cliente)) AS FormaCobro1,
VC.FormaCobro2,
VC.FormaCobro3,
VC.FormaCobro4,
VC.FormaCobro5,
ISNULL(VC.Referencia1, (SELECT CuentaPago FROM CteCFDInfoPagoD P WITH (NOLOCK) JOIN CteCFD CC WITH (NOLOCK) ON P.Cliente = CC.Cliente AND CC.InfoPago = P.InfoPago AND P.FormaPago = CC.InfoFormaPago WHERE P.Cliente = V.Cliente)) AS Referencia1,
VC.Referencia2,
VC.Referencia3,
VC.Referencia4,
VC.Referencia5,
(CASE WHEN NULLIF(ISNULL(NULLIF(VC.Referencia1, ''), (SELECT CUENTAPAGO FROM CteCFDInfoPagoD P WITH (NOLOCK) JOIN CteCFD CC WITH (NOLOCK) ON P.Cliente = CC.Cliente AND CC.InfoPago = P.InfoPago AND P.FormaPago = CC.InfoFormaPago WHERE P.Cliente = V.Cliente)), '')
...
(CASE WHEN NULLIF(ISNULL(NULLIF(VC.FormaCobro1, ''),(SELECT InfoFormaPago FROM CteCFD WITH (NOLOCK) WHERE Cliente = V.Cliente)), '')
...
JOIN VENTA VE WITH (NOLOCK) ON V.ID = VE.ID
JOIN Cte C WITH (NOLOCK) ON V.CLIENTE = C.Cliente
LEFT OUTER JOIN CteEnviarA CA WITH (NOLOCK) ON V.Cliente = CA.Cliente AND VE.EnviarA = CA.ID
JOIN ART A WITH (NOLOCK) ON V.ARTICULO = A.ARTICULO
JOIN VENTAD VD WITH (NOLOCK) ON V.ID = VD.ID AND V.Renglon = VD.Renglon AND V.RenglonID = VD.RenglonID
JOIN EMPRESA E WITH (NOLOCK) ON V.Empresa = E.Empresa
LEFT OUTER JOIN AnexoCta AC WITH (NOLOCK) ON V.Empresa = AC.Cuenta AND CONVERT(VARCHAR(10), V.ContUso) + 'F' = AC.Nombre AND AC.Rama = 'EMP' AND AC.Tipo = 'Imagen'
LEFT OUTER JOIN Agente AG WITH (NOLOCK) ON V.Agente = AG.Agente
JOIN Sucursal S WITH (NOLOCK) ON V.Sucursal = S.Sucursal
LEFT OUTER JOIN CFD i WITH (NOLOCK) ON V.ID = i.ModuloID AND i.Modulo = 'VTAS'
LEFT OUTER JOIN VentaCobro VC WITH (NOLOCK) ON V.ID = VC.ID
LEFT OUTER JOIN CteEnviarA CAD WITH (NOLOCK) ON CAD.Cliente = V.Cliente AND VD.EnviarA = CAD.ID
...
JOIN Sucursal S WITH (NOLOCK) ON V.Sucursal = S.Sucursal
LEFT OUTER JOIN CFD i WITH (NOLOCK) ON V.ID = i.ModuloID AND i.Modulo = 'VTAS'
JOIN SerieLoteMov SLM WITH (NOLOCK) ON V.ID = SLM.ID AND SLM.Modulo = 'VTAS' AND V.Articulo = SLM.Articulo AND V.RenglonID = SLM.RenglonID
LEFT OUTER JOIN SerieLoteProp SLP WITH (NOLOCK) ON SLM.Propiedades = SLP.Propiedades
LEFT OUTER JOIN VentaCobro VC WITH (NOLOCK) ON V.ID = VC.ID
LEFT OUTER JOIN CteEnviarA CAD WITH (NOLOCK) ON CAD.Cliente = V.Cliente AND VD.EnviarA = CAD.ID
WHERE V.ID = @ID
INSERT INTO #REPPRESUPUESTO
...
CA.Fax AS CteSucFax,
CA.eMail1 AS CteSuceMail1,
(SELECT MAX(Renglon) + 1 FROM VENTAD WITH (NOLOCK) WHERE ID = V.ID) AS Renglon,
(SELECT MAX(RenglonID) + 1 FROM VENTAD WITH (NOLOCK) WHERE ID = V.ID) AS RenglonID,
...
JOIN Cte C WITH (NOLOCK) ON V.CLIENTE = C.Cliente
LEFT OUTER JOIN CteEnviarA CA WITH (NOLOCK) ON V.Cliente = CA.Cliente AND V.EnviarA = CA.ID
JOIN EMPRESA E WITH (NOLOCK) ON V.Empresa = E.Empresa
LEFT OUTER JOIN AnexoCta AC WITH (NOLOCK) ON V.Empresa = AC.Cuenta AND CONVERT(VARCHAR(10), V.ContUso) + 'F' = AC.Nombre AND AC.Rama = 'EMP' AND AC.Tipo = 'Imagen'
LEFT OUTER JOIN Agente AG WITH (NOLOCK) ON V.Agente = AG.Agente
JOIN Sucursal S WITH (NOLOCK) ON V.Sucursal = S.Sucursal
