...
@MovID				varchar(20),
@Directo              bit
SELECT @MovRetencion = CxpRetencion FROM EmpresaCfgMov WITH (NOLOCK) WHERE Empresa = @Empresa
...
IF EXISTS(SELECT * FROM MovPos WITH (NOLOCK) WHERE Estacion = @@SPID AND Tipo = 'ORIGEN' AND OModulo = 'CXP' /*AND OMov = @MovRetencion*/)
INSERT INTO #Aplica(
...
JOIN #Cheques c WITH (NOLOCK) ON d.ID = c.ID
JOIN #Aplica dd WITH (NOLOCK) ON d.ID = dd.id 
JOIN MovTipo mt WITH (NOLOCK) ON mt.Modulo = 'DIN' AND mt.Mov = d.Mov
WHERE d.Empresa =  @Empresa
AND d.Estatus IN('CONCLUIDO', 'CONCILIADO')
