...
IF @EsPDF = 1
IF NOT EXISTS(SELECT * FROM AnexoMov WITH (NOLOCK) WHERE Rama = 'CXP' AND ID = @ID AND CFD = 1 AND Nombre = @NomArchivoPDF)
...
IF NOT EXISTS(SELECT * FROM AnexoMov WITH (NOLOCK) WHERE Rama = 'CXP' AND ID = @ModuloID AND CFD = 1 AND Nombre = @NomArchivoXML)
INSERT AnexoMov (Sucursal,  Rama,  ID,  Nombre,         Direccion,  Tipo,      Icono, CFD)
...
AND IDPago = @IDPago
IF @EsPDF = 1
IF NOT EXISTS(SELECT * FROM AnexoMov WITH (NOLOCK) WHERE Rama = @ModuloPago AND ID = @IDPago AND CFD = 1 AND Nombre = @NomArchivoPDF)
INSERT AnexoMov (Sucursal,  Rama,        ID,      Nombre,         Direccion,  Tipo,      Icono, CFD)
VALUES (@Sucursal, @ModuloPago, @IDPago, @NomArchivoPDF, @ArchivoPDF, 'Archivo', 745,   1)
IF NOT EXISTS(SELECT * FROM AnexoMov WITH (NOLOCK)  WHERE Rama = @ModuloPago AND ID = @IDPago AND CFD = 1 AND Nombre = @NomArchivoXML)
INSERT AnexoMov (Sucursal,  Rama,        ID,      Nombre,         Direccion,  Tipo,      Icono, CFD)
...
IF @EsPDF = 1
IF NOT EXISTS(SELECT * FROM AnexoMov WITH (NOLOCK) WHERE Rama = @ModuloAux AND ID = @ModuloID AND CFD = 1 AND Nombre = @NomArchivoPDF)
INSERT AnexoMov (Sucursal,  Rama,       ID,        Nombre,         Direccion,  Tipo,      Icono, CFD)
VALUES (@Sucursal, @ModuloAux, @ModuloID, @NomArchivoPDF, @ArchivoPDF, 'Archivo', 745,   1)
IF NOT EXISTS(SELECT * FROM AnexoMov WITH (NOLOCK) WHERE Rama = @ModuloAux AND ID = @ModuloID AND CFD = 1 AND Nombre = @NomArchivoXML)
INSERT AnexoMov (Sucursal,  Rama,       ID,        Nombre,         Direccion,  Tipo,      Icono, CFD)
