...
GO
ALTER PROCEDURE spCFDRutaArchivos
@Empresa     varchar(5),
@Modulo      varchar(5),
@Mov         varchar(20),
@RutaCancelacion bit,
...
SELECT @RutaOriginal = direccion FROM AnexoMov WITH (NOLOCK) WHERE Rama =@Modulo  AND  ID=@ID  AND Icono =17
IF @TipoDocumento ='PDF'
SELECT @RutaOriginal = direccion FROM AnexoMov WITH (NOLOCK) WHERE Rama =@Modulo  AND  ID=@ID  AND Icono =745
