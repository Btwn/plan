...
AS BEGIN
IF NOT EXISTS (SELECT IDPermiso FROM dbo.pNetPermiso WITH(NOLOCK) WHERE IDPermiso = @IDPermisoParent) AND @DescripcionParent IS NOT NULL
BEGIN
...
END
IF NOT EXISTS (SELECT IDRolePermiso FROM dbo.pNetRolePermiso WITH(NOLOCK) WHERE IDRole = @IDRole AND IDPermiso = @IDPermisoParent)
BEGIN
INSERT INTO dbo.pNetRolePermiso (IDRole, IDPermiso)
SELECT @IDRole, @IDPermisoParent
END
IF NOT EXISTS (SELECT IDPermiso FROM dbo.pNetPermiso WITH(NOLOCK) WHERE IDPermiso = @IDPermiso)
BEGIN
INSERT INTO dbo.pNetPermiso (IDPermiso, Descripcion, IDUsuarioTipo, Icon, Ruta, IDPermisoPadre, Mostrar)
SELECT @IDPermiso, @Descripcion, @IDUsuarioTipo, '', @RutaPNet, @IDPermisoParent, 1
END
IF NOT EXISTS (SELECT IDRolePermiso FROM dbo.pNetRolePermiso WITH(NOLOCK) WHERE IDRole = @IDRole AND IDPermiso = @IDPermiso)
BEGIN
