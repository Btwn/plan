...
@AjusteMov = CxcAjuste
FROM EmpresaCfgMov WITH(NOLOCK) WHERE Empresa = @Empresa
SELECT @RamaID = NULL
...
JOIN LC WITH(NOLOCK) on lc.LineaCredito = c.LineaCredito
...
SELECT @ImportePendiente = SUM(Saldo) FROM Cxc WITH(NOLOCK) WHERE RamaID = @RamaID
SELECT @ImporteConcluido = SUM(Importe) FROM Cxc WITH(NOLOCK) WHERE RamaID = @RamaID AND Estatus = 'CONCLUIDO'
SELECT @CxcID = MIN(ID) FROM Cxc WITH(NOLOCK) WHERE RamaID = @RamaID AND Estatus = 'PENDIENTE'
...
FROM TablaAmortizacion ta WITH(NOLOCK)
...
FROM Cxc c WITH(NOLOCK)
JOIN Mon m WITH(NOLOCK) ON m.Moneda = c.Moneda
WHERE ID = @RamaID
...
SELECT @DocMovID = MovID FROM Cxc WITH(NOLOCK) WHERE ID = @DocID
END
SELECT @Saldo = 0.0
SELECT @Saldo = Saldo FROM Cxc WITH(NOLOCK) WHERE ID = @DocID AND Estatus = 'PENDIENTE'
...
UPDATE Cxc WITH(ROWLOCK) SET Importe = @Saldo, Impuestos = NULL WHERE ID = @AjusteID
