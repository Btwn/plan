...
FROM CxC C WITH(NOLOCK) WHERE C.Mov IN ('Nota Credito', 'Nota Credito Mayoreo', 'Nota Credito VIU')
...
IF (@Mov='Endoso')  
BEGIN
SELECT @Mov=Aplica, @MovID=AplicaID FROM CxCD WITH(NOLOCK) WHERE ID=@ID
SELECT @Desglose = FacDesgloseIVA FROM CxC WITH(NOLOCK) WHERE Mov = @Mov AND MovID = @MovID
UPDATE CxC WITH (ROWLOCK) SET FacDesgloseIVA= @Desglose WHERE ID=@ID
END
