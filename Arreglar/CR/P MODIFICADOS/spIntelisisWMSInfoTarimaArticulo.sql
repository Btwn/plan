...
END
IF ISNULL(@ok,0) = 0 AND NOT EXISTS (SELECT TOP 1 Articulo FROM Art WITH(NOLOCK) WHERE Articulo = @Articulo)
BEGIN
SET @Ok = 1
...
SET Factor = b.Factor
FROM @Tabla a
JOIN ArtUnidad b  WITH(NOLOCK) ON (a.Articulo = b.Articulo AND a.UnidadTraspaso = b.Unidad)
UPDATE @Tabla
SET TarimaEstatus = b.Estatus,
EstatusControlCalidad = b.EstatusControlCalidad
FROM @Tabla a
JOIN Tarima b  WITH(NOLOCK) ON (a.Tarima = b.Tarima)
END
