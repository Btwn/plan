...
FROM VentaD WITH(NOLOCK), Venta WITH(NOLOCK), MovTipo WITH(NOLOCK), Art WITH(NOLOCK)
WHERE VentaD.ID = @ID
...
FROM CompraD WITH(NOLOCK), Compra WITH(NOLOCK), MovTipo WITH(NOLOCK), Art WITH(NOLOCK)
WHERE CompraD.ID = @ID
...
FROM InvD WITH(NOLOCK), Inv WITH(NOLOCK), MovTipo WITH(NOLOCK), Art WITH(NOLOCK)
WHERE InvD.ID = @ID
...
FROM ProdD d WITH(NOLOCK), Prod e WITH(NOLOCK), MovTipo WITH(NOLOCK), Art WITH(NOLOCK)
WHERE d.ID = @ID
...
SELECT @AlmacenOrigen = MIN(d.Almacen) FROM CompraD d WITH(NOLOCK) WHERE d.ID = @IDAplica AND d.Articulo = @Articulo AND ISNULL(d.SubCuenta, '') = ISNULL(@SubCuenta, '') AND d.Unidad = @MovUnidad AND Renglon = ISNULL(@AplicaRenglon,Renglon)
...
SELECT @AlmacenOrigen = MIN(d.Almacen) FROM VentaD d WITH(NOLOCK) WHERE d.ID = @IDAplica AND d.Articulo = @Articulo AND ISNULL(d.SubCuenta, '') = ISNULL(@SubCuenta, '') AND d.Unidad = @MovUnidad AND Renglon = ISNULL(@AplicaRenglon,Renglon)
/** 28.07.2006 **/
IF (@MovTipo = 'INV.R' AND @AplicaMovTipo = 'INV.P') OR (@MovTipo = 'INV.EI')
SELECT @AlmacenOrigen = MIN(d.Almacen) FROM InvD d WITH(NOLOCK) WHERE d.ID = @IDAplica AND d.Articulo = @Articulo AND ISNULL(d.SubCuenta, '') = ISNULL(@SubCuenta, '') AND d.Unidad = @MovUnidad AND Renglon = ISNULL(@AplicaRenglon,Renglon)
...
JOIN Inv WITH(NOLOCK) ON Inv.ID = InvD.ID
...
SELECT @ArtMoneda = MonedaCosto, @ArtTipo = UPPER(Tipo) FROM Art WITH(NOLOCK) WHERE Articulo = @Articulo
IF @SinAlmacen = 1
...
IF @Modulo = 'COMS' SELECT @Cliente = NULLIF(RTRIM(Cliente), ''), @DestinoTipo = DestinoTipo, @Destino = NULLIF(RTRIM(Destino), ''), @DestinoID = DestinoID FROM CompraD WITH(NOLOCK) WHERE ID = @IDAplica AND Renglon = @Renglon AND RenglonSub = @RenglonSub ELSE
IF @Modulo = 'INV'  SELECT @Cliente = NULLIF(RTRIM(Cliente), ''), @DestinoTipo = DestinoTipo, @Destino = NULLIF(RTRIM(Destino), ''), @DestinoID = DestinoID FROM InvD   WITH(NOLOCK) WHERE ID = @IDAplica AND Renglon = @Renglon AND RenglonSub = @RenglonSub ELSE
IF @Modulo = 'PROD' SELECT @Cliente = NULLIF(RTRIM(Cliente), ''), @DestinoTipo = DestinoTipo, @Destino = NULLIF(RTRIM(Destino), ''), @DestinoID = DestinoID FROM ProdD  WITH(NOLOCK) WHERE ID = @IDAplica AND Renglon = @Renglon AND RenglonSub = @RenglonSub
...
@Ok OUTPUT, @OkRef OUTPUT
ELSE*/
IF @Destino IS NOT NULL AND @DestinoID IS NOT NULL AND @DestinoTipo IN (SELECT Modulo FROM Modulo WITH(NOLOCK)) AND @Cancelar = 0
...
IF @NuevoPendiente IS NOT NULL AND ROUND(@NuevoPendiente, @Redondeo) = 0 UPDATE CompraD WITH(ROWLOCK) SET CantidadPendiente = NULL WHERE ID = @IDAplica AND Renglon = @Renglon AND RenglonSub = @RenglonSub
END ELSE
IF @Modulo = 'VTAS'
BEGIN
...
WHERE ID = @IDAplica AND Renglon = @Renglon AND RenglonSub = @RenglonSub
IF @NuevoPendiente IS NOT NULL AND ROUND(@NuevoPendiente, @Redondeo) = 0 UPDATE VentaD WITH(ROWLOCK) SET CantidadPendiente = NULL WHERE ID = @IDAplica AND Renglon = @Renglon AND RenglonSub = @RenglonSub
END ELSE
IF @Modulo = 'INV'
BEGIN
...
@NuevoPendiente = CantidadPendiente = NULLIF(ISNULL(CantidadPendiente, 0.0) - @PendienteObtenido, 0.0)
WHERE ID = @IDAplica AND Renglon = @Renglon AND RenglonSub = @RenglonSub
IF @NuevoPendiente IS NOT NULL AND ROUND(@NuevoPendiente, @Redondeo) = 0 UPDATE InvD WITH(ROWLOCK) SET CantidadPendiente = NULL WHERE ID = @IDAplica AND Renglon = @Renglon AND RenglonSub = @RenglonSub
END ELSE
IF @Modulo = 'PROD'
BEGIN
...
@NuevoPendiente = CantidadPendiente = NULLIF(ISNULL(CantidadPendiente, 0.0) - @PendienteObtenido, 0.0)
WHERE ID = @IDAplica AND Renglon = @Renglon AND RenglonSub = @RenglonSub
IF @NuevoPendiente IS NOT NULL AND ROUND(@NuevoPendiente, @Redondeo) = 0 UPDATE ProdD WITH(ROWLOCK) SET CantidadPendiente = NULL WHERE ID = @IDAplica AND Renglon = @Renglon AND RenglonSub = @RenglonSub
END
...
IF @@FETCH_STATUS <> -2
BEGIN
IF @Modulo = 'VTAS' UPDATE Venta  WITH(ROWLOCK) SET Saldo = CASE @Cancelar WHEN 1 THEN NULLIF(ISNULL(Saldo, 0.0) + @ImporteMatar, 0.0) ELSE NULLIF(ISNULL(Saldo, 0.0) - @ImporteMatar, 0.0) END WHERE ID = @IDAplica ELSE
IF @Modulo = 'COMS' UPDATE Compra WITH(ROWLOCK) SET Saldo = CASE @Cancelar WHEN 1 THEN NULLIF(ISNULL(Saldo, 0.0) + @ImporteMatar, 0.0) ELSE NULLIF(ISNULL(Saldo, 0.0) - @ImporteMatar, 0.0) END WHERE ID = @IDAplica
IF @@ERROR <> 0 SELECT @Ok = 1
END
FETCH NEXT FROM crMatarSaldos INTO @IDAplica,  @AplicaMov, @AplicaMovID, @AplicaMovTipo, @ImporteMatar
IF @@ERROR <> 0 SELECT @Ok = 1
...
FROM VentaD d WITH(NOLOCK), @MatarIndice m WITH(NOLOCK)
WHERE d.ID = m.ID
...
FROM CompraD d WITH(NOLOCK), @MatarIndice mWITH(NOLOCK)
WHERE d.ID = m.ID
GROUP BY m.ID, m.Mov, m.MovID, m.MovTipo, m.Estatus
ORDER BY m.ID, m.Mov, m.MovID, m.MovTipo, m.Estatus
ELSE
...
DECLARE crMatarPendiente CURSOR
FOR SELECT m.ID, m.Mov, m.MovID, m.MovTipo, m.Estatus, Sum(ISNULL(d.CantidadOrdenada, 0.0)) + Sum(ISNULL(d.CantidadReservada, 0.0)) + Sum(ISNULL(d.CantidadPendiente, 0.0))
FROM InvD d WITH(NOLOCK), @MatarIndice m WITH(NOLOCK)
WHERE d.ID = m.ID
GROUP BY m.ID, m.Mov, m.MovID, m.MovTipo, m.Estatus
ORDER BY m.ID, m.Mov, m.MovID, m.MovTipo, m.Estatus
ELSE
...
DECLARE crMatarPendiente CURSOR
FOR SELECT m.ID, m.Mov, m.MovID, m.MovTipo, m.Estatus, Sum(ISNULL(d.CantidadOrdenada, 0.0)) + Sum(ISNULL(d.CantidadReservada, 0.0)) + Sum(ISNULL(d.CantidadPendiente, 0.0))
FROM ProdD d WITH(NOLOCK), @MatarIndice m WITH(NOLOCK)
WHERE d.ID = m.ID
GROUP BY m.ID, m.Mov, m.MovID, m.MovTipo, m.Estatus
ORDER BY m.ID, m.Mov, m.MovID, m.MovTipo, m.Estatus
...
BEGIN
IF EXISTS(SELECT * FROM Inv WITH(NOLOCK) WHERE Empresa = @Empresa AND Estatus = 'CONFIRMAR' AND OrigenTipo = 'PROD' AND Origen = @AplicaMov AND OrigenID = @AplicaMovID)
SELECT @Ok = 25400
IF @MovTipo = 'PROD.E'
IF EXISTS(SELECT * FROM Inv WITH(NOLOCK) WHERE Empresa = @Empresa AND Estatus = 'PENDIENTE' AND OrigenTipo = 'PROD' AND Origen = @AplicaMov AND OrigenID = @AplicaMovID)
SELECT @Ok = 25400
END
END ELSE
...
IF @Modulo = 'VTAS' UPDATE Venta WITH(ROWLOCK) SET Estatus = @AplicaEstatusNuevo, FechaConclusion = @AplicaFechaConclusion WHERE ID = @IDAplica ELSE
IF @Modulo = 'COMS' UPDATE Compra WITH(ROWLOCK) SET Estatus = @AplicaEstatusNuevo, FechaConclusion = @AplicaFechaConclusion WHERE ID = @IDAplica ELSE
IF @Modulo = 'INV'  UPDATE Inv   WITH(ROWLOCK) SET Estatus = @AplicaEstatusNuevo, FechaConclusion = @AplicaFechaConclusion WHERE ID = @IDAplica ELSE
IF @Modulo = 'PROD' UPDATE Prod  WITH(ROWLOCK) SET Estatus = @AplicaEstatusNuevo, FechaConclusion = @AplicaFechaConclusion WHERE ID = @IDAplica
IF @@ERROR <> 0 SELECT @Ok = 1
...
BEGIN
IF @MovTipo = 'PROD.E'
IF EXISTS (SELECT e.ID FROM Inv e WITH(NOLOCK) JOIN InvD d WITH(NOLOCK) ON e.ID = d.ID WHERE e.Empresa = @Empresa AND e.Estatus = 'PENDIENTE' AND e.OrigenTipo = 'PROD' AND e.Origen = @AplicaMov AND e.OrigenID = @AplicaMovID
AND CantidadPendiente > 0 AND d.Producto IN (SELECT Articulo FROM ProdD WITH(NOLOCK) WHERE ID = @ID)
...
SELECT @Ok = 25400
IF @AplicaMovTipo = 'PROD.O'
IF EXISTS (SELECT e.ID FROM Inv e WITH(NOLOCK) JOIN InvD d WITH(NOLOCK) ON e.ID = d.ID WHERE e.Empresa = @Empresa AND e.Estatus = 'CONFIRMAR' AND e.OrigenTipo = 'PROD' AND e.Origen = @AplicaMov AND e.OrigenID = @AplicaMovID
AND CantidadPendiente > 0 AND d.Producto IN (SELECT Articulo FROM ProdD WITH(NOLOCK) WHERE ID = @ID)
AND CantidadPendiente > @CantidadPendiente*(SELECT ISNULL(m.Cantidad/m.CantidadP,1) FROM ProdProgramaMaterial m WITH(NOLOCK) WHERE m.ID = @IDAplica AND m.Producto = d.Producto AND m.Lote = d.ProdSerieLote AND m.Articulo = d.Articulo AND ISNULL(m.Subcuenta,'') = ISNULL(d.subcuenta,'')))
SELECT @Ok = 25400
END
END
FETCH NEXT FROM crMatarPendiente INTO @IDAplica, @AplicaMov, @AplicaMovID, @AplicaMovTipo, @AplicaEstatus, @CantidadPendiente
...
IF ROUND(@Cantidad, 4) > ROUND((SELECT Convert(float, Disponible) FROM ArtDisponible WITH(NOLOCK) WHERE Empresa = @Empresa AND Articulo = @Articulo AND Almacen = @AlmacenOrigen), 4) 
BEGIN
SELECT @Ok = 20290, @OkRef = 'Articulo: '+ (SELECT MIN(Articulo) FROM @AMatar WHERE Cantidad > 0 )
EXEC xpOk_20290 @Empresa, @Usuario, @Accion, @Modulo, @ID, @Ok OUTPUT, @OkRef OUTPUT
END
