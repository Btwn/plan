...
FROM EmpresaCfgMovVenta WITH(NOLOCK)
WHERE Empresa = @Empresa
...
BEGIN
SELECT @ZonaImpuesto = ZonaImpuesto FROM Cte WITH(NOLOCK) WHERE Cliente = @Cliente
SELECT @Tipo = Tipo, @Impuesto1 = Impuesto1, @Impuesto2 = Impuesto2, @Impuesto3 = Impuesto3 FROM Art WITH(NOLOCK) WHERE Articulo = @Articulo
...
IF @OK IS NULL
BEGIN
UPDATE Venta WITH(ROWLOCK) SET Importe = @Importe WHERE ID = @IDBonificacion
IF @@ERROR <> 0 SET @Ok = 1
END
IF @Ok IS NULL
EXEC spAfectar @Modulo, @IDBonificacion, @Accion,'TODO', NULL ,@Usuario, NULL, 1, @Ok OUTPUT, @OkRef OUTPUT, @Conexion = 1
SELECT @OMov = Mov, @OMovID = MovID FROM Venta WITH(NOLOCK) WHERE ID= @ID
SELECT @MovID = MovID FROM Venta WITH(NOLOCK) WHERE ID = @IDBonificacion
IF @OK IS NULL
...
IF @OK IS NULL
BEGIN
SELECT @MovID = MOVID FROM Venta WITH(NOLOCK) WHERE ID = @IDFactura
...
FROM MovFlujo mf WITH(NOLOCK) JOIN MovTipo mt WITH(NOLOCK)
...
SELECT @Mov = Mov, @MovID = MovID FROM Venta WITH(NOLOCK) WHERE ID = @ID
SELECT @Importe = Importe + Impuestos FROM Venta WITH(NOLOCK) WHERE ID = @IDBonificacion
IF @OK IS NULL
BEGIN
