...
IF EXISTS (SELECT * FROM eDocInNSPrefijo WITH(NOLOCK) WHERE Estacion = @Estacion)
...
END
UPDATE eDocInNSPrefijo  WITH(ROWLOCK) SET Prefijo = Prefijo +CHAR(58) WHERE Prefijo IS NOT NULL AND Estacion = @Estacion
END
