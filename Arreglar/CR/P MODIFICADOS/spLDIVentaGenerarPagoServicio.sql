...
SELECT TOP 1 @Articulo = Articulo, @Importe = ISNULL(Precio,0.0), @LDICuenta = LDICuenta, @LDIReferencia = LDIReferencia  FROM VentaD WITH (NOLOCK) WHERE ID = @ID
SELECT @Servicio = LDIServicio  FROM Art WITH (NOLOCK)  WHERE Articulo = @Articulo
