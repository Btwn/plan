...
FROM CFDINominaRecibo WITH (NOLOCK)
WHERE ID = @ID
...
SELECT @Ok = 10060, @OkRef = 'Tipo R�gimen'
...
SELECT @Ok = 10060, @OkRef = 'N�mero D�as Pagados'
IF @Ok IS NULL AND @PeriodicidadPago IS NULL
SELECT @Ok = 10060, @OkRef = 'Periodicidad de Pago'
IF @Ok IS NULL AND EXISTS(SELECT RID FROM CFDINominaPercepcionDeduccion WITH (NOLOCK) WHERE ID = @ID AND Personal = @Personal AND TipoSAT IS NULL)
SELECT @Ok = 10060, @OkRef = 'Tipo Percepci�n / Deducci�n '+Concepto FROM CFDINominaPercepcionDeduccion WITH (NOLOCK) WHERE ID = @ID AND Personal = @Personal AND TipoSAT IS NULL
...
SELECT @Ok = 10060, @OkRef = 'Clave Percepci�n / Deducci�n '+Concepto FROM CFDINominaPercepcionDeduccion WITH (NOLOCK) WHERE ID = @ID AND Personal = @Personal AND ClaveSAT IS NULL
IF @Ok IS NULL AND EXISTS(SELECT RID FROM CFDINominaIncapacidad WITH (NOLOCK) WHERE ID = @ID AND Personal = @Personal AND Dias IS NULL)
SELECT @Ok = 10060, @OkRef = 'D�as Incapacidad'
IF @Ok IS NULL AND EXISTS(SELECT RID FROM CFDINominaIncapacidad WITH (NOLOCK) WHERE ID = @ID AND Personal = @Personal AND TipoIncapacidad IS NULL)
SELECT @Ok = 10060, @OkRef = 'Tipo de Incapacidad'
IF @Ok IS NULL AND EXISTS(SELECT RID FROM CFDINominaIncapacidad WITH (NOLOCK) WHERE ID = @ID AND Personal = @Personal AND Descuento IS NULL)
SELECT @Ok = 10060, @OkRef = 'Descuento Incapacidad'
IF @Ok IS NULL AND EXISTS(SELECT RID FROM CFDINominaHoraExtra WITH (NOLOCK) WHERE ID = @ID AND Personal = @Personal AND Dias IS NULL)
SELECT @Ok = 10060, @OkRef = 'D�as Hora Extra'
IF @Ok IS NULL AND EXISTS(SELECT RID FROM CFDINominaHoraExtra WITH (NOLOCK) WHERE ID = @ID AND Personal = @Personal AND TipoHoras IS NULL)
SELECT @Ok = 10060, @OkRef = 'Tipo de Hora Extra'
IF @Ok IS NULL AND EXISTS(SELECT RID FROM CFDINominaHoraExtra WITH (NOLOCK) WHERE ID = @ID AND Personal = @Personal AND HorasExtra IS NULL)
SELECT @Ok = 10060, @OkRef = 'Horas Extra'
IF @Ok IS NULL AND EXISTS(SELECT RID FROM CFDINominaHoraExtra WITH (NOLOCK) WHERE ID = @ID AND Personal = @Personal AND ImportePagado IS NULL)
