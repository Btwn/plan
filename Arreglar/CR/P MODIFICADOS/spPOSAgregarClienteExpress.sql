...
IF EXISTS(SELECT 1 FROM Cte WITH (NOLOCK) WHERE Cliente = @Cliente AND @ClienteNuevo = 0)
UPDATE Cte WITH (ROWLOCK) Set Nombre = @Nombre,
Direccion = @Direccion,
...
ELSE
BEGIN
IF NOT EXISTS(SELECT * FROM ConsecutivoSucursal WITH (NOLOCK) WHERE Tipo = 'Cte' AND Sucursal = @Sucursal)
INSERT ConsecutivoSucursal(Tipo,  Sucursal, Consecutivo)
SELECT                     'CTE', @Sucursal, 0
SELECT @Consecutivo = ISNULL(Consecutivo,0)
FROM ConsecutivoSucursal cs WITH (NOLOCK)
WHERE cs.Tipo = 'Cte'
AND cs.Sucursal = @Sucursal
SELECT @Cliente = @Host + CONVERT(varchar, ISNULL(@Consecutivo,0) + 1)
UPDATE ConsecutivoSucursal WITH (ROWLOCK) SET Consecutivo = ISNULL(Consecutivo,0) +1
WHERE Tipo = 'Cte'
AND Sucursal = @Sucursal
IF EXISTS (SELECT 1 FROM Cte WITH (NOLOCK) WHERE Cliente = @Cliente)
BEGIN
SELECT @Cliente = NULL
END
