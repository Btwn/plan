...
@Rfc				varchar(20)
SELECT  @Curp = Curp, @FechaPago = FechaPago, @FechaInicialPago = FechaInicialPago, @FechaFinalPago = FechaFinalPago, @Departamento = Departamento,@Puesto	=Puesto ,@RfcEmisor=RfcEmisor, @Rfc=RFC FROM CFDINominaRecibo WITH (NOLOCK) WHERE RID = @ID  AND Personal =@Personal
SELECT @Asunto=EnviarAsuntoNomina from EmpresaCFD WITH (NOLOCK) where Empresa =@Empresa
