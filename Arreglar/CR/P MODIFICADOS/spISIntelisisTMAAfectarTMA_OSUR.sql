...
GO
ALTER PROCEDURE spISIntelisisTMAAfectarTMA_OSUR
@ID				int,
@iSolicitud		int,
@Version		float,
...
IF @Ok IS NULL
BEGIN
IF NOT EXISTS(SELECT * FROM TMA t WITH (NOLOCK) JOIN TMAD d WITH (NOLOCK) ON t.ID = d. ID  WHERE t.MovID = @MovID AND t.Mov = @Mov AND d.Tarima = @Tarima  AND t.Empresa = @Empresa AND t.Sucursal = @Sucursal) SELECT @Ok = 13110
IF NOT EXISTS(SELECT * FROM TMA t WITH (NOLOCK) JOIN TMAD d WITH (NOLOCK) ON t.ID = d. ID  WHERE t.MovID = @MovID AND t.Mov = @Mov AND d.Tarima = @Tarima AND d.Posicion = @Posicion  AND t.Empresa = @Empresa AND t.Sucursal = @Sucursal) SELECT @Ok = 13190
...
BEGIN
SELECT @Movimiento = TMASurtidoTransito FROM EmpresaCfgMovWMS WITH (NOLOCK) WHERE Empresa = @Empresa
