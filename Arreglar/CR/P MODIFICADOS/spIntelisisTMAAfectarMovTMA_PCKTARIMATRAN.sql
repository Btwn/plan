...
IF @Ok IS NULL
BEGIN
IF NOT EXISTS(SELECT * FROM Montacarga WITH(NOLOCK) WHERE Montacarga = @Montacargas)
SET @Ok = 20948
IF NOT EXISTS(SELECT * FROM TMA t  WITH(NOLOCK) JOIN TMAD d  WITH(NOLOCK) ON (t.ID = d.ID) WHERE d.Tarima = @Tarima AND d.PosicionDestino = @PosicionDestino AND t.Empresa = @Empresa AND t.Sucursal = @Sucursal AND t.Mov = @Mov AND t.MovID = @MovID)
BEGIN
IF NOT EXISTS(SELECT * FROM TMA t  WITH(NOLOCK) JOIN TMAD d  WITH(NOLOCK) ON (t.ID = d.ID) WHERE CASE WHEN CHARINDEX('-', d.Tarima, 1) >0 THEN SUBSTRING(d.Tarima, 1, CHARINDEX('-', d.Tarima, 1)-1) ELSE d.Tarima END = @Tarima AND d.PosicionDestino = @PosicionDestino AND t.Empresa = @Empresa AND t.Sucursal = @Sucursal AND t.Mov = @Mov AND t.MovID = @MovID)
SET @Ok = 13035
END
END
IF @Ok IS NULL
BEGIN
UPDATE TMAD
 WITH(ROWLOCK) SET CantidadA = CAST(@Cantidad AS float)
WHERE Tarima = @Tarima
AND ID = @ID2
