...
SELECT @ID2 = ID FROM Compra WITH(NOLOCK) WHERE Empresa = @Empresa AND Mov = @Mov AND MovID = @MovID
IF NOT EXISTS(SELECT * FROM AlmPos WITH(NOLOCK) WHERE Almacen = @Almacen AND Estatus = 'ALTA' AND Tipo = 'Recibo' AND Posicion = @Anden) SET @Ok = 13030
