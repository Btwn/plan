...
FROM WMSMontacargaTarea t
JOIN ListaID l WITH(NOLOCK) ON t.IDLista = l.ID AND t.Estacion = l.Estacion
WHERE t.Estacion = @Estacion
ORDER BY t.Tarima
OPEN crLista
...
CLOSE crLista
DEALLOCATE crLista
SELECT @Tipo = WMSTipoAcomodador FROM EmpresaCfg WITH(NOLOCK) WHERE Empresa = @Empresa
IF EXISTS(SELECT * FROM Agente WITH(NOLOCK) WHERE Agente = @Montacargista AND Tipo = @Tipo) AND @Error IS NULL  
UPDATE WMSMontacargaTarea WITH(ROWLOCK) SET Montacarga = @Montacargista WHERE Estacion = @Estacion AND IDLista IN(SELECT ID FROM ListaID WITH(NOLOCK) WHERE Estacion = @Estacion)
END
IF @Error IS NULL
