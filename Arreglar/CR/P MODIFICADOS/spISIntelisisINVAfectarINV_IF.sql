...
WITH(ID    varchar(255))
END
SELECT @Mov = Mov, @MovID = MovID, @Estatus = Estatus, @Empresa = Empresa FROM Inv WITH(NOLOCK) WHERE ID = @ID2
SELECT @MovTipo = Clave FROM MovTipo WITH(NOLOCK) WHERE Mov = @Mov AND Modulo = 'INV'
IF @MovTipo NOT IN('INV.IF')
SELECT @Ok = 35005, @OkRef = @Mov
IF @Ok IS NULL AND @Estatus = 'SINAFECTAR' AND @MovTipo = 'INV.IF'
BEGIN
SELECT @InvFisicoConteo = ISNULL(InvFisicoConteo,1) FROM EmpresaCfg WITH(NOLOCK) WHERE Empresa = @Empresa
SELECT @Mov3 = InvAjuste FROM EmpresaCfgMov WITH(NOLOCK) WHERE Empresa = @Empresa
/*
