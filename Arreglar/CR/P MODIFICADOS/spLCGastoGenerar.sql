...
SELECT @GastoMov = ACGasto FROM EmpresaCfgMov WITH(NOLOCK) WHERE Empresa = @Empresa
IF @Acreedor IS NULL SELECT @Ok = 40020
SELECT @Condicion = Condicion FROM Prov WITH(NOLOCK) WHERE Proveedor = @Acreedor
...
FROM LCGasto g WITH(NOLOCK)
LEFT OUTER JOIN Concepto c WITH(NOLOCK) ON c.Concepto = g.Concepto AND c.Modulo = 'GAS'
...
IF EXISTS(SELECT * FROM GastoD WITH(NOLOCK) WHERE ID = @GastoID)
BEGIN
EXEC spAfectar 'GAS', @GastoID, @Conexion = 1, @EnSilencio = 1, @Ok = @Ok OUTPUT, @OkRef = @OkRef OUTPUT
IF @Ok = 80030 SELECT @Ok = NULL, @OkRef = NULL
SELECT @GastoMovID = MovID FROM Gasto WITH(NOLOCK) WHERE ID = @GastoID
