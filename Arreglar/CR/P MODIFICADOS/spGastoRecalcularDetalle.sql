...
WHERE  ID = @ID
SELECT @ZonaImpuestos = ZonaImpuesto FROM Prov WITH (NOLOCK) WHERE Proveedor = @Acreedor
SELECT @MovClave = Clave FROM MovTipo WITH (NOLOCK) WHERE Mov = @Mov AND Modulo = 'GAS'
SELECT @TipoImpuesto = ISNULL(TipoImpuesto,0) FROM EmpresaGral WITH (NOLOCK) WHERE Empresa = @Empresa
SELECT @GastoSolicitudAnticipoImpuesto =ISNULL(GastoSolicitudAnticipoImpuesto,0), @Gasto2Retenciones = ISNULL(Gasto2Retenciones,0), @Gasto3Retenciones = ISNULL(Gasto3Retenciones,0) FROM EmpresaCfg2 WITH (NOLOCK) WHERE Empresa = @Empresa
...
BEGIN
SELECT @ConceptoImpuestos = Impuestos, @ConceptoRetencion = Retencion, @Conceptoretencion2 = Retencion2, @Conceptoretencion3 = Retencion3 FROM Concepto WITH (NOLOCK) WHERE Concepto = @Concepto AND Modulo = 'GAS'
...
UPDATE GastoD  WITH(ROWLOCK) SET Impuesto1 = @Impuesto1,
Impuesto2 = @Impuesto2,
Impuesto3 = @Impuesto3,
...
AND Renglon = @Renglon
IF @@ERROR <> 0 SET @Ok = 1
SELECT @SubTotal = ISNULL(Importe,0.0)-ISNULL(Retencion3,0.0) FROM GastoD WITH (NOLOCK) WHERE ID = @ID AND Renglon = @Renglon
IF @TipoImpuesto = 1
BEGIN
...
FROM GastoD WITH (NOLOCK) WHERE ID = @ID AND Renglon = @Renglon
...
SELECT @RetencionGasto3 = Importe*@Retencion3/100  FROM GastoD WITH (NOLOCK) WHERE ID = @ID AND Renglon = @Renglon
END
ELSE
BEGIN
...
SELECT @RetencionGasto3 =  ROUND(Importe*@Conceptoretencion3/100,4) FROM GastoD WITH (NOLOCK) WHERE ID = @ID AND Renglon = @Renglon
END
END
ELSE
SELECT @Impuestos = Impuestos,@RetencionGasto1 = Retencion, @RetencionGasto2 = Retencion2,@RetencionGasto3 = Retencion3   FROM GastoD WITH (NOLOCK) WHERE ID = @ID AND Renglon = @Renglon
END
IF @TipoImpuesto = 1 AND @Impuesto2Info = 0
SELECT @Impuestos2 = (@SubTotal*Impuesto2/100)
FROM GastoD WITH (NOLOCK) WHERE ID = @ID AND Renglon = @Renglon
IF @TipoImpuesto = 1 AND @Impuesto3Info = 0
SELECT   @Impuestos3 =   Cantidad*Impuesto3  FROM GastoD WITH (NOLOCK) WHERE ID = @ID AND Renglon = @Renglon
UPDATE GastoD  WITH(ROWLOCK) SET Impuestos = ISNULL(@Impuestos,0.0), Impuestos2 = ISNULL(@Impuestos2,0.0), Impuestos3 = ISNULL(@Impuestos3,0.0), Retencion = @RetencionGasto1, Retencion2 = @RetencionGasto2, Retencion3 = @RetencionGasto3
...
CLOSE crGastoD
DEALLOCATE crGastoD
SELECT @Impuestos = SUM(ISNULL(Impuestos,0.0)), @Retenciones = SUM(ISNULL(Retencion,0.0) + ISNULL(Retencion2,0.0) + ISNULL(Retencion3,0.0)) FROM GastoD WITH (NOLOCK) WHERE ID = @ID
UPDATE Gasto  WITH(ROWLOCK) SET Impuestos = @Impuestos, Saldo = Importe + @Impuestos - @Retenciones WHERE ID = @ID
IF @@ERROR <> 0 SET @Ok = 1
RETURN
END
