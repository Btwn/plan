...
SELECT @Moneda = Valor FROM AspelCfg WITH (NOLOCK) WHERE Descripcion = 'Moneda'
SELECT @Usuario = Valor FROM AspelCfg WITH (NOLOCK) WHERE Descripcion = 'Usuario'
INSERT ArtLinea (Linea,Clave)
SELECT DISTINCT A.Linea, A.Linea from AspelCargaProp A WITH (NOLOCK) 
WHERE Campo = 'Articulo' AND A.Linea <> ''
and a.linea not in (select LINEA from artlinea WITH (NOLOCK))
IF NOT EXISTS(SELECT Categoria FROM ArtCat WITH (NOLOCK) WHERE Categoria = @Observaciones)
...
WHERE Campo = 'Articulo' and VALOR not in (select articulo from art WITH (NOLOCK))
SELECT @Conta = COUNT(*) FROM Art WITH (NOLOCK) 
