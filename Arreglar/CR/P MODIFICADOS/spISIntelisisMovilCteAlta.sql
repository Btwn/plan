...
SELECT @DefMoneda = MonedaBase FROM MovilUsuarioCfg WITH (NOLOCK) WHERE Usuario = @Usuario
SELECT @Prefijo=ISNULL(Prefijo,'C'), @Nivel=Nivel, @Consecutivo=ISNULL(Consecutivo,0) FROM Consecutivo WITH (NOLOCK) WHERE tipo = 'CTE'
SELECT @Cliente = @Prefijo + CONVERT(varchar(4),@Consecutivo + 1)
IF (@Cliente IS NULL)
BEGIN
WHILE (EXISTS(SELECT Cliente FROM Cte WITH (NOLOCK) WHERE Cliente = @Cliente) OR @Cliente IS NULL) AND @Ok IS NULL
BEGIN
...
FROM EmpresaGral EG WITH (NOLOCK)
JOIN MovilUsuarioCfg MU WITH (NOLOCK) ON EG.Empresa = MU.Empresa AND MU.Usuario = @Usuario
END
END
IF @Cliente IS NULL
SET @Ok = 40010
...
IF (CASE WHEN LEN(@RFC) = 10 THEN ISNUMERIC(RIGHT(@RFC,6)) ELSE 1 END) = 1 AND NOT EXISTS(SELECT RFC FROM Cte WITH (NOLOCK) WHERE RFC = @RFC AND Cliente <> @Cliente)
BEGIN
EXEC spRegistroOk 'RFC', @RFC, '', 1 , @Correcto OUTPUT
END
IF ISNULL(@Correcto,0) = 0
SET @Ok = 60260
IF NOT EXISTS(SELECT * FROM Cte WITH (NOLOCK) WHERE RFC = @RFC)
BEGIN
INSERT Cte
