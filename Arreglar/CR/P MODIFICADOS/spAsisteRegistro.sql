...
FROM EmpresaCfgAcceso WITH(NOLOCK)
WHERE Empresa = @Empresa
SELECT @EstaPresente = ~@EstaPresente 
UPDATE Personal WITH(ROWLOCK) SET EstaPresente = @EstaPresente WHERE Personal = @Personal
IF @EstaPresente = 1
BEGIN
...
SELECT @ID = MIN(ID) FROM Asiste WITH(NOLOCK) WHERE Empresa = @Empresa AND Mov = 'Registro' AND Estatus = 'BORRADOR' AND FechaEmision = @Hoy AND FechaAplicacion = @FechaAplicacion AND Sucursal = @Sucursal
ELSE
SELECT @ID = MIN(ID) FROM Asiste WITH(NOLOCK) WHERE Empresa = @Empresa AND Mov = 'Registro' AND Estatus = 'BORRADOR' AND FechaEmision = @Hoy AND FechaAplicacion = @FechaAplicacion
IF @ID IS NULL
BEGIN
...
END
SELECT @Renglon = 0.0
SELECT @Renglon = ISNULL(MAX(Renglon), 0.0) + 2048.0 FROM AsisteD WITH(NOLOCK) WHERE ID = @ID
