...
SELECT @SaldoMN = SUM(s.Saldo*m.TipoCambio) FROM CxcSaldo s WITH (NOLOCK), Mon m WITH (NOLOCK) WHERE s.Moneda = m.Moneda AND Empresa=@Empresa AND Cliente=@Cliente
...
SELECT @Datos3 =(SELECT CteCredito.* FROM Cte  c WITH (NOLOCK) LEFT OUTER JOIN CteCredito WITH (NOLOCK) ON @empresa=CteCredito.Empresa AND c.Credito=CteCredito.Credito  WHERE c.Cliente = @Cliente FOR XML AUTO)
