...
FROM MFACuentaComplementaria WITH (NOLOCK) 
...
SELECT @Moneda = Moneda FROM MFACuentaComplementaria WITH (NOLOCK)  WHERE Cuenta = @Cuenta
EXEC spMFAContAcum @Cuenta = @Cuenta
END
SELECT @EjercicioAnt = MIN(Ejercicio) - 2 FROM Acum WITH (NOLOCK) 
WHILE(1=1)
BEGIN
SELECT @Ejercicio = MIN(Ejercicio)
FROM Acum WITH (NOLOCK) 
...
FROM MFAPolizaComp WITH (NOLOCK) 
...
FROM layout_cuentas a WITH (NOLOCK) 
...
FROM layout_polizas a WITH (NOLOCK) 
