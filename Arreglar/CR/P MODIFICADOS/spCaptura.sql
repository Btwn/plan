...
@Catalogo = NULLIF(RTRIM(Catalogo), ''), @Clave = NULLIF(RTRIM(Clave), '')
FROM Captura WITH (NOLOCK)
WHERE ID = @ID
...
BEGIN
EXEC spCapturaVerificar @ID, @Accion, @Empresa, @Usuario, @Modulo, @Mov, @MovID, @MovTipo, @MovMoneda, @MovTipoCambio, @FechaEmision, @Estatus, @EstatusNuevo,
@Catalogo, @Clave,
...
EXEC spCapturaAfectar @ID, @Accion, @Base, @Empresa, @Modulo, @Mov, @MovID OUTPUT, @MovTipo, @MovMoneda, @MovTipoCambio, @FechaEmision, @FechaAfectacion, @FechaConclusion,
