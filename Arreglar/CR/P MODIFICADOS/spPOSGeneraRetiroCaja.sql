...
GO
ALTER PROCEDURE spPOSGeneraRetiroCaja
@Empresa		varchar(5),
@Sucursal		int,
@Usuario		varchar(10),
@ID				varchar(36),
...
BEGIN
IF @MovClave = 'POS.TRM'
SELECT TOP 1 @Mov = Mov FROM MovTipo WITH (NOLOCK) WHERE Modulo = 'POS' AND Clave = 'POS.TCRC'
IF @MovClave = 'POS.CTRM'
SELECT TOP 1 @Mov = Mov FROM MovTipo WITH (NOLOCK) WHERE Modulo = 'POS' AND Clave = 'POS.CTCRC'
