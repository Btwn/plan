...
@Religion					varchar(50)
BEGIN TRANSACTION
IF EXISTS(SELECT * FROM POSValeSerie WITH (NOLOCK) WHERE Serie = @Monedero)
SELECT @Cliente = Cliente FROM POSValeSerie WITH (NOLOCK) WHERE Serie = @Monedero
...
IF @Ok IS NULL AND NOT EXISTS (SELECT * FROM POSValeSerie WITH (NOLOCK) WHERE Serie = @Monedero) AND @MonederoLDI = 0
BEGIN
