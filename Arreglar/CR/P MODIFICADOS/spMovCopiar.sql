...
BEGIN
IF (SELECT VentaAnexarOrdenes FROM EmpresaCfg2 WITH(NOLOCK) WHERE Empresa = @Empresa) = 1
BEGIN
SELECT @AnexarOrdenes = 1
SELECT @AnexoID = ISNULL(AnexoID, ID) FROM Venta WITH(NOLOCK) WHERE ID = @ID
END
END
...
IF @Modulo = 'VTAS'
SELECT @Max = ISNULL(MAX(CONVERT(int, SUBSTRING(MovID, LEN(@MasterMovID)+2, 20))), 0) FROM Venta WITH(NOLOCK) WHERE Empresa = @Empresa AND Mov = @GenerarMov AND MovID LIKE RTRIM(@MasterMovID)+'-%'
ELSE
SELECT @Max = ISNULL(MAX(CONVERT(int, SUBSTRING(MovID, LEN(@MasterMovID)+2, 20))), 0) FROM Mov WITH(NOLOCK) WHERE Empresa = @Empresa AND Mov = @GenerarMov AND MovID LIKE RTRIM(@MasterMovID)+'-%'
...
(SELECT ISNULL(ArrastrarTipoCambioGenerarMov, 0) FROM MovTipo WITH(NOLOCK) WHERE Modulo = @Modulo AND Mov = @Mov) = 1
...
BEGIN
IF @Modulo = 'VTAS'
UPDATE Venta WITH(ROWLOCK) SET AnexoID = @AnexoID WHERE ID = @GenerarID
END
END
ELSE
BEGIN
...
IF EXISTS(SELECT * FROM EmpresaCfgModulo WITH(NOLOCK) WHERE Empresa = @Empresa AND Modulo = @Modulo AND Tiempos = 'Si')
