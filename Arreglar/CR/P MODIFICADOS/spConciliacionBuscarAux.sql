...
SELECT @DineroMov = Mov, @DineroMovID = MovID, @ContID = ContID FROM Dinero WITH (NOLOCK) WHERE ID = @DineroID
...
IF @MovTipo = 'CONC.BC'
IF (SELECT Estatus FROM Cont WITH (NOLOCK) WHERE ID = @ContID) = 'CONCLUIDO'
BEGIN
IF @Cargo IS NOT NULL
