...
SELECT @MovCompra = CompraOrden FROM EmpresaCfgMov ecm WITH(NOLOCK) WHERE ecm.Empresa = @Empresa
...
FROM CompraMES cm WITH(NOLOCK)
JOIN CompraDMES cd WITH(NOLOCK) ON cm.Lanzamiento = cd.Lanzamiento AND cd.NumerodePedido = cm.NumerodePedido
JOIN Art a WITH(NOLOCK) ON cd.Articulo = a.Articulo
LEFT OUTER JOIN MonMES mm WITH(NOLOCK) ON cm.Moneda = mm.Moneda
LEFT OUTER JOIN UnidadMes um WITH(NOLOCK) ON cd.UnidadPedida = um.UnidadMedida
LEFT OUTER JOIN ArtCostoSucursal acs WITH(NOLOCK) ON acs.Empresa = @Empresa AND acs.Sucursal = ISNULL(cm.Sucursal, 0) AND acs.Articulo = cd.Articulo
...
FROM #MesCompra mc
JOIN Art a WITH(NOLOCK) ON mc.Articulo = a.Articulo AND (a.TipoArticulo <> '3')
...
FROM	VentaDMES WITH(NOLOCK)
WHERE	Lanzamiento = @Lanzamiento
SELECT @IDCompra = NULL
SELECT @TipoCambio = m.TipoCambio FROM Mon m WITH(NOLOCK) WHERE m.Moneda = @Moneda
SELECT @ZonaImpuesto = p.ZonaImpuesto FROM Prov p WITH(NOLOCK) WHERE p.Proveedor = @Proveedor
...
END
UPDATE CompraMes WITH(ROWLOCK) SET EstatusIntelIMES = 1, ModuloID = @IDCompra
WHERE Serie = @Serie AND NumerodePedido = @Numero AND Lanzamiento = @Lanzamiento AND Proveedor = @Proveedor
UPDATE CompraDMes WITH(ROWLOCK) SET CantidadPendiente = Cantidad, ModuloID = @IDCompra, EstatusIntelIMES = 1
...
FROM #MesCompra mc
JOIN Art a WITH(NOLOCK) ON mc.Articulo = a.Articulo AND a.TipoArticulo = '3'
LEFT OUTER JOIN ArtProv ap WITH(NOLOCK) ON ap.Articulo = a.Articulo AND ap.Proveedor = @Proveedor
...
UPDATE CompraMes WITH(ROWLOCK) SET EstatusIntelIMES = 1, ModuloID = @IDCompra
WHERE Serie = @Serie AND NumerodePedido = @Numero AND Lanzamiento = @Lanzamiento AND Proveedor = @Proveedor
UPDATE CompraDMes WITH(ROWLOCK) SET CantidadPendiente = Cantidad, ModuloID = @IDCompra
...
Proyecto         varchar(50)  NULL,
SubProyecto      varchar(50)  NULL)
IF NOT EXISTS(SELECT mt.Clave FROM MovTipo mt WITH(NOLOCK) WHERE mt.Modulo = 'INV' AND mt.Mov = 'Entrada Produccion')
INSERT INTO MovTipo
...
IF NOT EXISTS(SELECT mt.Clave FROM MovTipo mt WITH(NOLOCK) WHERE mt.Modulo = 'INV' AND mt.Mov = 'Consumo Produccion')
INSERT INTO MovTipo
(Modulo, Mov,                 Orden, Clave, SubClave, ConsecutivoModulo, ConsecutivoMov,       Factor)
VALUES('INV', 'Consumo Produccion', 2010, 'INV.S', NULL,    'INV',             'Consumo Produccion', 1     )
IF NOT EXISTS(SELECT mt.Clave FROM MovTipo mt WITH(NOLOCK) WHERE mt.Modulo = 'COMS' AND mt.Mov = 'EntradaCompraMaquila')
INSERT INTO MovTipo
(Modulo, Mov,                 Orden, Clave, SubClave, ConsecutivoModulo, ConsecutivoMov,       Factor)
VALUES('COMS', 'EntradaCompraMaquila', 50, 'COMS.EG', NULL,    'CXP',             'EntradaCompraMaquila', 1     )
IF NOT EXISTS(SELECT mt.Clave FROM MovTipo mt WITH(NOLOCK) WHERE mt.Modulo = 'CXP' AND mt.Mov = 'EntradaCompraMaquila')
INSERT INTO MovTipo
(Modulo, Mov,                 Orden, Clave, SubClave, ConsecutivoModulo, ConsecutivoMov,       Factor)
...
FROM MovMES mm WITH(NOLOCK)
LEFT OUTER JOIN MonMES m WITH(NOLOCK) ON mm.Moneda = m.Moneda
LEFT OUTER JOIN UnidadMes um WITH(NOLOCK) ON mm.Unidad = um.UnidadMedida
...
FROM #MesInv mi
JOIN Art a WITH(NOLOCK) ON mi.Articulo = a.Articulo
...
FROM VentaMES vm WITH(NOLOCK)
INNER JOIN VentaDMES vdm WITH(NOLOCK) ON vdm.id = vm.ID
...
IF NOT EXISTS(SELECT * FROM SerieLoteProp slp WITH(NOLOCK) WHERE slp.Propiedades = @Propiedades)
...
END
UPDATE MovMES WITH(ROWLOCK) SET ProcesadoINTELISIS = 1 WHERE ID = @ID
FETCH NEXT FROM crMesInvD INTO @ID, @Articulo, @SubCuenta, @Cantidad, @Lote, @Unidad, @Costo, @CosteMaquinaria, @CosteMaterial, @CosteIndirecto, @CosteOperarios, @ArtTipo, @OrdenProduccion
END
CLOSE crMesInvD
...
BEGIN
SELECT @IDInv = NULL
SELECT @TipoCambio = m.TipoCambio FROM Mon m WITH(NOLOCK) WHERE m.Moneda = 'Pesos'
INSERT INTO Inv(Empresa, Mov, FechaEmision, Referencia, Moneda, TipoCambio, Usuario, Estatus, Almacen, OrigenTipo, Sucursal, Proyecto, MesLanzamiento)
...
FROM CotizaMes cm WITH(NOLOCK)
JOIN CotizaDMES cd WITH(NOLOCK) ON cm.ID = cd.ID
LEFT OUTER JOIN MonMES m WITH(NOLOCK) ON cm.Moneda = m.Moneda
LEFT OUTER JOIN UnidadMes um WITH(NOLOCK) ON cd.UnidadPedida = um.UnidadMedida
...
SELECT @TipoCambio = m.TipoCambio FROM Mon m WITH(NOLOCK) WHERE m.Moneda = @Moneda
SELECT top 1 @Origen = Mov, @OrigenID = MovID, @IDOport = ID FROM Oportunidad WITH(NOLOCK) WHERE Estatus = 'REVISION' AND Proyecto = @SubProyecto
...
CLOSE crMesInvD
DEALLOCATE crMesInvD
UPDATE CotizaMES WITH(ROWLOCK) SET EstatusIntelIMES = 1 WHERE ID = @ID
