...
SELECT @FechaEmision = FechaEmision, @FechaRegistro = FechaRegistro, @Usuario = Usuario, @TipoCambio = TipoCambio, @Sucursal = Sucursal FROM Venta WITH (NOLOCK)   WHERE ID = @ID
SELECT @Mov = MovFactura FROM POSCfg WITH (NOLOCK) WHERE Empresa = @Empresa
SELECT @Contacto = @Contacto FROM MovTipoCFDFlex WITH (NOLOCK) WHERE Modulo = @Modulo AND Mov = @Mov
SELECT @MovIDTemp = NombreArchivo, @XML = DocumentoXML FROM POSL WITH (NOLOCK) WHERE ID = @IDPOS
...
SELECT @RutaTemporal = RutaTemporal, @EnviarAlAfectar = EnviarAlAfectar, @RutaTimbrarCFDI = RutaTimbrarCFDI FROM EmpresaCFD WITH (NOLOCK) WHERE Empresa = @Empresa
...
IF @Ok IS NULL
BEGIN
UPDATE CFD WITH (ROWLOCK) SET GenerarPDF = 1 WHERE Modulo = @Modulo AND ModuloID = @ID
IF @@ERROR <> 0 SET @Ok = 1
END
END
END
IF EXISTS(SELECT * FROM CFDFlexTemp WITH (NOLOCK) WHERE ID = @ID AND Modulo = @Modulo)
