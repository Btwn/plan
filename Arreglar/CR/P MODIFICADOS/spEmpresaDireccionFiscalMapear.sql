...
SELECT @Empresa = MIN(Clave) FROM ListaSt WITH (NOLOCK) WHERE Estacion = @Estacion
...
SELECT @CodigoPostal = CodigoPostal FROM Empresa WITH (NOLOCK) WHERE Empresa = @Empresa
IF ISNULL(@CodigoPostal,'') <> '' AND ((SELECT COUNT(1) FROM SATDireccionFiscal WITH (NOLOCK)  WHERE ClaveCP = @CodigoPostal) = 1)
BEGIN
...
FROM SATDireccionFiscal WITH (NOLOCK) WHERE ClaveCP = @CodigoPostal
IF ISNULL(@ClaveCP,'') <> '' AND ISNULL(@ClavePais,'') <> '' AND ISNULL(@ClaveEstado,'') <> '' AND ISNULL(@ClaveMunicipio,'') <> ''
AND ISNULL(@ClaveColonia,'') <> '' 
BEGIN
UPDATE EmpresaDireccionFiscal WITH (ROWLOCK)SET Mapeado = 1, Icono = 434, ClaveCP = @ClaveCP, ClavePais = @ClavePais, ClaveEstado = @ClaveEstado,
...
END
END
SELECT @Empresa = MIN(Clave) FROM ListaSt WITH (NOLOCK) WHERE Estacion = @Estacion AND Clave > @Empresa
END
RETURN
END
