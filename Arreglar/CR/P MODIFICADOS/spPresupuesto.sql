...
@PPTO	bit,
@Dif	money
IF (SELECT ISNULL(PresupuestoNivelUEN, 0) FROM Cta WITH(NOLOCK) WHERE Cuenta = @Cuenta) = 0 SELECT @UEN = NULL
...
UPDATE Presupuesto WITH(ROWLOCK)
...
SELECT @PPTO = PPTO FROM EmpresaGral WITH(NOLOCK) WHERE Empresa = @Empresa
IF @PPTO = 1
BEGIN
