...
'Empresa'           = v.Empresa,
'Moneda'            = v.Moneda,
'Categoria'         = CASE WHEN ISNULL(c.Categoria,'(Sin Categor�a)') = '' THEN '(Sin Categor�a)' ELSE ISNULL(c.Categoria,'(Sin Categor�a)') END,
...
FROM VentaCalc v
 WITH(NOLOCK) JOIN Cte c  WITH(NOLOCK) ON v.Cliente = c.Cliente
...
GROUP BY v.Moneda, ISNULL(c.Categoria,'(Sin Categor�a)'), dbo.fnTipoCambio(v.Moneda), e.ContMoneda, v.Empresa
) AS x
...
'Estacion'			= @EstacionTrabajo,
'Moneda2'           = v.Moneda,
'Categoria'         = CASE WHEN ISNULL(c.Categoria,'') = '' THEN '(Sin Categor�a)' ELSE c.Categoria END,
'ImporteTotal'      = SUM(v.ImporteTotal*mt.Factor),
