...
SELECT TOP 1 @AlmacenB = Almacen,@ArticuloB=Articulo  FROM invArtUbicaciones WITH(NOLOCK) WHERE Articulo=@Articulo AND Renglon=@Renglon AND ID =@ID
...
IF NOT EXISTS(SELECT   Articulo FROM invArtUbicaciones WITH(NOLOCK) WHERE Articulo=@Articulo AND Renglon=@Renglon AND ID =@ID)
