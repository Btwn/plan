...
(SELECT Direccion FROM AnexoCta WITH (NOLOCK) WHERE RAMA = 'EMP' AND CUENTA = V.Empresa AND TIPO = 'Imagen' AND Nombre = CONVERT(VARCHAR(10), V.UEN) + 'F') AS CODIGO,
(SELECT NOMBRE FROM FISCALREGIMEN WITH (NOLOCK) WHERE FiscalRegimen = E.FiscalRegimen) AS REGIMENFISCAL,
CX.FormaCobro1,
CX.FormaCobro2,
CX.FormaCobro3,
CX.FormaCobro4,
...
LEFT OUTER JOIN VENTATCALC V WITH (NOLOCK) ON CX.Referencia = V.Mov + ' ' + V.MovID
LEFT OUTER JOIN VENTA VE WITH (NOLOCK) ON V.ID = VE.ID
JOIN Cte C WITH (NOLOCK) ON CX.CLIENTE = C.Cliente
LEFT OUTER JOIN CteEnviarA CA WITH (NOLOCK) ON CX.Cliente = CA.Cliente AND CX.ClienteEnviarA = CA.ID
LEFT OUTER JOIN ART A WITH (NOLOCK) ON V.ARTICULO = A.ARTICULO
LEFT OUTER JOIN VENTAD VD WITH (NOLOCK) ON V.ID = VD.ID AND V.Renglon = VD.Renglon AND V.RenglonID = VD.RenglonID
JOIN EMPRESA E WITH (NOLOCK) ON CX.Empresa = E.Empresa
LEFT OUTER JOIN AnexoCta AC WITH (NOLOCK) ON CX.Empresa = AC.Cuenta AND CONVERT(VARCHAR(10), CX.ContUso) + 'F' = AC.Nombre AND AC.Rama = 'EMP' AND AC.Tipo = 'Imagen'
LEFT OUTER JOIN Agente AG WITH (NOLOCK) ON CX.Agente = AG.Agente
JOIN Sucursal S WITH (NOLOCK) ON CX.Sucursal = S.Sucursal
LEFT OUTER JOIN CFD i WITH (NOLOCK) ON CX.ID = i.ModuloID AND i.Modulo = 'CXC'
LEFT OUTER JOIN VentaCobro VC WITH (NOLOCK) ON V.ID = VC.ID
WHERE CX.ID = @ID
