...
AS BEGIN
IF NOT EXISTS(SELECT * FROM FormaVirtual  WITH (NOLOCK)  WHERE FormaVirtual = @FormaVirtualA)
BEGIN
...
FROM FormaVirtualCarpeta  WITH (NOLOCK)
WHERE FormaVirtual = @FormaVirtualD
...
FROM FormaVirtualCampo  WITH (NOLOCK)
WHERE FormaVirtual = @FormaVirtualD
...
FROM FormaVirtualAccion  WITH (NOLOCK)
WHERE FormaVirtual = @FormaVirtualD
END
RETURN
END
