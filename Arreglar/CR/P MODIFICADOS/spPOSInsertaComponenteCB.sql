...
FROM POSArtComponente WITH (NOLOCK)
WHERE ArticuloP = @ArticuloPrincipal
IF NOT EXISTS(SELECT * FROM POSArtComponente WITH (NOLOCK) WHERE ArticuloP = @ArticuloPrincipal AND Articulo = @Articulo AND SubCuenta = @SubCuenta )
SELECT @Ok = 4, @Mensaje = 'EL ARTICULO NO CORRESPONDE AL JUEGO'
IF @Ok IS NULL
BEGIN
SELECT TOP 1 @JuegoC = a.Juego
FROM POSArtJuegoComponente a WITH (NOLOCK)
...
AND @Articulo IN (SELECT Articulo FROM POSArtComponente WITH (NOLOCK) WHERE  ArticuloP = @ArticuloPrincipal AND Juego = a.Juego AND SubCuenta = @SubCuenta)
AND a.Opcional = 1
