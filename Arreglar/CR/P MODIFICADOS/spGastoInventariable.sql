...
FROM GastoD gd  WITH (NOLOCK) JOIN Gasto G  WITH (NOLOCK) ON gd.ID = g.ID
JOIN Concepto c  WITH (NOLOCK) ON gd.Concepto = c.Concepto
JOIN dbo.MovTipo mt  WITH (NOLOCK) ON mt.Mov = g.Mov
