...
BEGIN
IF @MovClave = 'POS.TCM'
SELECT TOP 1 @Mov = Mov FROM MovTipo WITH (NOLOCK) WHERE Modulo = 'POS' AND Clave = 'POS.TCAC'
IF @MovClave = 'POS.CTCM'
SELECT TOP 1 @Mov = Mov FROM MovTipo WITH (NOLOCK) WHERE Modulo = 'POS' AND Clave = 'POS.CTCAC'
