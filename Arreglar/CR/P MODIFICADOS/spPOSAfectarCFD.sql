...
IF EXISTS(SELECT 1 FROM POSLSerieLote pls WITH (NOLOCK) WHERE pls.ID = @ID)
INSERT SerieLoteMov (
...
UPDATE POSL WITH (ROWLOCK) Set Estatus = 'TRASPASADO' , AfectadoEnLinea = @AfectadoEnLinea WHERE ID = @ID
...
UPDATE POSL WITH (ROWLOCK) Set Estatus = 'FACTURADO' , AfectadoEnLinea = @AfectadoEnLinea,Origen = @MovCFD, OrigenID = @MovIDCFD WHERE ID = @ID
END
