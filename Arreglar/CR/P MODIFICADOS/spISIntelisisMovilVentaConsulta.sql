...
JOIN Cte Cliente WITH (NOLOCK) ON Visitas.Contacto = Cliente.Cliente AND Cliente.Cliente = @Cliente
JOIN Venta V WITH (NOLOCK) ON Cliente.Cliente = V.Cliente
...
JOIN CampanaD Visitas WITH (NOLOCK) ON Ca.ID = Visitas.ID AND Visitas.Usuario = @Usuario
JOIN Cte Cliente WITH (NOLOCK) ON Visitas.Contacto = Cliente.Cliente AND Cliente.Cliente = @Cliente
JOIN Venta MovilVentaConsulta WITH (NOLOCK) ON Cliente.Cliente = MovilVentaConsulta.Cliente AND MovilVentaConsulta.ID = @IDVenta
JOIN VentaD Detalle WITH (NOLOCK) ON MovilVentaConsulta.ID = Detalle.ID
