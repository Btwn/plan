...
IF @Ok IS NULL
BEGIN
IF @MovTipo IN ('ISL.A', 'ISL.V') UPDATE IntelisisSL WITH(ROWLOCK) SET Vencimiento   = @Vencimiento   WHERE Licencia = @Licencia
IF @MovTipo IN ('ISL.A', 'ISL.M') UPDATE IntelisisSL WITH(ROWLOCK) SET Mantenimiento = @Mantenimiento WHERE Licencia = @Licencia
IF @MovTipo IN ('ISL.A', 'ISL.L')
BEGIN
UPDATE IntelisisSLD WITH(ROWLOCK) SET Cantidad = @Cantidad, TieneMovimientos = 1 WHERE Licencia = @Licencia AND Licenciamiento = @Licenciamiento
IF @@ROWCOUNT = 0
...
END
UPDATE ISLD WITH(ROWLOCK) SET VencimientoA = @VencimientoA, MantenimientoA = @MantenimientoA, CantidadA = @CantidadA WHERE CURRENT OF crDetalle
END
END
FETCH NEXT FROM crDetalle INTO @Licencia, @Vencimiento, @Mantenimiento, @Licenciamiento, @Cantidad
END
