...
SELECT @MovTipo   = UPPER(Clave), @GenerarGasto = ISNULL(GenerarGasto, 0), @SubClave = SubClave FROM MovTipo WITH(NOLOCK) WHERE Modulo = @Modulo AND Mov = @Mov
...
FROM EmpresaConcepto WITH(NOLOCK)
