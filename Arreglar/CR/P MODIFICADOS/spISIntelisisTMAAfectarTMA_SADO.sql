...
SELECT @Sucursal = Sucursal FROM Sucursal WITH (NOLOCK) WHERE Nombre = @SucursalNombre
SELECT @RequiereMontacarga = ISNULL(RequiereMontacarga,0) FROM MapeoMovMovil WITH (NOLOCK) WITH (NOLOCK) WHERE Modulo='TMA' AND Mov = @Mov
SELECT @Modificar = ISNULL(ModificarPosicionSugeridaWMS,0), @Agente = DefAgente FROM Usuario WITH (NOLOCK) WHERE Usuario = @Usuario
SELECT @Estatus = Estatus FROM TMA WITH (NOLOCK) WHERE ID = @IdTma
SELECT @Procesado = ISNULL(Procesado,0) FROM TMAD WITH (NOLOCK) WHERE ID = @IdTma AND Renglon = @Renglon
...
IF @Ok IS NULL AND NOT EXISTS(SELECT * FROM TMA t WITH (NOLOCK) JOIN TMAD d WITH (NOLOCK) ON t.ID = d. ID WHERE d.Tarima = @Tarima AND t.Empresa = @Empresa AND t.Mov = @Mov AND t.MovID = @MovID) SELECT @Ok = 13110
...
IF @Ok IS NULL
BEGIN
IF NOT EXISTS(SELECT * FROM TMA t WITH (NOLOCK) JOIN TMAD d WITH (NOLOCK) ON (t.ID = d.ID) WHERE d.Posicion = @Posicion AND d.Tarima = @Tarima AND t.Empresa = @Empresa AND t.Mov = @Mov AND t.MovID = @MovID)
BEGIN
...
UPDATE TMAD WITH (ROWLOCK)  SET CantidadA = 1 WHERE ID = @IdTma AND Renglon = @Renglon
IF @MovTipo = 'TMA.SADO'
BEGIN
SELECT @Mov3 = TMAOrdenAcomodo FROM EmpresaCfgMovWMS WITH (NOLOCK) WHERE Empresa = @Empresa
...
IF @Ok IS NULL
BEGIN
UPDATE TMA WITH (ROWLOCK) SET Montacarga = @Montacargas, Agente = @Agente WHERE ID = @IDGenerar
...
BEGIN
SELECT @Mov3 = TMAOrdenReacomodo FROM EmpresaCfgMovWMS WITH (NOLOCK) WHERE Empresa = @Empresa
EXEC @IDGenerar = spAfectar 'TMA', @IdTma, 'GENERAR', 'Seleccion', @Mov3, @Usuario, @Estacion, @EnSilencio = 1, @Conexion = 1, @Ok = @Ok OUTPUT, @OkRef = @OkRef OUTPUT
IF @Ok BETWEEN 80030 AND 81000 SELECT @Ok = NULL, @OkRef = NULL
IF @Ok IS NULL
BEGIN
...
IF @Ok IS NULL
BEGIN
SELECT @Mov2 = LTRIM(RTRIM(ISNULL(Mov,''))), @MovID2 = LTRIM(RTRIM(ISNULL(MovID,''))) FROM TMA WITH (NOLOCK)  WHERE ID = @IDGenerar
SELECT @PosicionDestinoGenerar = LTRIM(RTRIM(ISNULL(PosicionDestino,''))) FROM TMAD WITH (NOLOCK)  WHERE ID = @IDGenerar
SELECT @DescripcionPosicionDestino = LTRIM(RTRIM(ISNULL(Descripcion,''))) FROM AlmPos WITH (NOLOCK)  WHERE Posicion = @PosicionDestinoGenerar
