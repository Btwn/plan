...
IF EXISTS(SELECT * FROM Usuario WITH (NOLOCK) WHERE Usuario = @Usuario AND BloquearCancelarFactura = 1)
SELECT @Ok = 20995
...
SELECT @FormaEnvio = FormaEnvio FROM Venta WITH (NOLOCK) WHERE ID = @ID
...
WHERE Empresa = @Empresa
AND Modulo  = @Modulo
IF /*@CxID IS NULL OR */@CfgCancelarFacturaFisicamente = 0 AND EXISTS(SELECT * FROM MovFlujo WITH (NOLOCK) WHERE Cancelado = 0 AND Empresa = @Empresa AND OModulo = 'CXC' AND OID = @CxID AND DModulo = 'CXC')
SELECT @Ok = 60060, @OkRef = RTRIM(@CxMov)+' '+LTRIM(Convert(Char, @CxMovID))
IF @Ok IS NOT NULL RETURN
IF @FormaEnvio IS NOT NULL AND EXISTS(SELECT * FROM EmpresaCfgMovEsp WITH (NOLOCK) WHERE Empresa = @Empresa AND Asunto = 'EMB' AND Modulo = @Modulo AND Mov = @Mov)
BEGIN
SELECT @CfgEmbarcar = Embarcar FROM FormaEnvio WITH (NOLOCK) WHERE FormaEnvio = @FormaEnvio
END
...
SELECT @FechaCancelacion = FechaEmision FROM Venta WITH (NOLOCK) WHERE ID = @ID
ELSE
...
UPDATE Venta WITH (ROWLOCK) SET CancelacionID = @CancelacionID, FechaCancelacion = @FechaRegistro, Estatus = 'CANCELADO' WHERE ID IN (@ID, @CancelacionID)
ELSE
UPDATE Venta WITH (ROWLOCK) SET CancelacionID = @CancelacionID WHERE ID IN (@ID, @CancelacionID)
...
IF @Ok IS NULL
BEGIN
SELECT @EstatusNuevo = Estatus FROM Venta WITH (NOLOCK) WHERE ID = @CancelacionID
...
IF @Ok IS NULL
BEGIN
SELECT @Importe = Importe FROM Cxc WITH (NOLOCK) WHERE ID = @IDGenerar
