...
IF NOT EXISTS (SELECT Articulo FROM Art WITH(NOLOCK) WHERE Articulo = @Articulo )
BEGIN
INSERT INTO Art (
...
UPDATE HArticulos  WITH(ROWLOCK) SET EstatusRegistro = 'PROCESADO'
UPDATE HArticulos  WITH(ROWLOCK) SET DestinoID = @IDPrecios WHERE ID = @ID
SELECT @MovID = MovID
FROM PC
WITH(NOLOCK) WHERE ID = @IDPrecios
