...
AS BEGIN
IF EXISTS(SELECT * FROM msdb.dbo.sysmail_profile WITH (NOLOCK) WHERE name = @NombrePerfil)
EXECUTE msdb.dbo.sysmail_delete_profile_sp @profile_name = @NombrePerfil
IF EXISTS(SELECT * FROM msdb.dbo.sysmail_account WITH (NOLOCK) WHERE name = @NombreCuenta)
