...
GO
ALTER PROCEDURE spPOSOfertaPuntosAplicar
@ID               	varchar(50),
...
JOIN POSOfertaTemp t WITH (NOLOCK) ON d.Renglon = t.Renglon AND d.Articulo = d.Articulo AND d.ID = t.IDR AND t.Estacion = @Estacion
WHERE d.ID = @ID
AND ISNULL(d.Aplicado,0) = 1
AND t.ID IN(SELECT ID FROM ListaID WITH (NOLOCK) WHERE Estacion = @Estacion)
