...
IF EXISTS(SELECT *  FROM POSLVenta WITH (NOLOCK) WHERE ID = @IDOrigen AND Articulo IN(@ArtOfertaFP))
SELECT @Ok = 30465
IF @Ok IS NULL
...
IF @@ERROR <> 0
SET @Ok = 1
IF @OK IS NULL AND EXISTS(SELECT * FROM POSLSerieLote WITH (NOLOCK) WHERE ID = @IDOrigen)
BEGIN
