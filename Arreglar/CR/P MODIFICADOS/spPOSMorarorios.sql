...
SELECT @AutorizarGasto=AutorizarGasto FROM Usuario WITH(NOLOCK) WHERE Usuario=@Usuario
...
IF @MovTipo = 'CXC.CA'
BEGIN
SELECT @Concepto = Concepto FROM Cxc WITH(NOLOCK) WHERE ID = @ID
IF @Concepto like 'Canc%'
BEGIN
INSERT INTO CXCDNC (ID, Renglon, Aplica, AplicaID)
SELECT  ID, 1, PadreMAVI, PadreIDMAVI from CXC WITH(NOLOCK) WHERE ID = @ID
END
ELSE
BEGIN
INSERT INTO CXCDNC (ID, Renglon, Aplica, AplicaID)
SELECT  ID, 1, Origen, OrigenID from NegociaMoratoriosMAVI WITH(NOLOCK) WHERE NotaCargoMorID = @ID
END
END
SELECT @MovTipo=Clave FROM MovTipo WITH(NOLOCK) WHERE Modulo = 'CXC' AND Mov = @Mov
IF @MovTipo = 'CXC.NC'
BEGIN
INSERT INTO CXCDNC (ID, Renglon, Aplica, AplicaID)
SELECT  ID, 1, Origen, OrigenID from NegociaMoratoriosMAVI WITH(NOLOCK) WHERE NotaCredBonID = @ID
END
END
/***************************** INICIA COBRO **************************/
IF @Modulo = 'CXC' AND @MovTipo = 'CXC.C' AND @EstatusNuevo = 'CONCLUIDO'
BEGIN
IF NOT EXISTS (SELECT ISNULL(TipoCobro,0) FROM TipoCobroMAVI WITH(NOLOCK) WHERE IDCobro = @ID )
BEGIN
SELECT @TipoCobro = 0
END ELSE
SELECT @TipoCobro = ISNULL(TipoCobro,0) FROM TipoCobroMAVI WITH(NOLOCK) WHERE IDCobro = @ID
IF @TipoCobro = 1
BEGIN  
SELECT @PorcMoratorioBonificar = ISNULL(PorcMoratorioBonificar,0), @MoratorioXPagar = ISNULL(MoratorioXPagar,0) FROM CobroXPoliticaHistMAVI WITH(NOLOCK) WHERE IdCobro = @ID
IF @MoratorioXPagar = 0 SELECT @RemanenteNvo = 0
ELSE
SELECT @RemanenteNvo = @MoratorioXPagar
SELECT @Padre = Origen, @PadreID = OrigenID FROM NegociaMoratoriosMAVI WITH(NOLOCK) WHERE IDCobro = @ID
...
UNION
SELECT Mov, MovID
FROM NegociaMoratoriosMAVI WITH(NOLOCK) WHERE IDCobro = @ID
UNION
SELECT Mov, MovID FROM CondonaMorxSistMAVI WITH(NOLOCK) WHERE IDCobro = @ID
GROUP BY Mov, MovID
OPEN crCobroPol
...
IF EXISTS(SELECT * FROM HistCobroMoratoriosMAVI WITH(NOLOCK) WHERE IDCobro = @ID AND Mov = @Mov AND MovID = @MovID)
...
SELECT  @IDDocPendiente = MIN(Id) FROM NegociaMoratoriosMAVI WITH(NOLOCK) WHERE IDCobro = @ID  AND ROUND(ImporteAPagar,0) < ROUND(ImporteReal,0)
IF @IDDocPendiente IS NULL
SELECT @IDDocPendiente = MIN(ID) FROM CXC  WHERE PadreMAVI = @Padre AND PadreIDMAVI = @PadreID AND Estatus = 'PENDIENTE' AND MOVID NOT IN (SELECT MovID FROM NegociaMoratoriosMAVI WITH(NOLOCK) WHERE IDCobro = @ID )
ELSE
BEGIN
SELECT @MovDocV = Mov, @MovIDDocV = MovID FROM NegociaMoratoriosMAVI WITH(NOLOCK) WHERE IDCobro = @ID
...
END
UPDATE CXC WITH(ROWLOCK) SET  InteresesMoratoriosMAVI = ROUND(@MoratorioXPagar, 2)
WHERE ID = @IDDocPendiente 
UPDATE CobroXPoliticaHistMAVI WITH(ROWLOCK) SET IdCxcMasViejo = @DocMasAntiguo WHERE IdCobro =  @ID
END
END  
IF EXISTS(SELECT * FROM CondonaMorxSistMAVI WITH(NOLOCK) WHERE IDCobro = @ID AND Estatus = 'ALTA')
BEGIN
...
FROM CondonaMorxSistMAVI WITH(NOLOCK) WHERE IDCobro = @ID AND Estatus = 'ALTA'
END
IF @TipoCobro = 0
BEGIN
IF (SELECT COUNT(*) FROM CondonaMorxSistMAVI WITH(NOLOCK) where IdCobro = @ID) > 0
BEGIN
SET @FechaAplicacion = Getdate()
SET @FechaAplicacion = CONVERT(varchar(8),@FechaAplicacion,112)
...
FROM CondonaMorxSistMAVI WITH(NOLOCK)
...
FROM NegociaMoratoriosMAVI WITH(NOLOCK) WHERE IDCobro = @ID AND Mov = @MovMor AND MovId = @MovMorID
END
...
BEGIN
UPDATE CondonaMoratoriosMAVI WITH(ROWLOCK)
...
SELECT TOP 1 @dAplica=aplica, @dAplicaID=aplicaid FROM cxcd WITH(NOLOCK) WHERE id=@ID
...
BEGIN
SELECT @PadreMAVI = Origen, @PadreIDMAVI = OrigenID FROM NegociaMoratoriosMAVI WITH(NOLOCK) WHERE NotaCargoMorId = @IDPadreMAVI
SELECT @IDPadreMAVI = ID FROM CXC WHERE Mov = @PadreMAVI AND MovID =  @PadreIDMAVI
END
...
BEGIN
SELECT @PadreMAVI = Origen, @PadreIDMAVI = OrigenID FROM NegociaMoratoriosMAVI WITH(NOLOCK) WHERE NotaCredBonID = @IDPadreMAVI
SELECT @IDPadreMAVI = ID FROM CXC WHERE Mov = @PadreMAVI AND MovID =  @PadreIDMAVI
END
END
/***************************** TERMINA COBRO **************************/
...
HistCobroMoratoriosMAVI WITH(NOLOCK)
WHERE IDCobro = @ID
...
FROM HistCobroMoratoriosMAVI WITH(NOLOCK) WHERE Mov = @MovMor AND MovID = @MovMorID AND IDCobro = @ID
...
IF @Estadistico > 0
BEGIN
UPDATE CobroXPoliticaHistMAVI WITH(ROWLOCK) SET EstatusCargoMorEst = 'CONCLUIDO' WHERE IDCobro = @IdCobroCargoMoratorioEstAnt
UPDATE Cxc WITH(ROWLOCK) SET Estatus = 'CONCLUIDO' WHERE ID= @Estadistico
END 
END
...
INSERT INTO #HIST
SELECT c.id FROM
HistCobroMoratoriosMAVI h WITH(NOLOCK), cxc c
...
BEGIN 
SELECT @FechaAnterior = NULL, @RemananteAnt = 0, @FechaOrigAnt = NULL,  @InteresesMoratoriosAnt = 0
SELECT @IDMAx = MAX(IDCobro) FROM HistCobroMoratoriosMAVI WITH(NOLOCK)
WHERE Mov = @MovMor AND MovID = @MovMorID
SELECT @FechaAnterior = FechaOriginalAnt,
@RemananteAnt  = InteresMoratoriosAnt
FROM HistCobroMoratoriosMAVI WITH(NOLOCK)
...
UPDATE CXC WITH(ROWLOCK) SET Sucursal = @SucursalV WHERE ID=@ID
UPDATE CXC WITH(ROWLOCK) SET SucursalOrigen = @SucursalV WHERE ID=@ID
END
END
END
...
FROM MovTipo WITH(NOLOCK) WHERE Mov = @Mov AND Modulo = @Modulo
IF @Modulo = 'CXC'
BEGIN
...
FROM Cxc c JOIN CxcD d WITH(NOLOCK) ON c.Mov = d.Aplica AND c.MovID = d.AplicaID
...
SELECT @MovTipo=MovTIpo.Clave FROM CXC JOIN MovTipo WITH(NOLOCK) ON MovTipo.Mov = CXC.Mov
WHERE MovTipo.Modulo = 'CXC' AND CXC.ID = @IDPadreMAVI
IF @MovTipo = 'CXC.CA'
BEGIN
SELECT @Concepto = Concepto, @MovIDPadre = MovID FROM Cxc WHERE ID = @IDPadreMAVI
...
ELSE
BEGIN
SELECT  @PadreMAVI=Origen, @PadreIDMAVI=OrigenID from NegociaMoratoriosMAVI WITH(NOLOCK) WHERE NotaCargoMorID = @ID         
END
SELECT @IDPAdreMAVI = ID FROM CXC WHERE Mov= @PadreMAVI AND MovID = @PadreIDMAVI
END
END
...
IF (SELECT ConsecutivoFiscal  from movtipo WITH(NOLOCK) where mov = @PadreMAVI AND Modulo = 'CXC') = 0
SELECT @MovTipoCFDFlex = 0, @eDoc = 0
