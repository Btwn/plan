...
SELECT @UUID = UUID, @Documento = Documento, @Proveedor = Proveedor, @ConceptoSAT = ConceptoSAT, @Ejerc = Ejercicio, @MesIni = Periodo, @MesFin = Periodo FROM CFDRetencion WITH (NOLOCK) WHERE Modulo = 'CXP' AND ModuloID = @ID
...
IF NOT EXISTS(SELECT * FROM AnexoMov WITH (NOLOCK) WHERE Rama = @Modulo AND ID = @ModuloID AND CFD = 1 AND Nombre = @NombreArchivo)
