...
FROM MovTipo WITH (NOLOCK) WHERE Mov = @Mov AND Modulo = 'VTAS'
...
IF @@ERROR<>0 SET @Ok = 1
END
IF NOT EXISTS(SELECT * FROM Venta WITH (NOLOCK) WHERE ReferenciaOrdenCompra = @IDPOS AND Estatus = 'CONCLUIDO')
BEGIN
IF @Ok IS NULL
BEGIN
...
UPDATE VentaD WITH (ROWLOCK) SET Costo = @Costo  WHERE ID = @IDVenta AND Renglon = @Renglon AND Articulo = @Articulo
FETCH NEXT FROM crDetalle INTO @Renglon, @Articulo, @SubCuenta, @Unidad
END
CLOSE crDetalle
...
END
ELSE
SELECT @IDVenta = ID FROM Venta WITH (NOLOCK) WHERE ReferenciaOrdenCompra = @IDPOS
SELECT @OkRef = REPLACE(ISNULL(@OkRef,''),'<BR>','@##@')
IF @Ok IS NULL
BEGIN
SELECT @Estatus = Estatus FROM Venta WITH (NOLOCK) WHERE ID = @IDVenta
COMMIT TRANSACTION
END
ELSE
BEGIN
