...
INSERT @Insercion (Datos)
SELECT 'IF NOT EXISTS(SELECT * FROM eDocDMapeoCampo eddmc  WITH(NOLOCK) JOIN eDocD edd  WITH(NOLOCK) ON edd.RID = eddmc.IDSeccion WHERE eddmc.Modulo = ' + dbo.fneDocComillas(edd.Modulo) + ' AND eddmc.eDoc = ' + dbo.fneDocComillas(edd.eDoc) + ' AND edd.Seccion = ' + dbo.fneDocComillas(edd.Seccion) + ' AND eddmc.CampoXML = ' + dbo.fneDocComillas(eddmc.CampoXML) + ')  INSERT eDocDMapeoCampo (Modulo, eDoc, IDSeccion, CampoXML, CampoVista, FormatoOpcional, Traducir, Opcional, BorrarSiOpcional, TablaSt, Decimales, CaracterExtendidoAASCII, ConvertirPaginaCodigo437, ConvertirComillaDobleAASCII, NumericoNuloACero) SELECT ' + dbo.fneDocComillas(edd.Modulo) + ', ' + dbo.fneDocComillas(edd.eDoc) + ', (SELECT RID FROM eDocD WITH(NOLOCK) WHERE Modulo = ' + dbo.fneDocComillas(edd.Modulo) + ' AND eDoc = ' + dbo.fneDocComillas(edd.eDoc) + ' AND Seccion = ' + dbo.fneDocComillas(edd.Seccion) + '), ' + dbo.fneDocComillas(eddmc.CampoXML) + ',  ' + dbo.fneDocComillas(eddmc.CampoVista) + ',' + dbo.fneDocComillas(eddmc.FormatoOpcional) + ',' + CONVERT(varchar,ISNULL(eddmc.Traducir,0)) + ',' + CONVERT(varchar,ISNULL(eddmc.Opcional,0)) + ',' + dbo.fneDocComillas(eddmc.BorrarSiOpcional) + ', ' + dbo.fneDocComillas(eddmc.TablaSt) + ', ' + CONVERT(varchar,ISNULL(Decimales,2)) + ',' + CONVERT(varchar,ISNULL(eddmc.CaracterExtendidoAASCII,0)) + ',' + CONVERT(varchar,ISNULL(eddmc.ConvertirPaginaCodigo437,0)) + ',' + CONVERT(varchar,ISNULL(eddmc.ConvertirComillaDobleAASCII,0)) + ',' + CONVERT(varchar,ISNULL(eddmc.NumericoNuloACero,0)) + ' '
FROM eDocDMapeoCampo eddmc  WITH(NOLOCK) JOIN eDocD edd
 WITH(NOLOCK) ON edd.eDoc = eddmc.eDoc AND edd.Modulo = eddmc.Modulo AND eddmc.IDSeccion = edd.RID
