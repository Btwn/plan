...
JOIN #Pagos p ON g.ID = p.AplicaModuloID
JOIN Version ver WITH (NOLOCK) ON 1=1
JOIN Concepto WITH (NOLOCK) ON d.Concepto = Concepto.Concepto AND Concepto.Modulo = 'GAS'
JOIN Prov pr WITH (NOLOCK) ON g.Acreedor = pr.Proveedor
LEFT OUTER JOIN Pais WITH (NOLOCK) ON pr.Pais = Pais.Pais
WHERE g.Empresa = @Empresa
