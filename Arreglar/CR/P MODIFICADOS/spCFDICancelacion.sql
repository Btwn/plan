...
IF (SELECT ISNULL(CFD, 0) FROM Empresa WITH (NOLOCK) WHERE Empresa = @Empresa) = 1
SELECT @EsCFD = 1
ELSE
IF (SELECT ISNULL(CFDFlex, 0) FROM EmpresaGral WITH (NOLOCK) WHERE Empresa = @Empresa) = 1
SELECT @EsCFDFlex = 1
SELECT @Fechatimbrado = convert(varchar(20),FechaTimbrado,127), @UUID = UUID, @Documento = Documento, @SelloCancelacionSAT = SelloCancelacionSAT FROM CFD WITH (NOLOCK) WHERE Modulo = @Modulo AND ModuloID = @ID
...
IF @EsCFD = 1
SELECT @passKey = ContrasenakeyCSD FROM EmpresaCFD WITH (NOLOCK) WHERE Empresa = @Empresa
...
BEGIN
UPDATE CFD WITH (ROWLOCK) SET Cancelado = 1, AcuseCancelado = @DatosXml, RutaAcuse = @AlmacenarRuta, SelloCancelacionSAT = @FirmaCancelacionSAT WHERE Modulo = @Modulo AND ModuloID = @ID
IF NOT EXISTS(SELECT * FROM AnexoMov WITH (NOLOCK) WHERE Rama = @Modulo AND ID = @ID AND CFD = 1 AND Direccion =@AlmacenarRuta)
