...
GO
ALTER PROCEDURE spInforTransferenciaCOMS_F
@ID                     int,
@iSolicitud       int,
@Version          float,
...
SELECT @Serie = SerieMES FROM MovTipo WITH(NOLOCK) WHERE Mov = @Mov AND Modulo = 'COMS'
...
FROM CompraD d  WITH(NOLOCK) JOIN Compra c  WITH(NOLOCK) ON c.ID = d.ID
WHERE d.ID = @IDP
