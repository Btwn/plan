...
SELECT @Complemento = Complemento FROM CFDIRetSATRetencion WITH (NOLOCK) WHERE Clave = @ConceptoSAT
IF ISNULL(RTRIM(@Complemento), '') = ''
SELECT @XMLComplemento = ''
ELSE
BEGIN
SELECT @Vista = Vista, @XMLComplemento = Plantilla FROM CFDIRetencionCompXMLPlantilla WITH (NOLOCK) WHERE Complemento = @Complemento
...
FROM CFDIRetencionCompXMLCampo WITH (NOLOCK)
WHERE Complemento = @Complemento
AND CampoXML > @CampoXMLAnt
IF @CampoXML IS NULL BREAK
SELECT @CampoXMLAnt = @CampoXML
SELECT @CampoVista = CampoVista FROM CFDIRetencionCompXMLCampo WITH (NOLOCK) WHERE Complemento = @Complemento AND CampoXML = @CampoXML
