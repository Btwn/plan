...
IF @Modulo = 'VTAS'  SELECT @RenglonID = RenglonID, @RenglonTipo = RenglonTipo, @UEN = UEN, @Almacen = Almacen, @Codigo = Codigo, @Articulo = Articulo, @SubCuenta = SubCuenta, @Cantidad = Cantidad, @Unidad = Unidad, @Factor = Factor, @CantidadInventario = CantidadInventario, @Costo = Costo, @CostoActividad = CostoActividad, @Precio = Precio,  @DescuentoTipo = DescuentoTipo,  @DescuentoLinea = DescuentoLinea,  @DescuentoImporte = DescuentoImporte, @Impuesto1 = Impuesto1,  @Impuesto2 = Impuesto2,  @Impuesto3 = Impuesto3, @Retencion1 = Retencion1, @Retencion2 = Retencion2, @Retencion3 = Retencion3,  @DescripcionExtra = DescripcionExtra, @Paquete = Paquete, @ContUso = ContUso, @Comision = Comision, @Aplica = Aplica, @AplicaID = AplicaID, @Agente = Agente, @Departamento = Departamento, @Espacio = Espacio, @Estado = Estado, @AFArticulo = AFArticulo, @AFSerie = AFSerie, @CostoUEPS = CostoUEPS, @CostoPEPS = CostoPEPS, @UltimoCosto = UltimoCosto, @PrecioLista = PrecioLista, @Posicion = Posicion, @DepartamentoDetallista = DepartamentoDetallista, @ABC = ABC FROM VentaD WITH(NOLOCK) WHERE ID = @ID AND Renglon = @Renglon AND RenglonSub = @RenglonSub ELSE 
IF @Modulo = 'CXC'   SELECT @Importe = Importe, @Aplica = Aplica, @AplicaID = AplicaID, @Fecha = Fecha, @Comision = Comision, @DescuentoRecargos = DescuentoRecargos, @InteresesOrdinarios = InteresesOrdinarios, @InteresesMoratorios = InteresesMoratorios FROM CxcD WITH(NOLOCK) WHERE ID = @ID AND Renglon = @Renglon AND RenglonSub = @RenglonSub ELSE
IF @Modulo = 'CXP'   SELECT @Importe = Importe, @Aplica = Aplica, @AplicaID = AplicaID, @Fecha = Fecha, @DescuentoRecargos = DescuentoRecargos, @InteresesOrdinarios = InteresesOrdinarios, @InteresesMoratorios = InteresesMoratorios FROM CxpD WITH(NOLOCK) WHERE ID = @ID AND Renglon = @Renglon AND RenglonSub = @RenglonSub ELSE
IF @Modulo = 'FIS'   SELECT @ObligacionFiscal = ObligacionFiscal, @Importe = Importe, @Tasa = Tasa, @CtoTipo = ContactoTipo, @Contacto = Contacto, @AFArticulo = AFArticulo, @AFSerie = AFSerie FROM FiscalD WITH(NOLOCK) WHERE ID = @ID AND Renglon = @Renglon ELSE
IF @Modulo = 'ST'    SELECT @Aplica = Aplica, @AplicaID = AplicaID FROM SoporteD WITH(NOLOCK) WHERE ID = @ID AND Renglon = @Renglon ELSE
IF @Modulo = 'COMS'  SELECT @RenglonID = RenglonID, @RenglonTipo = RenglonTipo, @Almacen = Almacen, @Codigo = Codigo, @Articulo = Articulo, @SubCuenta = SubCuenta, @Cantidad = Cantidad, @Unidad = Unidad, @Factor = Factor, @CantidadInventario = CantidadInventario, @Costo = Costo, @CostoInv = CostoInv, @DescuentoTipo = DescuentoTipo, @DescuentoLinea = DescuentoLinea, @DescuentoImporte = DescuentoImporte, @Impuesto1 = Impuesto1, @Impuesto2 = Impuesto2, @Impuesto3 = Impuesto3, @Retencion1 = Retencion1, @Retencion2 = Retencion2, @Retencion3 = Retencion3, @DescripcionExtra = DescripcionExtra, @Paquete = Paquete, @ContUso = ContUso, @Aplica = Aplica, @AplicaID = AplicaID, @DestinoTipo = DestinoTipo, @Destino = Destino, @DestinoID = DestinoID, @Cliente = Cliente, @CostoUEPS = CostoUEPS, @CostoPEPS = CostoPEPS, @UltimoCosto = UltimoCosto, @PrecioLista = PrecioLista, @Posicion = Posicion, @DepartamentoDetallista = DepartamentoDetallista, @ABC = ABC FROM CompraD WITH(NOLOCK) WHERE ID = @ID AND Renglon = @Renglon AND RenglonSub = @RenglonSub ELSE
IF @Modulo = 'AGENT' SELECT @Importe = Importe, @Aplica = Aplica, @AplicaID = AplicaID FROM AgentD WITH(NOLOCK) WHERE ID = @ID AND Renglon = @Renglon AND RenglonSub = @RenglonSub ELSE
IF @Modulo = 'GAS'   SELECT @Concepto = Concepto, @Fecha = Fecha, @Referencia = Referencia, @Cantidad = Cantidad, @Precio = Precio, @Importe = Importe, @Retencion1 = Retencion, @Retencion2 = Retencion2, @Retencion3 = Retencion3, @Impuestos = Impuestos, @ContUso = ContUso, @Espacio = Espacio, @Actividad = Actividad, @Proyecto = Proyecto, @UEN = UEN, @SerieLote = VIN, @DescripcionExtra = DescripcionExtra, @AFArticulo = AFArticulo, @AFSerie = AFSerie, @Porcentaje = PorcentajeDeducible, @Provision = Provision, @Personal = Personal, @ABC = ABC FROM GastoD WITH(NOLOCK) WHERE ID = @ID AND Renglon = @Renglon AND RenglonSub = @RenglonSub ELSE
IF @Modulo = 'DIN'   SELECT @Importe = Importe, @FormaPago = FormaPago, @Referencia = Referencia, @Aplica = Aplica, @AplicaID = AplicaID FROM DineroD WITH(NOLOCK) WHERE ID = @ID AND Renglon = @Renglon AND RenglonSub = @RenglonSub ELSE
IF @Modulo = 'AF'    SELECT @Articulo = Articulo, @SerieLote = Serie, @Importe = Importe, @Impuestos = Impuestos, @Depreciacion = Depreciacion FROM ActivoFijoD WITH(NOLOCK) WHERE ID = @ID AND Renglon = @Renglon AND RenglonSub = @RenglonSub ELSE
/*  IF @Modulo = 'VALE'
IF @Modulo = 'CR'
IF @Modulo = 'CAM'
IF @Modulo = 'CAP'
...
IF @Modulo = 'ASIS'
IF @Modulo = 'EMB'   */
IF @Modulo = 'PROD'  SELECT @RenglonID = RenglonID, @RenglonTipo = RenglonTipo, @Almacen = Almacen, @Codigo = Codigo, @Articulo = Articulo, @SubCuenta = SubCuenta, @Cantidad = Cantidad, @Unidad = Unidad, @Factor = Factor, @CantidadInventario = CantidadInventario, @Costo = Costo, @SerieLote = ProdSerieLote, @Paquete = Paquete, @DestinoTipo = DestinoTipo, @Destino = Destino, @DestinoID = DestinoID, @Aplica = Aplica, @AplicaID = AplicaID, @Cliente = Cliente, @Ruta = Ruta, @DescripcionExtra = DescripcionExtra, @Tipo = Tipo, @Comision = Comision, @Personal = Personal, @CostoUEPS = CostoUEPS, @CostoPEPS = CostoPEPS, @UltimoCosto = UltimoCosto, @PrecioLista = PrecioLista, @Posicion = Posicion, @DepartamentoDetallista = DepartamentoDetallista FROM ProdD WITH(NOLOCK) WHERE ID = @ID AND Renglon = @Renglon AND RenglonSub = @RenglonSub ELSE
IF @Modulo = 'INV'   SELECT @RenglonID = RenglonID, @RenglonTipo = RenglonTipo, @Cantidad = Cantidad, @Almacen = Almacen, @Codigo = Codigo, @Articulo = Articulo, @SubCuenta = SubCuenta, @Costo = Costo, @CostoInv = CostoInv, @ContUso = ContUso, @Espacio = Espacio, @Paquete = Paquete, @Aplica = Aplica, @AplicaID = AplicaID, @DestinoTipo = DestinoTipo, @Destino = Destino, @DestinoID = DestinoID, @Cliente = Cliente, @Unidad = Unidad, @Factor = Factor, @CantidadInventario = CantidadInventario, @SerieLote = ProdSerieLote, @Merma = Merma, @Desperdicio = Desperdicio, @Producto = Producto, @SubProducto = SubProducto, @Tipo = Tipo, @Precio = Precio, @DescripcionExtra = DescripcionExtra, @CostoUEPS = CostoUEPS, @CostoPEPS = CostoPEPS, @UltimoCosto = UltimoCosto, @PrecioLista = PrecioLista, @Posicion = Posicion, @DepartamentoDetallista = DepartamentoDetallista FROM InvD WITH(NOLOCK) WHERE ID = @ID AND Renglon = @Renglon AND RenglonSub = @RenglonSub ELSE
IF @Modulo = 'NOM'   SELECT @Personal = Personal, @Cuenta = Cuenta, @Importe = Importe, @Cantidad = Cantidad, @Concepto = Concepto, @Referencia = Referencia, @FormaPago = FormaPago, @Porcentaje = Porcentaje, @ContUso = ContUso FROM NominaD WITH(NOLOCK) WHERE ID = @ID AND Renglon = @Renglon ELSE
IF @Modulo = 'PC'    SELECT @Articulo = Articulo, @SubCuenta = SubCuenta, @Precio = Nuevo FROM PCD WITH(NOLOCK) WHERE ID = @ID AND Renglon = @Renglon
RETURN
END
