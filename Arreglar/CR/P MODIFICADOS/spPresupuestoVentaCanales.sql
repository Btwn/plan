...
JOIN MovTipo AS mt WITH(NOLOCK) ON mt.Mov = vt.Mov
JOIN Art AS a WITH(NOLOCK) ON a.Articulo = vt.Articulo
JOIN Cte c WITH(NOLOCK) ON c.Cliente=vt.Cliente
...
,v.Periodo
FROM Venta AS v WITH(NOLOCK)
JOIN VentaD vd WITH(NOLOCK) ON v.ID = vd.ID
LEFT JOIN Art AS a WITH(NOLOCK) ON a.Articulo = vd.Articulo
JOIN Cte c WITH(NOLOCK) ON c.Cliente=v.Cliente
JOIN MovTipo mt WITH(NOLOCK) ON mt.Mov=v.Mov
WHERE mt.Modulo='VTAS'
