...
SELECT @OrdenMov = InvOrdenEntarimado FROM EmpresaCfgMov WITH(NOLOCK) WHERE Empresa = @Empresa
IF @Accion = 'CANCELAR'
SELECT @OrdenID = ID FROM Inv WITH(NOLOCK) WHERE Mov = @OrdenMov AND OrigenTipo = @Modulo AND Origen = @Mov AND OrigenID = @MovID AND Estatus IN ('PENDIENTE', 'CONCLUIDO')
ELSE BEGIN
IF @Modulo = 'COMS'
SELECT @Posicion = Posicion FROM Compra WITH(NOLOCK) WHERE ID = @ID
ELSE
IF @Modulo = 'INV'
...
FROM VentaD
 WITH(NOLOCK) JOIN Venta  WITH(NOLOCK) ON Venta.ID = VentaD.ID
LEFT OUTER JOIN ArtCosto  WITH(NOLOCK) ON VentaD.Articulo = ArtCosto.Articulo AND ArtCosto.Empresa = @Empresa AND ArtCosto.Sucursal = @Sucursal
WHERE Venta.ID = @ID
END ELSE
IF @Modulo = 'COMS'
BEGIN
...
FROM CompraD
 WITH(NOLOCK) JOIN Compra  WITH(NOLOCK) ON CompraD.ID = Compra.ID
LEFT OUTER JOIN ArtCosto  WITH(NOLOCK) ON CompraD.Articulo = ArtCosto.Articulo AND ArtCosto.Empresa = @Empresa AND ArtCosto.Sucursal = @Sucursal
WHERE CompraD.ID = @ID
END ELSE
IF @Modulo = 'INV'
BEGIN
...
FROM Inv WITH(NOLOCK)
 WHERE ID = @ID
SELECT @AlmacenOrigen = Almacen FROM Inv WITH(NOLOCK) WHERE ID = @ID
SELECT @OrdenID = SCOPE_IDENTITY()
INSERT InvD (
ID,       Renglon, RenglonSub, RenglonID, Sucursal,  Almacen,  Articulo, SubCuenta, Cantidad, Unidad, Factor, CantidadInventario, Costo, CostoInv, FechaCaducidad)
...
FROM InvD
 WITH(NOLOCK) JOIN Inv  WITH(NOLOCK) ON InvD.ID = Inv.ID
LEFT OUTER JOIN ArtCosto  WITH(NOLOCK) ON InvD.Articulo = ArtCosto.Articulo AND ArtCosto.Empresa = @Empresa AND ArtCosto.Sucursal = @Sucursal
WHERE InvD.ID = @ID
AND Cantidad > 0
END ELSE
IF @Modulo = 'PROD'
...
FROM ProdD
 WITH(NOLOCK) JOIN Prod  WITH(NOLOCK) ON ProdD.ID = Prod.ID
LEFT OUTER JOIN ArtCosto  WITH(NOLOCK) ON ProdD.Articulo = ArtCosto.Articulo AND ArtCosto.Empresa = @Empresa AND ArtCosto.Sucursal = @Sucursal
WHERE Prod.ID = @ID
END
INSERT SerieLoteMov (
...
FROM SerieLoteMov m
 WITH(NOLOCK) JOIN InvD d  WITH(NOLOCK) ON d.ID = @OrdenID AND d.RenglonID = m.RenglonID AND d.Articulo = m.Articulo AND ISNULL(d.SubCuenta, '') = ISNULL(m.SubCuenta, '')
...
SELECT @OrdenMov = Mov, @OrdenMovID = MovID FROM Inv WITH(NOLOCK) WHERE ID = @OrdenID
