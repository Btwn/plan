...
FROM EmpresaCfg ITH (NOLOCK)
WHERE Empresa = @Empresa
SELECT @Agente = DefAgente, @Almacen=DefAlmacen, @Sucursal = Sucursal FROM Usuario ITH (NOLOCK) WHERE Usuario = @Usuario
SELECT @MovAjuste = 'Ajuste Posicion WMS'
SELECT @Concepto = Concepto FROM EmpresaConcepto ITH (NOLOCK) WHERE Empresa = @Empresa AND Modulo = 'INV' AND Mov = @MovAjuste
...
FROM ArtDisponibleTarima a ITH (NOLOCK)
...
AND a.Almacen = @Almacen) > 0 AND @Ok IS NULL
SELECT @Ok=13077
IF @Ok IS NULL AND NOT EXISTS(SELECT ISNULL(Posicion,'') FROM AlmPos WITH (NOLOCK) WHERE Posicion=@PosicionD AND Almacen = @Almacen) SELECT @Ok=13030, @OkRef = @PosicionD
IF @Ok IS NULL AND NOT EXISTS(SELECT ISNULL(Posicion,'') FROM AlmPos WITH (NOLOCK) WHERE Posicion=@Posicion AND Almacen = @Almacen) SELECT @Ok=13030, @OkRef = @Posicion
...
IF @Ok BETWEEN 80030 AND 81000 OR @Ok IS NULL SELECT @Ok = NULL , @OkRef = RTRIM(Mov)+' '+RTRIM(MovID) FROM Inv WITH (NOLOCK) WHERE ID = @ID
END
IF @Ok IS NULL
BEGIN
COMMIT TRANSACTION
