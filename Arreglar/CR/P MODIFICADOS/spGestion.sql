...
BEGIN
IF EXISTS(SELECT * FROM Usuario WITH(NOLOCK) WHERE Usuario = @Usuario AND Autorizar = 1 AND AutorizarGestion = 1)
...
SELECT @IDOrigen = MIN(ID) FROM Gestion WITH(NOLOCK) WHERE Empresa = @Empresa AND Mov = @Origen AND MovID = @OrigenID AND Estatus IN ('PENDIENTE', 'ALTAPRIORIDAD', 'PRIORIDADBAJA', 'CONCLUIDO')
...
UPDATE Gestion
 WITH(ROWLOCK) SET RamaID = @RamaID,
IDOrigen = @IDOrigen
WHERE ID = @ID
END
IF @IDOrigen IS NOT NULL
...
FROM MovTipo
WITH(NOLOCK) WHERE Modulo = @Modulo AND Mov = @Origen
...
IF EXISTS(SELECT * FROM GestionPara WITH(NOLOCK) WHERE ID = @ID AND UPPER(Participacion) = 'REQUERIDO')
