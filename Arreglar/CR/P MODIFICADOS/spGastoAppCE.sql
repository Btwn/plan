...
FROM MacAddressAPP
WITH(NOLOCK) WHERE MacAddress= @MacAddress
...
SELECT @Mov = GastoComprobante FROM EmpresaCfgMov WITH(NOLOCK) WHERE EmpresaCfgMov.Empresa = @Empresa
SELECT @Moneda = ContMoneda, @TipoCambio = Mon.TipoCambio  FROM EmpresaCfg  WITH(NOLOCK) JOIN Mon  WITH(NOLOCK) ON EmpresaCfg.ContMoneda = Mon.Moneda WHERE EmpresaCfg.Empresa = @Empresa
SELECT @FechaTrabajo = dbo.fnFechaSinHora(GETDATE()), @Renglon = 0.00
SELECT @ContRelacionarComp = ContRelacionarComp FROM MovTipo WITH(NOLOCK) WHERE Mov = @Mov AND Modulo = 'GAS'
...
MovAPP.Documento
FROM MovAPP
WITH(NOLOCK) WHERE MovAPP.MacAddress = @MacAddress
ORDER BY MovAPP.ID
OPEN crGastoD
...
SELECT @AcreedorRef = Proveedor FROM Prov WITH(NOLOCK) WHERE RTRIM(LTRIM(RFC)) = @RFC
