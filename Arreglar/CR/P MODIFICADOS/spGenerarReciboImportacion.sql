...
GO
ALTER PROCEDURE spGenerarReciboImportacion
@Modulo                 varchar(5),
@ID                     int,
@Accion                 varchar(20),
@Empresa                varchar(5),
...
@GastoGenerado          bit,
@RenglonID              int
IF NOT EXISTS(SELECT ID FROM CompraD WITH(NOLOCK) WHERE ID = @ID AND MonedaD IS NOT NULL AND TipoCambioD IS NOT NULL)
BEGIN
RETURN
END
SELECT @Fecha = GETDATE(), @GastoGenerado = 0
EXEC spExtraerFecha @Fecha OUTPUT
SELECT @GenerarMov = 'Entrada Importacion'
...
FROM CompraD WITH(NOLOCK) WHERE ID = @ID
OPEN crMoneda
...
FROM CompraD
WITH(NOLOCK) WHERE ID = @ID AND MonedaD = @Moneda AND TipoCambioD = @TipoCambio
...
AND ID = @ID
AND RenglonID IN (SELECT RenglonID FROM CompraD WITH(NOLOCK) WHERE ID = @GenerarID)
IF @GastoGenerado = 0
BEGIN
...
END
SELECT @RenglonID = MAX(RenglonID) FROM CompraD WITH(NOLOCK) WHERE ID = @GenerarID
EXEC spAfectar @Modulo, @GenerarID, @Accion, 'Todo', NULL, @Usuario, NULL, 1, @Ok = @Ok OUTPUT, @OkRef = @OkRef OUTPUT, @Conexion = 1
IF @Ok IS NULL
BEGIN
SELECT @DMovID = MovID FROM Compra WITH(NOLOCK) WHERE ID = @GenerarID
...
UPDATE Compra  WITH(ROWLOCK) SET Estatus = 'CONCLUIDO', FechaConclusion = @Fecha WHERE ID = @ID
...
IF @Ok IS NULL
UPDATE Compra  WITH(ROWLOCK) SET Estatus = 'CANCELADO', FechaCancelacion = @Fecha WHERE ID = @ID
END
RETURN
END
