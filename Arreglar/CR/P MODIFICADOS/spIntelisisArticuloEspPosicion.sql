...
SET @Mov = @Movimiento
IF @Ok IS NULL
IF NOT EXISTS(SELECT ISNULL(Posicion,'') FROM AlmPos with(nolock) WHERE Posicion = @Posicion) SET @Ok = 13030
IF @Ok IS NULL
IF NOT EXISTS(SELECT ISNULL(ArticuloEsp,'') FROM AlmPos with(nolock) WHERE Posicion = @Posicion) SET @Ok = 13075
IF @Ok IS NULL
BEGIN
SELECT TOP 1 @Sucursal = Sucursal FROM Sucursal with(nolock) WHERE Nombre = @Sucursal2
SELECT @IdTma = ID
FROM TMA
...
FROM TMAD with(nolock) WHERE ID = @IdTma
AND Almacen = @Almacen
...
FROM AlmPos ap
 WITH(NOLOCK) JOIN Art ar  WITH(NOLOCK) ON ar.Articulo = ap.ArticuloEsp
