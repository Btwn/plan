...
Art.CostoEstandar,
Art.CostoReposicion,
CostoPromedio = (SELECT TOP 1 a.CostoPromedio FROM ArtCostoHist a WITH(NOLOCK) WHERE a.Empresa = @Empresa AND a.Sucursal = ac.Sucursal AND a.Articulo = Art.Articulo GROUP BY a.CostoPromedio, a.Fecha, a.Empresa, a.Sucursal, a.Articulo HAVING a.Fecha = (SELECT MAX(b.Fecha) FROM ArtCostoHist b WITH(NOLOCK) WHERE b.Empresa = a.Empresa AND b.Sucursal = a.Sucursal AND b.Articulo = a.Articulo AND b.Fecha < @FechaA)),
UltimoCosto = (SELECT TOP 1 a.UltimoCostoActual FROM ArtCostoHist a WITH(NOLOCK) WHERE a.Empresa = @Empresa AND a.Sucursal = ac.Sucursal AND a.Articulo = Art.Articulo GROUP BY a.UltimoCostoActual, a.Fecha, a.Empresa, a.Sucursal, a.Articulo HAVING a.Fecha = (SELECT MAX(b.Fecha) FROM ArtCostoHist b WITH(NOLOCK) WHERE b.Empresa = a.Empresa AND b.Sucursal = a.Sucursal AND b.Articulo = a.Articulo AND b.Fecha < @FechaA)),
e.Moneda,
e.Almacen,
Alm.Nombre,
SUM(e.Existencia)
FROM Art WITH(NOLOCK)
JOIN ArtCostoSucursal ac WITH(NOLOCK) ON Art.Articulo = ac.Articulo
JOIN ArtExistenciaNeta e WITH(NOLOCK) ON Art.Articulo = e.Articulo
