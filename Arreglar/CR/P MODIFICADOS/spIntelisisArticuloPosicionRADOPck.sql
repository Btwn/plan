...
GO
ALTER PROCEDURE spIntelisisArticuloPosicionRADOPck
@ID				int,
@iSolicitud		int,
@Version		float,
...
@Descripcion1=a.Descripcion1+' '+'CB:' + ISNULL(c.Codigo,'')+' -CAPACIDAD:'+CONVERT(varchar, ISNULL(a.CantidadPresentacion, 0))+ ' '+ ISNULL(a.Presentacion, '') 
FROM Art a /*WITH(NOLOCK)*/
INNER JOIN ArtUnidad b  WITH(NOLOCK) ON a.Articulo =b.Articulo AND a.UnidadCompra = b.Unidad
JOIN CB c  WITH(NOLOCK) ON c.Cuenta = a.Articulo
