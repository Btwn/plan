...
FROM EstadoCuenta
LEFT OUTER JOIN Auxiliar  WITH(NOLOCK) ON EstadoCuenta.AuxiliarID=Auxiliar.ID
JOIN Prov  WITH(NOLOCK) ON EstadoCuenta.Cuenta=Prov.Proveedor
JOIN Empresa  WITH(NOLOCK) ON Empresa.Empresa = EstadoCuenta.Empresa
JOIN EmpresaCfg  WITH(NOLOCK) ON EmpresaCfg.Empresa = Empresa.Empresa
JOIN Mon  WITH(NOLOCK) ON Mon.Moneda = EstadoCuenta.Moneda
