...
IF EXISTS (SELECT * FROM Fiscal WITH (NOLOCK) WHERE Mov IN (@DeclaracionMov, @ComplementariaMov) AND Estatus = 'CONFIRMAR')
SELECT @OK = 60440
IF EXISTS (SELECT * FROM Fiscal WITH (NOLOCK) WHERE Mov = @DeclaracionMov AND Estatus = 'CONCLUIDO' AND FechaEmision BETWEEN @FechaD AND @FechaA )
SELECT @Ok = 60450
IF @OK IS NULL
BEGIN
