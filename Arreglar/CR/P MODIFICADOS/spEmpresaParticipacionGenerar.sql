...
@TipoCambio		float,
@MovCont		char(20)
SELECT @MovCont = ContDiario FROM EmpresaCfgMov WITH(NOLOCK) WHERE Empresa = @Empresa
SELECT @ContMoneda = mon.Moneda, @TipoCambio = mon.TipoCambio
FROM EmpresaCfg cfg, Mon
WITH(NOLOCK) WHERE cfg.Empresa = @Empresa AND mon.Moneda = cfg.ContMoneda
...
WITH(NOLOCK) WHERE p.Empresa = @Empresa AND p.Moneda = m.Moneda
...
SELECT 'Se Gener� 1 P�liza (por Confirmar)'
RETURN
END
