...
IF EXISTS(SELECT l.ID FROM ListaModuloID l WITH (NOLOCK) JOIN Nomina n WITH (NOLOCK) ON l.ID = n.ID  JOIN CFDINominaMov m WITH (NOLOCK) ON n.Mov = m.Mov WHERE l.Estacion = @Estacion)
BEGIN
IF NOT EXISTS( SELECT ID FROM CFDINominaDatosMov WITH (NOLOCK) WHERE Estacion = @Estacion)
