...
SELECT @Licenciamiento = Licenciamiento FROM Usuario WITH(NOLOCK) WHERE Usuario = @Usuario
...
INSERT INTO #ClavePermitida (Clave) SELECT Clave FROM MovTipo WITH(NOLOCK) WHERE Modulo = @ModuloClavePermitida
END
ELSE
