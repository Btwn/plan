...
SELECT @MovDev = POSDefMovDev FROM POSCfg WITH (NOLOCK) WHERE Empresa = @Empresa
BEGIN TRAN
IF @Modulo = 'VTAS'
BEGIN
...
IF EXISTS(SELECT * FROM POSSerieTarjetaMovM WITH (NOLOCK) WHERE Modulo = @Modulo AND ID = @IDPOS) 
BEGIN
SELECT @Cuenta = NULLIF(Serie,''),
@Puntos = ABS(ISNULL(Importe,0.0) )
FROM POSSerieTarjetaMovM WITH (NOLOCK)
WHERE Modulo = @Modulo
AND ID = @IDPOS
...
BEGIN
IF EXISTS(SELECT *
FROM TarjetaMonedero T WITH (NOLOCK)
...
IF @OK IS NULL
BEGIN
UPDATE POSSerieTarjetaMovM WITH (ROWLOCK)
