...
WITH(NOLOCK) WHERE ID = @ID
			...
			BEGIN
				SELECT @IDVenta = ID
				FROM Venta
WITH(NOLOCK) WHERE Mov = @OrigenCxc
...
WITH(NOLOCK) WHERE ID = @ID
	...
	BEGIN
		...
		BEGIN
			...
			FROM Prov
WITH(NOLOCK) WHERE Nombre = @BeneficiarioEspecifico
...
WITH(NOLOCK) WHERE Empresa = @Empresa
	...
	FROM CtaDinero
WITH(NOLOCK) WHERE CtaDinero = @CtaDinero
...
WITH(NOLOCK) WHERE Empresa = @Empresa
	...
	IF @IgnorarFormaPago = 0
		...
		FROM FormaPago
WITH(NOLOCK) WHERE FormaPago = @FormaPago
	...
	BEGIN
		...
		BEGIN
			...
			IF @Cuenta IS NOT NULL
				...
				FROM Cte
WITH(NOLOCK) WHERE Cliente = @Cuenta
...
WITH(NOLOCK) WHERE Cliente = @Contacto
			...
			IF @Modulo IN ('CXP', 'GAS', 'AF')
				...
				FROM Prov
WITH(NOLOCK) WHERE Proveedor = @Cuenta

			IF @Modulo = 'AGENT'
				SELECT @Nombre = NULLIF(RTRIM(Nombre), '')
					  ,@BeneficiarioNombre = NULLIF(RTRIM(BeneficiarioNombre), '')
				FROM Agente
WITH(NOLOCK) WHERE Agente = @Cuenta
	...
	BEGIN
		...
		IF EXISTS (SELECT * FROM MovTipo WITH(NOLOCK) WHERE Modulo = 'DIN' AND Mov = @DineroMov AND GenerarGasto = 1)
			SELECT @Ok = 70140
				  ,@OkRef = @DineroMov
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @BeneficiarioNombre = NULLIF(RTRIM(Nombre), '')
			FROM Beneficiario
WITH(NOLOCK) WHERE Beneficiario = @Beneficiario
	...
	IF EXISTS (SELECT * FROM EmpresaCfgMovGenera WITH(NOLOCK) WHERE Empresa = @Empresa AND Modulo = @Modulo AND Mov = @Mov AND GeneraModulo = 'DIN')
	BEGIN
...
WITH(NOLOCK) WHERE Cancelado = 0
	...
	BEGIN
		...
		BEGIN
			...
			FROM DineroD d WITH(NOLOCK)
			LEFT OUTER JOIN FormaPago fp WITH(NOLOCK)
