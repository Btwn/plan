...
IF (SELECT ISNULL(VentaMonederoA,0) FROM EmpresaCfg2  WITH (NOLOCK)  WHERE Empresa = @Empresa) = 1 
RETURN
IF @Accion = 'CANCELAR'
SELECT @Ok = 60050
IF @Modulo = 'POS'
...
SELECT @Ok = 14055
IF @Modulo = 'VTAS'
IF NOT EXISTS (SELECT * FROM Venta WITH (NOLOCK)  WHERE ID = @ID)
SELECT @Ok = 14055
...
FROM LDIServicioEstacion WITH (NOLOCK) 
...
SELECT @MonedaMonedero = Moneda, @Cliente = Cliente FROM POSValeSerie  WITH (NOLOCK) WHERE Serie = @NoTarjeta
SELECT @Moneda = Moneda, @TipoCambio = TipoCambio FROM POSLTipoCambioRef WITH (NOLOCK) WHERE EsPrincipal = 1 AND Sucursal = @Sucursal
SELECT @MonederoTipoCambio = TipoCambio FROM POSLTipoCambioRef WITH (NOLOCK) WHERE Sucursal = @Sucursal AND Moneda = @MonedaMonedero
...
UPDATE LDIMovLog WITH (ROWLOCK) SET   Comprobante = REPLACE(REPLACE(REPLACE(Comprobante,'^',':'),'@dd',':'),CHAR(10),'<BR>') ,Fecha = dbo.fnLDIFecha(Fecha)
WHERE  ID = @IDLDI
