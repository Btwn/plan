...
FROM CB c
 WITH(NOLOCK) JOIN ArtUnidad a  WITH(NOLOCK) ON (c.Cuenta = a.Articulo AND c.Unidad = a.Unidad)
...
IF @Ok IS NOT NULL
BEGIN
SELECT @OkRef = LTRIM(RTRIM(ISNULL(Descripcion,''))) FROM MensajeLista WITH(NOLOCK) WHERE Mensaje = @Ok
