...
SELECT @Sucursal = Sucursal FROM Nomina WITH (NOLOCK) WHERE ID = @ID
SELECT @ArchivoXML = ArchivoXML FROM CFDINominaRecibo WITH (NOLOCK) WHERE ID = @ID AND Personal = @Personal
SELECT @DatosXML = Documento FROM CFDNomina WITH (NOLOCK) WHERE Modulo = 'NOM' AND ModuloID = @ID AND Personal = @Personal
SELECT @Ruta = dbo.fnCFDINominaObtenerRuta(@ArchivoXML)+ '\'
SELECT @NombreArchivo = REPLACE(@ArchivoXML, @Ruta, '')
IF NOT EXISTS(SELECT * FROM AnexoMov WITH (NOLOCK) WHERE Rama = 'NOM' AND ID = @ID AND CFD = 1 AND Direccion = @ArchivoXML)
