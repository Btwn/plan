...
JOIN CompraD cd WITH(NOLOCK) ON cd.ID=c.ID
RIGHT JOIN Art a WITH(NOLOCK) ON a.Articulo=cd.Articulo
...
FROM Compra c WITH(NOLOCK)
JOIN CompraD cd WITH(NOLOCK) ON cd.ID=C.ID
JOIN Art a WITH(NOLOCK) ON a.Articulo=cd.Articulo
...
,c.Periodo
FROM Compra c WITH(NOLOCK)
JOIN CompraD AS cd WITH(NOLOCK) ON cd.ID=c.ID
JOIN Art a WITH(NOLOCK) ON a.Articulo=cd.Articulo
JOIN MovTipo mt WITH(NOLOCK) ON mt.Mov=c.Mov
WHERE mt.Modulo='COMS'
