...
FROM CFD
JOIN EmpresaCFD cfg WITH (NOLOCK) ON cfg.Empresa=@Empresa
WHERE cfd.Modulo = @Modulo AND cfd.ModuloID = @ModuloID
UPDATE CFD WITH (ROWLOCK) SET Documento = REPLACE(REPLACE(CONVERT(varchar(MAX), Documento),'&#39;',''''),'?<?xml','<?xml') WHERE Modulo = @Modulo AND ModuloID = @ModuloID
END
