...
BEGIN
SELECT @CfgMAF = MAF FROM EmpresaGral WITH (NOLOCK) WHERE Empresa = @Empresa
IF @CfgMAF = 1
BEGIN
SELECT @SubClaveAplica = mt.SubClave, @AFArticuloAplica = AFArticulo, @AFSerieAplica = AFSerie FROM Venta v  WITH (NOLOCK)JOIN MovTipo mt WITH (NOLOCK) ON v.Mov = mt.Mov AND mt.Modulo = 'VTAS' WHERE ID = @IDAplica
