...
JOIN MovTipo MT  WITH (NOLOCK) ON V.Mov = MT.Mov
AND MT. Modulo = 'VTAS'
WHERE V.ID = @ID
...
IF EXISTS(SELECT * FROM SerieTarjetaMovM  WITH (NOLOCK) WHERE Modulo = @Modulo AND ID = @ID) 
BEGIN
SELECT @Cuenta = NULLIF(Serie,''),
@Puntos = ISNULL(Importe,0.0)
FROM SerieTarjetaMovM WITH (NOLOCK) 
WHERE Modulo = @Modulo
AND ID = @ID
