...
GO
ALTER PROCEDURE spGenerarAsisteCorteMov
@Sucursal		int,
@Empresa		char(5),
@Usuario		char(10),
...
FROM PersonalAsisteDifDia WITH(NOLOCK)
WHERE Empresa = @Empresa AND Fecha >= @FechaInicial AND Fecha < DATEADD(day, 1, @FechaFinal)
...
BEGIN
SELECT @Jornada = Jornada FROM Personal WITH(NOLOCK) WHERE Personal = @Personal
SELECT @Domingo   = Domingo   FROM Jornada WITH(NOLOCK) WHERE Jornada = @Jornada
SELECT @Sabado    = Sabado    FROM Jornada WITH(NOLOCK) WHERE Jornada = @Jornada
SELECT @Lunes     = Lunes     FROM Jornada WITH(NOLOCK) WHERE Jornada = @Jornada
SELECT @Martes    = Martes    FROM Jornada WITH(NOLOCK) WHERE Jornada = @Jornada
SELECT @Miercoles = Miercoles FROM Jornada WITH(NOLOCK) WHERE Jornada = @Jornada
SELECT @Jueves    = Jueves    FROM Jornada WITH(NOLOCK) WHERE Jornada = @Jornada
SELECT @Viernes   = Viernes   FROM Jornada WITH(NOLOCK) WHERE Jornada = @Jornada
...
IF @@DATEFIRST = 1
BEGIN
IF ((@Fecha IN (SELECT Fecha FROM DiaFestivo)) OR (@Fecha IN (SELECT Fecha FROM JornadaDiaFestivo WITH(NOLOCK) WHERE Jornada = @Jornada))) AND @AsistDiaFestivolaborado = 1
BEGIN
INSERT PersonalAsisteDif (Empresa,  Personal,  Fecha,  Cantidad, Tipo,             Concepto)
...
FROM PersonalAsisteDifMin WITH(NOLOCK)
...
SELECT @CfgMovCorte = ISNULL(NULLIF(RTRIM(AsisteCorte), ''), 'Corte') FROM EmpresaCfgMov WITH(NOLOCK) WHERE Empresa = @Empresa
SELECT @ID = NULL
SELECT @ID = MAX(ID) FROM Asiste WITH(NOLOCK) WHERE Empresa = @Empresa AND Usuario = @Usuario AND Estatus IN ('SINAFECTAR', 'BORRADOR', 'CONFIRMAR')
IF @ID IS NULL
BEGIN
...
DELETE AsisteD WHERE ID = @ID
UPDATE Asiste
 WITH(ROWLOCK) SET FechaEmision = @FechaEmision,
Estatus = 'BORRADOR',
FechaD  = @FechaInicial,
FechaA  = @FechaFinal
WHERE ID = @ID
...
WITH(NOLOCK) WHERE d.Empresa = @Empresa AND d.Fecha >= @FechaInicial AND d.Fecha < DATEADD(day, 1, @FechaFinal)
