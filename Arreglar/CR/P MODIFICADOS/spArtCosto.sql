...
AND Serie IN (SELECT SerieLote FROM SerieLoteMov WITH(NOLOCK) WHERE /*Sucursal = @Sucursal AND */Empresa = @Empresa AND Modulo = @Modulo AND ID = @ID AND RenglonID = @RenglonID AND Articulo = @Articulo)
RETURN
END
...
IF @MovMoneda <> @ArtMoneda AND @CfgFormaCosteo = 'ARTICULO' AND EXISTS (SELECT * FROM EmpresaCfgModulo WITH(NOLOCK) where Empresa = @Empresa AND Modulo = @Modulo)
BEGIN
IF EXISTS (SELECT * FROM EmpresaCfgModulo WITH(NOLOCK) where Empresa = @Empresa AND Modulo = @Modulo AND TipoCambio = 'General')
SELECT @ArtCosto          = @Costo          / @ArtFactor,	
@ArtCostoSinGastos = @CostoSinGastos / @ArtFactor	
ELSE
IF EXISTS (SELECT * FROM EmpresaCfgModulo WITH(NOLOCK) where Empresa = @Empresa AND Modulo = @Modulo AND TipoCambio = 'Compra')
...
ELSE
IF EXISTS (SELECT * FROM EmpresaCfgModulo WITH(NOLOCK) where Empresa = @Empresa AND Modulo = @Modulo AND TipoCambio = 'Venta')
...
UPDATE VINCostoExtra WITH(ROWLOCK) SET SalidaID = NULL WHERE Empresa = @Empresa AND VIN IN (SELECT SerieLote FROM SerieLoteMov WITH(NOLOCK) WHERE Empresa = @Empresa AND Modulo = @Modulo AND ID = @ID AND RenglonID = @RenglonID AND Articulo = @Articulo AND SubCuenta = ISNULL(@SubCuenta, ''))
END 
ELSE
IF @EsSalida = 1
BEGIN
...
FROM SerieLoteMov m WITH(NOLOCK), SerieLote s WITH(NOLOCK)
...
*/
UPDATE VINCostoExtra WITH(ROWLOCK) SET SalidaID = @ID WHERE Empresa = @Empresa AND VIN IN (SELECT SerieLote FROM SerieLoteMov WITH(NOLOCK) WHERE Empresa = @Empresa AND Modulo = @Modulo AND ID = @ID AND RenglonID = @RenglonID AND Articulo = @Articulo AND SubCuenta = ISNULL(@SubCuenta, ''))
END
...
IF @CostearNivelSubCuenta = 1
BEGIN
UPDATE ArtSubCosto WITH(ROWLOCK)
