...
@Dominio = sl.Dominio
FROM IntelisisSL sl
 WITH(NOLOCK) JOIN Cte c  WITH(NOLOCK) ON c.Cliente = sl.Cliente
...
IF @Ok IS NULL
BEGIN
  UPDATE IntelisisSL WITH(ROWLOCK) SET TieneMovimientos = 1 WHERE Licencia = @Licencia AND TieneMovimientos = 0
UPDATE IntelisisSLD  WITH(ROWLOCK) SET TieneMovimientos = 1 WHERE Licencia = @Licencia AND TieneMovimientos = 0
SELECT @Usuarios = (SELECT Licenciamiento, Cantidad FROM IntelisisSLD WITH(NOLOCK) WHERE Licencia = @Licencia AND Estatus = 'ALTA' FOR XML RAW ('Acceso'), ROOT ('Usuarios'))
SELECT @CantidadTotal = SUM(Cantidad) FROM IntelisisSLD WITH(NOLOCK) WHERE Licencia = @Licencia AND Estatus = 'ALTA'
END
IF @Ok IS NOT NULL
BEGIN
