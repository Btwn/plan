...
JOIN SerieLoteMov slm WITH(NOLOCK) ON slm.Empresa = @Empresa AND slm.Modulo = 'INV' AND slm.ID = d.ID AND slm.Articulo = d.Articulo AND ISNULL(slm.SubCuenta, '') = ISNULL(d.Subcuenta, '')
WHERE d.ID = @ID
OPEN crSerieLoteMov
