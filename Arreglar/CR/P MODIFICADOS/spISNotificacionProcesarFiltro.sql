...
IF NOT EXISTS(SELECT 1 FROM NotificacionFiltroNormalizada WITH (NOLOCK) WHERE RTRIM(Notificacion) = RTRIM(@Notificacion))
SELECT @GenerarNotificacion = 1
ELSE
BEGIN
IF EXISTS(SELECT 1
FROM NotificacionFiltroNormalizada WITH (NOLOCK)
