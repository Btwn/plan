...
FROM MovTipo WITH(NOLOCK)
WHERE Modulo = @Modulo AND Mov = @Mov
IF ISNULL(@CFDReporteIntelisis,'') <> '' AND (@Estatus = @EstatusCfg OR (@EstatusCfg = '(VARIOS)' AND @Estatus IN (SELECT NULLIF(Estatus,'') FROM MovTipoCFDFlexEstatus WITH(NOLOCK) WHERE Modulo = @Modulo AND Mov = @Mov)))
BEGIN
