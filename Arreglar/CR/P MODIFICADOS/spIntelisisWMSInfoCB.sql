...
IF NOT EXISTS(SELECT * FROM CB WITH(NOLOCK) WHERE Codigo = @CB)SET @Ok = 72040
IF @Ok IS NULL
BEGIN
SELECT @Articulo = Cuenta, @SubCuenta=ISNULL(SubCuenta,'') FROM CB WITH(NOLOCK) WHERE Codigo = @CB
IF NOT EXISTS(SELECT * FROM Compra c  WITH(NOLOCK) JOIN CompraD d  WITH(NOLOCK) ON c.ID = d.ID WHERE c.Empresa = @Empresa AND c.Mov = @Mov AND c.MovID = @MovID AND d.Articulo = @Articulo) SELECT @Ok =10530
SELECT @Fecha = GETDATE()
SELECT @Caducidad =ISNULL(CaducidadMinima,0), @TieneCaducidad = ISNULL(TieneCaducidad,0)  FROM Art WITH(NOLOCK) WHERE Articulo = @Articulo
...
FROM CB c  WITH(NOLOCK) JOIN Art a  WITH(NOLOCK) ON c.Cuenta = a.Articulo
JOIN ArtUnidad au  WITH(NOLOCK) ON AU.Articulo=a.Articulo AND au.Unidad=a.UnidadCompra
WHERE  c.Codigo = @CB
END
