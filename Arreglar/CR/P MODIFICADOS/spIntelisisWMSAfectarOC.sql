...
IF @@ERROR <> 0 SET @Ok = 1
SELECT @ID2 = ID FROM Compra WITH(NOLOCK) WHERE  Mov=@Mov AND MovID=@MovID AND Empresa=@Empresa
/*RJP 171110 Valida que existan partidas recibidas para que la entrada de compra no se genere vacia*/
IF @OK is NULL
BEGIN
SELECT @Partidas = count(ID) FROM CompraD WITH(NOLOCK) WHERE ID = @ID2  AND (CantidadA > 0 or CantidadA is not null)
IF @Partidas = 0
SET @OK = 60010 
END
/*RJP 171110 Valida que existan partidas recibidas para que la entrada de compra no se genere vacia*/
...
FROM FechaCaducidadMovil f
 WITH(NOLOCK) JOIN CompraD d  WITH(NOLOCK) ON f.ID = d.ID AND f.Articulo = d.Articulo
WHERE d.ID = @ID2
AND f.Procesado = 0
AND f.Cantidad > 0
OPEN crRenglon
...
BEGIN
UPDATE CompraD  WITH(ROWLOCK) SET FechaCaducidad = @FechaCaducidad2 WHERE ID = @IDGenerar AND Articulo = @ArticuloCaducidad AND Cantidad = @Cantidad2
FETCH NEXT FROM crRenglon  INTO @Renglon, @RenglonSub, @Cantidad2, @FechaCaducidad2, @ArticuloCaducidad
END
CLOSE crRenglon
DEALLOCATE crRenglon
IF @Ok IS NULL
UPDATE Compra  WITH(ROWLOCK) SET Usuario = @Usuario WHERE ID = @IDGenerar
...
EXEC(@StrSQL)
SELECT @Moneda = Moneda, @TipoCambio = TipoCambio, @FechaEmision = FechaEmision FROM Compra WITH(NOLOCK) WHERE ID = @ID
/*
...
END
IF @Ok IS NULL  
IF EXISTS(SELECT * FROM MovTipo WITH(NOLOCK) WHERE Modulo = 'COMS' AND Mov = @Mov2 AND Clave = 'COMS.EI')
BEGIN
UPDATE CompraD
 WITH(ROWLOCK) SET ImportacionProveedor = b.Proveedor
FROM CompraD a
INNER JOIN Compra b
 WITH(NOLOCK) ON (a.Aplica = b.Mov AND a.AplicaID = b.MovID)
WHERE a.ID = @IDGenerar
END
ELSE
BEGIN
UPDATE CompraD  WITH(ROWLOCK) SET ImportacionProveedor = NULL WHERE ID = @IDGenerar
END
SELECT @Sucursal = Sucursal FROM Compra WITH(NOLOCK) WHERE ID = @ID2
...
IF @OK IS NULL
BEGIN
UPDATE Compra  WITH(ROWLOCK) SET PosicionWMS = @AndenRecibo WHERE ID = @IDGenerar
...
SET @Ok = NULL
SET @StrSQL='UPDATE Compra
 WITH(ROWLOCK) SET TipoDocSF=NULL,
...
UPDATE Compra  WITH(ROWLOCK) SET Referencia = @Referencia
Where ID = @IDGenerar   
END
IF @@ERROR <> 0 SET @Ok = 1
...
IF @Ok IS NULL
BEGIN
SELECT @OkRef = RTRIM(Mov)+' '+RTRIM(MovID) FROM Compra WITH(NOLOCK) WHERE ID = @IDGenerar 
END
ELSE
