...
WITH(NOLOCK) WHERE Cliente = @Cliente
	...
	FROM Personal
WITH(NOLOCK) WHERE Personal = @PersonalCobrador
...
WITH(NOLOCK) WHERE Moneda = @Moneda
	...
	FROM Descuento
WITH(NOLOCK) WHERE Descuento = @Descuento
...
WITH(NOLOCK) WHERE ID = @ID
				...
				BEGIN
					...
					FROM CXC
WITH(NOLOCK) WHERE Mov = @OrigenDoc
...
WITH(NOLOCK) WHERE Empresa = @Empresa
	...
	BEGIN
		...
		BEGIN
			...
			IF LEN(@Recibo) > 35
				SELECT @Ok = 10010
					  ,@OkRef = 'CFD - Recibo Excede Tama_�o Maximo 35'
			...
			IF LEN(@DepartamentoClave) > 35
				SELECT @Ok = 10010
					  ,@OkRef = 'CFD - Clave Departamento Cliente Excede Tama_�o Maximo 35'
			...
			IF @MonedaClave NOT IN ('', NULL, 'MXN', 'XEU', 'USD')
				SELECT @Ok = 10010
					  ,@OkRef = 'CFD - Moneda Clave Debe ser MXN, XEU_ USD)'
...
WITH(NOLOCK) WHERE ID = @ID
			...
			FROM Unidad
WITH(NOLOCK) WHERE Unidad = @Unidad
...
WITH(NOLOCK) WHERE ID = @ID
			...
			FROM TipoEmpaque
WITH(NOLOCK) WHERE TipoEmpaque = @ArtTipoEmpaque
			...
			IF @PrimerSerieLote IS NOT NULL
				...
				FROM SerieLote s
				 WITH(NOLOCK) JOIN SerieLoteProp p
			...
			BEGIN
				...
				BEGIN
					...
					IF LEN(@EAN13) > 14
						SELECT @Ok = 10010
							  ,@OkRef = 'CFD - Excede Tama_�o Caracteres 14'
