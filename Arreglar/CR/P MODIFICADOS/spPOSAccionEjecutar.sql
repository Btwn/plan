...
FROM POSTempArtJuego WITH (NOLOCK)
WHERE RID = @ID AND Estacion = @Estacion AND ID = @Codigo
SELECT  @Cantidad = Cantidad FROM POSLVenta WITH (NOLOCK) WHERE ID = @ID AND RenglonID = @RenglonID AND RenglonTipo = 'J'
IF NOT EXISTS(SELECT * FROM ArtJuego WITH (NOLOCK) WHERE Articulo = @Articulo AND Opcional = 1)
SELECT @Ok = 20626
IF NOT EXISTS(SELECT * FROM POSLVenta WITH (NOLOCK) WHERE ID = @ID AND RenglonID = @RenglonID AND RenglonTipo = 'J')
SELECT @Ok = 20336
IF @Ok IS NULL
BEGIN
...
FROM ArtJuegoOmision j WITH (NOLOCK)
JOIN ArtJuego d WITH (NOLOCK) ON j.Articulo = d.Articulo AND j.Juego = d.Juego
...
BEGIN
IF NOT EXISTS (SELECT * FROM Agente WITH (NOLOCK) WHERE Agente = @Codigo)
SELECT @Ok = 26090, @OkRef = @Codigo
IF @Ok IS NULL
UPDATE POSL WITH (ROWLOCK) SET Agente = @Codigo WHERE ID = @ID
...
BEGIN
IF NOT EXISTS (SELECT * FROM POSConceptoDINTemp WITH (NOLOCK) WHERE Estacion = @Estacion and Orden = CONVERT(INT,@Codigo))
SELECT @ok = 20702, @OkRef = @Codigo
SELECT @Concepto = Concepto
FROM POSConceptoDINTemp WITH (NOLOCK)
WHERE Estacion = @Estacion AND Orden = CONVERT(INT,@Codigo)
UPDATE POSL WITH (ROWLOCK) SET Concepto = @Concepto WHERE ID = @ID
SELECT @Codigo = NULL
END
IF @Accion = 'MODIFICAR CONDICION' AND @Ok IS NULL
BEGIN
IF NOT EXISTS (SELECT * FROM Condicion WITH (NOLOCK) WHERE Condicion = @Codigo)
SELECT @ok = 20700, @OkRef = @Codigo
UPDATE POSL WITH (ROWLOCK) SET Condicion = @Codigo WHERE ID = @ID
SELECT @Codigo = NULL
END
...
BEGIN
IF ISNULL(@Codigo,'') NOT IN (SELECT cd.CtaDinero FROM CtaDinero cd WITH (NOLOCK) WHERE cd.Tipo = 'Banco') AND @Ok IS NULL
SELECT @Ok = 10540, @OkRef = @Codigo
IF (SELECT ISNULL(Moneda, @MonedaPrincipal) FROM CtaDinero cd WITH (NOLOCK) WHERE cd.Tipo = 'Banco' AND CtaDinero = @Codigo)
NOT IN (SELECT Moneda FROM POSLTipoCambioRef WITH (NOLOCK) WHERE Sucursal = @Sucursal) AND @Ok IS NULL
SELECT @Ok = 30050, @okRef = @Codigo
END
IF @MovClave IN ('POS.TCM','POS.TRM' )
BEGIN
IF ISNULL(@Codigo,'') NOT IN (SELECT cd.CtaDinero FROM CtaDinero cd WITH (NOLOCK) WHERE cd.Tipo = 'Caja'
...
BEGIN
UPDATE POSL WITH (ROWLOCK) SET CtaDineroDestino = CtaDinero
WHERE ID = @ID
UPDATE POSL WITH (ROWLOCK) SET CtaDinero = @Codigo
WHERE ID = @ID
END
IF @MovClave IN('POS.ACM')
BEGIN
IF (SELECT NULLIF(CtaDineroDestino,'') FROM POSL WITH (NOLOCK) WHERE ID = @ID)IS NULL
UPDATE POSL WITH (ROWLOCK) SET CtaDineroDestino = CtaDinero
WHERE ID = @ID
UPDATE POSL WITH (ROWLOCK) SET CtaDinero = @Codigo
WHERE ID = @ID
...
UPDATE POSL WITH (ROWLOCK) SET CtaDineroDestino = @Codigo
WHERE ID = @ID
IF @MovClave IN ('POS.TRM','POS.TCM')
UPDATE POSL WITH (ROWLOCK) SET CtaDinero = @Codigo, CtaDineroDestino = @Caja, Caja = @Codigo
WHERE ID = @ID
...
IF NOT EXISTS(SELECT * FROM CtaDinero WITH (NOLOCK) WHERE CtaDinero = @CtaDinero AND Tipo = 'Caja') AND @Ok IS NULL
SELECT @Ok = 10510, @OkRef = @CtaDinero
IF @Sucursal <> (SELECT Sucursal FROM CtaDinero WITH (NOLOCK) WHERE CtaDinero = @CtaDinero) AND @Ok IS NULL
SELECT @Ok = 30270, @OkRef = 'La Caja pertenece a otra Sucursal'
SELECT @CajeroActual  = Cajero FROM POSL WITH (NOLOCK) WHERE ID  = @ID
IF EXISTS(SELECT * FROM POSEstatusCaja WITH (NOLOCK) WHERE Host = @Host AND Caja = @Caja AND Abierto = 1)
SELECT @Ok = 30435, @OkRef = @CajeroActual
IF @OK IS NULL
BEGIN
UPDATE POSL WITH (ROWLOCK) SET CtaDineroDestino = @CtaDinero, Caja = @CtaDinero
WHERE ID = @ID
UPDATE POSLCobro WITH (ROWLOCK) SET  CtaDineroDestino = @CtaDinero
WHERE ID = @ID
END
...
UPDATE POSL WITH (ROWLOCK) SET CtaDineroDestino = @CtaDinero
WHERE ID = @ID
UPDATE POSLCobro WITH (ROWLOCK) SET  CtaDinero = @CtaDinero, Caja = @CtaDinero, CtaDineroDestino = NULL
WHERE ID = @ID
UPDATE Usuario WITH (ROWLOCK) SET DefCtaDinero =  @CtaDinero
WHERE Usuario = @Usuario
...
UPDATE POSL WITH (ROWLOCK) SET BeneficiarioNombre = @Beneficiario,CtaDineroDestino = CtaDinero
WHERE ID = @ID
DELETE FROM POSLAccion WHERE Host = @Host AND Caja = @Caja
...
BEGIN
SELECT @Concepto = Concepto
FROM POSConceptoCXCTemp WITH (NOLOCK)
WHERE Estacion = @Estacion AND Orden = @Codigo
IF NOT EXISTS (SELECT * FROM  POSConceptoCXCTemp WITH (NOLOCK) WHERE Estacion = @Estacion AND Orden = @Codigo)
SELECT @Ok = 20336 , @OkRef = @OkRef+' POR FAVOR INTRODUZCA EL NUMERO DEL CONCEPTO'
IF @OK IS NULL
BEGIN
UPDATE POSL WITH (ROWLOCK) SET Concepto = @Concepto
WHERE ID = @ID
DELETE FROM POSLAccion WHERE Host = @Host AND Caja = @Caja
SELECT @Codigo = NULL, @CodigoAccion = NULL
...
FROM POSAlmTemp WITH (NOLOCK)
WHERE Estacion = @Estacion AND Orden = @Codigo
IF NOT EXISTS (SELECT * FROM  POSAlmTemp WITH (NOLOCK) WHERE Estacion = @Estacion AND Orden = @Codigo)
...
UPDATE POSL WITH (ROWLOCK) SET AlmacenDestino = @AlmacenD
WHERE ID = @ID
IF @MovClave = 'POS.INVA'
BEGIN
UPDATE POSL WITH (ROWLOCK) SET AlmacenDestino = Almacen
WHERE ID = @ID
UPDATE POSL WITH (ROWLOCK) SET Almacen = @AlmacenD
WHERE ID = @ID
END
DELETE FROM POSLAccion WHERE Host = @Host AND Caja = @Caja
SELECT @Codigo = NULL, @CodigoAccion = NULL
...
BEGIN
UPDATE POSL WITH (ROWLOCK) SET Referencia = @Codigo
WHERE ID = @ID
DELETE FROM POSLAccion WHERE Host = @Host AND Caja = @Caja
SELECT @Codigo = NULL
END
...
IF NOT EXISTS(SELECT * FROM Agente WITH (NOLOCK) WHERE Agente = @Cajero AND Tipo = 'Cajero') AND @Ok IS NULL
SELECT @Ok = 10515, @OkRef = @Cajero
IF @Sucursal <> (SELECT SucursalEmpresa FROM Agente WITH (NOLOCK) WHERE Agente = @Cajero) AND @Ok IS NULL
SELECT @Ok = 10200, @OkRef = 'El Cajero pertenece a otra Sucursal'
SELECT @CajeroActual  = Cajero FROM POSL WITH (NOLOCK) WHERE ID  = @ID
IF EXISTS(SELECT * FROM POSEstatusCaja WITH (NOLOCK) WHERE Host = @Host AND Caja = @Caja AND Abierto = 1)
SELECT @Ok = 30435, @OkRef = @CajeroActual
IF @OK IS NULL
UPDATE POSL WITH (ROWLOCK) SET Cajero = @Cajero
WHERE ID = @ID
DELETE FROM POSLAccion WHERE Host = @Host AND Caja = @Caja
SELECT @Codigo = NULL
END
...
BEGIN
IF NOT EXISTS(SELECT * FROM ArtOpcion  a WITH (NOLOCK) JOIN CB c WITH (NOLOCK) ON a.Articulo = c.Cuenta WHERE c.Codigo = @Codigo )
SELECT @Ok = 20740, @OkRef = @Codigo
IF @Ok IS NULL
SELECT @Articulo = Cuenta FROM CB WITH (NOLOCK) WHERE Codigo = @Codigo
SELECT @Expresion = 'POSMatrizOpciones( '+CHAR(39)+@ID+CHAR(39)+','+ CHAR(39)+@Articulo+CHAR(39)+','+' '+')'
DELETE POSLAccion WHERE Host = @Host AND Caja = @Caja
END
IF @Accion = 'CAMBIAR MOVIMIENTO'
BEGIN
IF @Codigo NOT IN (SELECT Mov FROM MovTipo WITH (NOLOCK) WHERE Modulo = 'POS'
AND Clave NOT IN ('POS.CTCAC','POS.CTCRC','POS.STE','POS.FTE','POS.TCRC','POS.TCAC','POS.SALDOIN'))
AND @Codigo NOT IN (SELECT CONVERT(varchar,Orden) FROM MovTipo WITH (NOLOCK) WHERE Modulo = 'POS')
SELECT @Ok = 26080, @OkRef = @Codigo
IF @Codigo IN (SELECT CONVERT(varchar, Orden) FROM MovTipo WITH (NOLOCK) WHERE Modulo = 'POS')
...
IF @Codigo NOT IN (SELECT Mov FROM POSUsuarioMov WITH (NOLOCK) WHERE Usuario = ISNULL(NULLIF(@UsuarioPerfil,''),@Usuario))
...
END
FROM POSL, MovTipo mt WITH (NOLOCK)
WHERE mt.Modulo = 'POS'
AND mt.Mov = @Codigo
AND ID = @ID
...
SELECT @Orden = 0
UPDATE POSAlmTemp WITH (ROWLOCK)
SET @Orden = Orden = @Orden + 1
FROM POSAlmTemp
WHERE Estacion = @Estacion
DELETE FROM POSLAccion WHERE Host = @Host AND Caja = @Caja
...
BEGIN
SELECT @CondicionPedidoContado = NULLIF(CondicionPedidoContado,'') FROM POSCfg WITH (NOLOCK) WHERE Empresa = @Empresa
UPDATE POSL WITH (ROWLOCK) SET Condicion = @CondicionPedidoContado
WHERE ID = @ID
END
...
BEGIN
IF EXISTS(SELECT * FROM POSLSerieLote WITH (NOLOCK) WHERE ID = @ID AND SerieLote = @SerieLote
...
IF NOT EXISTS(SELECT 1 FROM SerieLote WITH (NOLOCK) WHERE SerieLote = @SerieLote AND Articulo = @Articulo
...
FROM POSLSerieLote pl WITH (NOLOCK)
WHERE pl.ID = @ID
...
BEGIN
SELECT @Renglon = Renglon FROM POSLVenta WITH (NOLOCK) WHERE ID = @ID AND POSAgentePend = 1
...
UPDATE POSLVenta WITH (ROWLOCK) SET POSAgentePend = 0 WHERE ID = @ID AND Renglon = @Renglon
END
END
END
IF @Accion = 'BUSCAR MOVIMIENTO' AND @Ok IS NULL
BEGIN
IF EXISTS(SELECT * FROM POSLPorCobrar pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'POS' AND mt.Clave IN ('POS.F', 'POS.N','POS.NPC')
WHERE  pl.Estatus = 'PORCOBRAR' AND LTRIM(RTRIM(pl.Mov)) + ' ' + LTRIM(RTRIM(pl.MovId)) = @Codigo)
SELECT @Codigo = ID FROM POSLPorCobrar pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'POS' AND mt.Clave IN ('POS.F', 'POS.N','POS.NPC')
WHERE pl.Estatus = 'PORCOBRAR' AND LTRIM(RTRIM(pl.Mov)) + ' ' + LTRIM(RTRIM(pl.MovId)) = @Codigo
IF EXISTS(SELECT * FROM POSLPorCobrar WITH (NOLOCK) WHERE IDCB  = @Codigo AND Estatus = 'PORCOBRAR')
SELECT @Codigo = ID FROM POSLPorCobrar WITH (NOLOCK) WHERE IDCB  = @Codigo AND Estatus = 'PORCOBRAR'
IF EXISTS(SELECT 1 FROM POSLPorCobrar WITH (NOLOCK) WHERE ID = @Codigo AND Estatus = 'PORCOBRAR')
BEGIN
SELECT @Mov = cd.MovOmision
FROM Usuario u WITH (NOLOCK)
JOIN CtaDinero cd WITH (NOLOCK) ON u.DefCtaDinero = cd.CtaDinero AND cd.Tipo = 'Caja'
...
FROM Usuario WITH (NOLOCK)
WHERE Usuario = @Usuario
SELECT @CheckSumDO = ISNULL(DCheckSum,0), @CheckSumEO = ISNULL(ECheckSum,0) FROM POSLPorCobrar WITH (NOLOCK) WHERE ID = @Codigo
...
FROM POSLPorCobrarD WITH (NOLOCK) WHERE ID = @Codigo
...
FROM POSLPorCobrar WITH (NOLOCK)
WHERE ID = @Codigo
IF @@ERROR <> 0
SET @Ok = 1
IF @Ok IS NULL
...
IF @@ERROR <> 0
SET @Ok = 1
IF @Ok IS NULL AND EXISTS(SELECT * FROM POSLVenta p WITH (NOLOCK) JOIN Art a WITH (NOLOCK) ON p.Articulo = a.Articulo   WHERE a.Tipo IN ('Serie', 'Lote'))
BEGIN
INSERT POSLSerieLote(
ID,  RenglonID, Articulo, SubCuenta, SerieLote)
SELECT
@ID, RenglonID, Articulo, ISNULL(SubCuenta,''), SerieLote
FROM POSLPorCobrarSerieLote WITH (NOLOCK)
WHERE ID = @Codigo
END
END
ELSE
...
BEGIN
IF EXISTS(SELECT * FROM POSL pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'POS' AND mt.Clave IN ('POS.F', 'POS.N','POS.NPC','POS.P')
WHERE LTRIM(RTRIM(pl.Mov)) + ' ' + LTRIM(RTRIM(pl.MovId)) = @Codigo AND pl.Importe >= 0)
SELECT @Codigo = ID FROM POSL pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'POS' AND mt.Clave IN ('POS.F', 'POS.N','POS.NPC', 'POS.P')
WHERE LTRIM(RTRIM(pl.Mov)) + ' ' + LTRIM(RTRIM(pl.MovId)) = @Codigo AND pl.Importe >= 0
IF EXISTS(SELECT * FROM POSLCB cb WITH (NOLOCK) JOIN POSL pl WITH (NOLOCK) ON cb.ID = pl.ID WHERE cb.IDCB  = @Codigo AND pl.Importe >= 0)
SELECT @Codigo = cb.ID FROM POSLCB cb WITH (NOLOCK) JOIN POSL pl WITH (NOLOCK) ON cb.ID = pl.ID WHERE cb.IDCB = @Codigo AND pl.Importe >= 0
IF NOT EXISTS(SELECT * FROM POSL pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'POS' AND mt.Clave IN ('POS.F', 'POS.N','POS.NPC', 'POS.P')
WHERE pl.ID = @Codigo)
SELECT @Ok = 35005
IF EXISTS(SELECT * FROM POSL WITH (NOLOCK) WHERE ID = @Codigo)
BEGIN
SELECT
plv.Articulo,
plv.Cantidad,
...
FROM POSLVenta plv WITH (NOLOCK)
LEFT OUTER JOIN POSLVenta plv2 WITH (NOLOCK) ON plv2.ID = @Codigo
...
FROM POSL p WITH (NOLOCK)
WHERE ID = @ID
IF EXISTS(SELECT * FROM POSL pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'POS' AND mt.Clave IN ('POS.F', 'POS.N','POS.NPC')
WHERE LTRIM(RTRIM(pl.Mov)) + ' ' + LTRIM(RTRIM(pl.MovId)) = @Codigo AND pl.Importe >= 0)
SELECT @Codigo = ID FROM POSL pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'POS' AND mt.Clave IN ('POS.F', 'POS.N','POS.NPC')
WHERE LTRIM(RTRIM(pl.Mov)) + ' ' + LTRIM(RTRIM(pl.MovId)) = @Codigo AND pl.Importe >= 0
IF EXISTS(SELECT * FROM POSLCB cb WITH (NOLOCK) JOIN POSL pl WITH (NOLOCK) ON cb.ID = pl.ID WHERE cb.IDCB = @Codigo AND pl.Importe >= 0)
SELECT @Codigo = cb.ID FROM  POSLCB cb WITH (NOLOCK) JOIN POSL pl WITH (NOLOCK) ON cb.ID = pl.ID WHERE cb.IDCB = @Codigo AND pl.Importe >= 0
IF NOT EXISTS(SELECT * FROM POSL pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'POS' AND mt.Clave IN ('POS.F', 'POS.N','POS.NPC', 'POS.P')
WHERE pl.ID = @Codigo)
SELECT @Ok = '35005'
IF (SELECT Devolucion FROM POSL pl WITH (NOLOCK) WHERE pl.ID = @Codigo) = 1
SELECT @Ok = '20706'
IF (SELECT DevolucionP FROM POSL pl WITH (NOLOCK) WHERE pl.ID = @Codigo) = 1 AND @Accion = 'DEVOLUCION TOTAL'
SELECT @Ok = '50001'
IF (SELECT ISNULL(NULLIF(POSDiasDev,''),0) FROM POSCfg WITH (NOLOCK) WHERE Empresa = @Empresa) = 0
AND ((SELECT CONVERT (INT, dbo.fnFechaSinHora (GETDATE()) - (SELECT dbo.fnFechaSinHora(FechaRegistro) FROM POSL WITH (NOLOCK) WHERE ID = @Codigo))) >
(SELECT ISNULL(POSDiasDev,365) FROM POSCfg WITH (NOLOCK) WHERE Empresa = @Empresa))
SELECT @Ok = '20707'
IF EXISTS(SELECT * FROM POSL WITH (NOLOCK) WHERE ID = @Codigo)
BEGIN
SELECT
...
BEGIN
IF EXISTS(SELECT * FROM POSL pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'POS' AND mt.Clave IN ('POS.F', 'POS.N')
WHERE LTRIM(RTRIM(pl.Mov)) + ' ' + LTRIM(RTRIM(pl.MovId)) = @Codigo)
SELECT @Codigo = ID
FROM POSL pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'POS' AND mt.Clave IN ('POS.F', 'POS.N')
WHERE LTRIM(RTRIM(pl.Mov)) + ' ' + LTRIM(RTRIM(pl.MovId)) = @Codigo
IF EXISTS(SELECT * FROM POSLCB cb WITH (NOLOCK) JOIN POSL pl WITH (NOLOCK) ON cb.ID = pl.ID WHERE cb.IDCB = @Codigo AND pl.Importe >= 0)
SELECT @Codigo = cb.ID FROM  POSLCB cb WITH (NOLOCK) JOIN POSL pl WITH (NOLOCK) ON cb.ID = pl.ID WHERE cb.IDCB = @Codigo AND pl.Importe >= 0
IF NOT EXISTS(SELECT * FROM POSL pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'POS' AND mt.Clave IN ('POS.F', 'POS.N')
WHERE pl.ID = @Codigo)
BEGIN
IF @WebService = 1
EXEC spPOSWSSolicitudDevRef  @ID, @Empresa, @Estacion, @Sucursal, @Usuario, @Codigo, @Ok OUTPUT, @OkRef OUTPUT
IF @Ok IS NULL AND EXISTS(SELECT * FROM POSVentaPedidodTemp2 WITH (NOLOCK) WHERE Estacion = @Estacion)
BEGIN
SELECT @NoExiste = 0
...
BEGIN
IF EXISTS(SELECT * FROM POSL pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'POS' AND mt.Clave =
...
SELECT @Codigo = ID FROM POSL pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'POS' AND mt.Clave =
CASE WHEN @MovClave = 'POS.CAC' THEN 'POS.AC'
WHEN @MovClave = 'POS.CCC' THEN 'POS.CC'
WHEN @MovClave = 'POS.CPC' THEN 'POS.CP'
WHEN @MovClave = 'POS.CACM' THEN 'POS.ACM'
...
END
WHERE LTRIM(RTRIM(pl.Mov)) + ' ' + LTRIM(RTRIM(pl.MovId)) = @Codigo AND pl.Estatus = 'CONCLUIDO'
IF EXISTS(SELECT * FROM POSLCB cb WITH (NOLOCK) JOIN POSL pl WITH (NOLOCK) ON cb.ID = pl.ID WHERE cb.IDCB = @Codigo)
SELECT @Codigo = cb.ID  FROM POSLCB cb WITH (NOLOCK) JOIN POSL pl WITH (NOLOCK) ON cb.ID = pl.ID WHERE cb.IDCB = @Codigo
IF NOT EXISTS(SELECT * FROM POSL pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'POS' AND mt.Clave =
CASE WHEN @MovClave = 'POS.CAC' THEN 'POS.AC'
WHEN @MovClave = 'POS.CCC' THEN 'POS.CC'
WHEN @MovClave = 'POS.CPC' THEN 'POS.CP'
WHEN @MovClave = 'POS.CACM' THEN 'POS.ACM'
...
SELECT @Ok = 35005
IF @Ok IS NULL  AND  @MovClave NOT IN ('POS.CTCM') AND EXISTS(SELECT * FROM POSL  p WITH (NOLOCK) JOIN MovTipo m WITH (NOLOCK) ON p.Mov = m.Mov AND m.Modulo = 'POS'
WHERE p.FechaRegistro > (SELECT FechaRegistro FROM POSL WITH (NOLOCK) WHERE ID = @Codigo) AND p.Estatus IN('CONCLUIDO','TRASPASADO')
AND m.Clave NOT IN('POS.FTE','POS.STE'))
SELECT @Ok = 30151
IF @Ok IS NULL AND  @MovClave IN ('POS.CTCM') AND EXISTS(SELECT * FROM POSL p WITH (NOLOCK) JOIN MovTipo m WITH (NOLOCK) ON p.Mov = m.Mov AND m.Modulo = 'POS'
WHERE p.IDR = @Codigo AND p.Estatus IN('CONCLUIDO','TRASPASADO') AND m.Clave = 'POS.CTCM')
SELECT @Ok = 60050
IF @Ok IS NULL AND  @MovClave IN ('POS.CTRM') AND EXISTS(SELECT * FROM POSL p WITH (NOLOCK) JOIN MovTipo m WITH (NOLOCK) ON p.Mov = m.Mov AND m.Modulo = 'POS'
...
IF (SELECT Tipo FROM Art WITH (NOLOCK) WHERE Articulo = @Articulo) IN ('Serie', 'Lote')
SELECT @Ok = 73040, @OkRef = 'Esta funcionalidad no aplica para articulos tipo Serie/Lote'
IF (SELECT Tipo FROM Art WITH (NOLOCK) WHERE Articulo = @Articulo) IN ('Juego')
SELECT @Ok = 73040, @OkRef = 'Esta funcionalidad no aplica para articulos tipo Juego'
IF EXISTS(SELECT * FROM ArtOpcion WITH (NOLOCK)  WHERE Articulo = @Articulo)  AND @SubCuenta IS NULL
...
END
SELECT @CantidadCodigo=Cantidad  FROM CB WITH (NOLOCK) WHERE Codigo = @Codigo AND TipoCuenta = 'Articulos'
IF @Codigo IS NULL
SELECT @Codigo = MAX(Codigo)
FROM CB WITH (NOLOCK)
WHERE TipoCuenta = 'Articulos'
...
IF @Ok IS NULL
BEGIN
UPDATE POSLVenta WITH (ROWLOCK) SET Cantidad = Cantidad+(@Cantidad-1)
...
BEGIN
IF NOT EXISTS (SELECT * FROM POSFormaEnvioTemp WITH (NOLOCK) WHERE Estacion = @Estacion and Orden = CONVERT(INT,@Codigo))
SELECT @ok = 20702, @OkRef = @Codigo
SELECT @FormaEnvioV = FormaEnvio
FROM POSFormaEnvioTemp WITH (NOLOCK)
WHERE Estacion = @Estacion AND Orden = CONVERT(INT,@Codigo)
UPDATE POSL WITH (ROWLOCK) SET FormaEnvio = @FormaEnvioV WHERE ID = @ID
SELECT @Codigo = NULL
DELETE FROM POSLAccion WHERE Host = @Host AND Caja = @Caja
END
IF @Accion = 'COPIAR OTRO MOV VENTAS'
BEGIN
IF EXISTS(SELECT 1 FROM Venta v WITH (NOLOCK) WHERE UPPER(LTRIM(RTRIM(v.Mov))+ ' ' + LTRIM(RTRIM(v.MovID))) = @Codigo OR CONVERT(varchar,v.ID) = @Codigo
...
BEGIN
UPDATE POSL WITH (ROWLOCK) SET PedidoReferencia = @Codigo
WHERE ID = @ID
DELETE POSLAccion WHERE Host = @Host AND Accion = ISNULL(@CodigoAccion, @Accion) AND Caja = @Caja
SELECT @CodigoAccion = 'INTRODUCIR CONCEPTOCXC'
...
END
IF @Accion = 'INSERTA AGENTE' AND (SELECT POSAgenteDetalle  FROM POSCFG WITH (NOLOCK) WHERE EMPRESA = @Empresa) = 1
BEGIN
SELECT TOP 1 @Renglon = Renglon FROM POSLVenta WITH (NOLOCK) WHERE ID = @ID AND NULLIF(Agente,'') IS NULL
IF @Renglon IS NOT NULL
BEGIN
SELECT @Expresion = 'Asigna(Info.Renglon,'+CONVERT(varchar, @Renglon)+') FormaModal('+CHAR(39)+'POSAgenteDetalle'+CHAR(39)+')'
END
