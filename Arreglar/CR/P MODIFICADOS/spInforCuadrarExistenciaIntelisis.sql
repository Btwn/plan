...
SELECT @Sucursal = Sucursal FROM Alm WITH(NOLOCK) WHERE Almacen = @AlmacenU
...
FROM ArtExistenciaIntMES e  WITH(NOLOCK) JOIN Alm a  WITH(NOLOCK) ON a.Almacen = e.Almacen
...
BEGIN
SELECT @Posicion = DefPosicionRecibo FROM Alm WITH(NOLOCK) WHERE ISNULL(Ubicaciones,0) = 1 AND NULLIF(DefPosicionSurtido,'') IS NOT NULL AND Almacen = @Almacen
...
WITH(NOLOCK) WHERE ISNULL(ExistenciaMES,0.0)-ISNULL(ExistenciaInte,0.0)<0.0
SET @Renglon = 0.0
SET @RenglonID = 0
OPEN crDetalle2
...
WHILE @@FETCH_STATUS = 0 AND @Ok IS NULL
BEGIN
SELECT @Posicion = DefPosicionSurtido FROM Alm WITH(NOLOCK) WHERE ISNULL(Ubicaciones,0) = 1 AND NULLIF(DefPosicionSurtido,'') IS NOT NULL AND Almacen = @Almacen
