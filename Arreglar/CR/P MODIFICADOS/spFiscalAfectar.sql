...
BEGIN
IF NOT(@FiscalGenerarRetenciones = 1 AND (SELECT MAX(O.Tipo) FROM FiscalD d  WITH (NOLOCK) JOIN ObligacionFiscal o WITH (NOLOCK)  ON d.ObligacionFiscal = O.ObligacionFiscal WHERE ID = @ID) IN ('Retencion 1', 'Retencion 2'))
BEGIN
SELECT @CxImporte = 0.0, @CxMov = @IXPMov
SELECT @CxImporte = ISNULL(SUM(Neto), 0.0) FROM FiscalD  WITH (NOLOCK)  WHERE ID = @ID
IF @CxImporte < 0.0
