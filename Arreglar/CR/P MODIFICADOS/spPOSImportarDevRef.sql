...
WHERE ID = @ID
IF EXISTS(SELECT * FROM POSVentaPedidoDTemp2 WITH(NOLOCK) WHERE Estacion = @Estacion  AND CantidadAplicar >0.0 )
BEGIN
...
FROM POSVentaPedidoDTemp2 WITH(NOLOCK)
WHERE Estacion = @Estacion
...
WHERE plv.ID = @ID
UPDATE POSL WITH(ROWLOCK) Set Importe = ROUND(@TotalImporte,@RedondeoMonetarios), Impuestos = ROUND(@TotalImpuesto1,@RedondeoMonetarios)  WHERE ID = @ID
END
IF EXISTS(SELECT * FROM POSLVenta WITH(NOLOCK) WHERE ID = @ID AND Articulo = @ArtOfertaImporte AND RenglonTipo = 'O' AND OfertaID IS NOT NULL AND ISNULL(Puntos,0.0)<0.0)
BEGIN
...
END
IF @OK IS NULL AND EXISTS(SELECT * FROM POSVentaPedidoSerieloteTemp WITH(NOLOCK) WHERE Estacion = @Estacion)
BEGIN
DECLARE crArticulo CURSOR FOR
SELECT Articulo, SubCuenta, SerieLote, ISNULL(Cantidad,0.0), RenglonID
FROM POSVentaPedidoSerieloteTemp WITH(NOLOCK)
WHERE Estacion = @Estacion
OPEN crArticulo
