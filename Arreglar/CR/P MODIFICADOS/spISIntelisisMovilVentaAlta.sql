...
JOIN Art Art WITH(NOLOCK) ON TD.Articulo = Art.Articulo
SELECT @Moneda = LP.Moneda, @AlmacenDef = Ct.AlmacenDef
FROM Agente Ag WITH(NOLOCK)
JOIN Cte Ct WITH(NOLOCK) ON Ag.Agente = Ct.Agente AND Ag.Agente = @Agente
LEFT JOIN ListaPreciosD LP WITH(NOLOCK) ON Ct.ListaPreciosEsp = LP.Lista
...
FROM SaldoU S WITH(NOLOCK) JOIN @TableImportes I WITH(NOLOCK) ON S.Cuenta = I.Articulo
ORDER BY I.RenglonID
IF @Moneda IS NULL
SELECT TOP 1 @Moneda = Moneda FROM Mon WITH(NOLOCK) ORDER BY TipoCambio, Orden
SELECT @ListaPreciosEsp = ListaPreciosEsp, @EnviarA = EnviarA FROM CampanaD WITH(NOLOCK) WHERE RID = @IDVisita
IF NOT EXISTS(SELECT * FROM CteEnviarA WITH(NOLOCK) WHERE Cliente = @Cliente AND ID = @EnviarA)
SET @EnviarA = NULL
IF ISNULL(@ListaPreciosEsp,'') = ''
SELECT @ListaPreciosEsp = DefListaPreciosEsp FROM Usuario WITH(NOLOCK) WHERE Usuario = @Usuario
INSERT INTO Venta
...
BEGIN
UPDATE Venta WITH(ROWLOCK) SET DescuentoGlobal = CASE WHEN ISNULL(Importe,0) > 0 THEN (ISNULL(@DescuentoImporte,0.00) / ISNULL(Importe,0)) * 100 ELSE 0 END  WHERE ID = @IDPedido
END
IF @DescuentoGlobalTipo = 'porcentaje'
BEGIN
UPDATE Venta WITH(ROWLOCK) SET DescuentoGlobal = CASE WHEN ISNULL(Importe,0) > 0 THEN ISNULL(@DescuentoGlobal,0.00) ELSE 0 END  WHERE ID = @IDPedido
END
SET @Renglon = 1024
...
JOIN Art a WITH(NOLOCK) ON i.Articulo = a.Articulo
OPEN crImportes
...
FROM ArtJuego j WITH(NOLOCK)
JOIN ArtJuegoD d WITH(NOLOCK) ON j.Articulo = d.Articulo AND j.Juego = d.Juego
JOIN Art a WITH(NOLOCK) ON d.Opcion = a.Articulo
...
IF @IDVisita IS NOT NULL AND EXISTS (SELECT ID FROM CampanaD WITH(NOLOCK) WHERE RID = cast(@IDVisita as int))
BEGIN
SELECT @IDO = ID
