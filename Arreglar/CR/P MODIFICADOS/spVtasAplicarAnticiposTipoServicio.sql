...
UPDATE Venta WITH(ROWLOCK) SET AnticipoFacturadoTipoServicio = 0 WHERE ID = @ID
...
SELECT @Unidad = Unidad FROM Art WITH(NOLOCK) WHERE Articulo = @CfgAnticipoArticuloServicio
IF @CfgAnticipoTipoServicio = 1 AND @Ok IS NULL
BEGIN
SELECT @Renglon = ISNULL(MAX(ISNULL(Renglon,0.0)),0.0) + 2048.0 FROM VentaD WITH(NOLOCK) WHERE ID = @ID
SELECT @RenglonID = ISNULL(MAX(ISNULL(RenglonID,0)),0.0) + 1 FROM VentaD WITH(NOLOCK) WHERE ID = @ID
