...
FROM InterfaseAspel WITH (NOLOCK) WHERE SistemaAspel = 'NOI' AND Empresa = @Empresa
SELECT @TipoPeriodo = TipoPeriodo,@CtaDinero = CtaDinero ,@Moneda = Moneda , @ConceptoDIN =ConceptoDIN
FROM InterfaseAspelNOI WITH (NOLOCK) WHERE  Empresa = @Empresa
SELECT @ZonaEconomica = ZonaEconomica FROM Sucursal WITH (NOLOCK) WHERE Sucursal = @Sucursal
SET @OK = NULL
