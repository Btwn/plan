...
WHERE Campo = 'AgenteA'
SELECT @Agente = DefAgente FROM Usuario WITH(NOLOCK) WHERE Usuario = @Agente
IF NOT EXISTS(SELECT * FROM Agente WITH(NOLOCK) WHERE Agente = @Agente) SELECT @Ok = 26090, @OkRef=@Agente 
SELECT @Texto =(SELECT Inv.* FROM Inv  WITH(NOLOCK) JOIN MovTipo mt  WITH(NOLOCK) ON Inv.Mov=mt.Mov AND mt.Modulo='INV'
