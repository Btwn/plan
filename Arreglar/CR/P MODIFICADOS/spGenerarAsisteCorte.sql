...
GO
ALTER PROCEDURE spGenerarAsisteCorte
@Sucursal		int,
@Empresa		char(5),
@Usuario		char(10),
...
WITH(NOLOCK) WHERE p.Jornada = j.Jornada
...
/*    IF (SELECT AsisteJornadasNocturnas FROM EmpresaCfg WITH(NOLOCK) WHERE Empresa = @Empresa) = 1 AND @Ok IS NULL
