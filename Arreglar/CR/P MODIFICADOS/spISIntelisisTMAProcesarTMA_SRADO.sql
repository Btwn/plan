...
SELECT @Articulo = Articulo FROM ArtDisponibleTarima WITH (NOLOCK)  WHERE Tarima = @Tarima AND Almacen = @Almacen
IF (SELECT COUNT(*) FROM TMA a WITH (NOLOCK)  /*WITH(NOLOCK)*/
INNER JOIN TMAD b WITH (NOLOCK)  /*WITH(NOLOCK)*/ ON a.ID = b.ID
...
b.PosicionDestino IN(SELECT Posicion FROM AlmPos  WITH (NOLOCK) WHERE Almacen=@Almacen AND ArticuloEsp=@Articulo AND Tipo='Domicilio') AND a.Almacen = @Almacen
...
IF @OK IS NULL
BEGIN
SELECT @IDOrigen = ID FROM TMA WITH (NOLOCK)  WHERE Empresa = @Empresa AND Sucursal = @Sucursal AND Mov = @Mov AND MovID = @MovID
...
IF @Ok IS NULL
BEGIN
SELECT @Mov2 = isnull(Mov,'') , @MovID2 = isnull(MovID,'') FROM TMA  WITH (NOLOCK) WHERE ID = @IDDestino
END
IF @@ERROR <> 0 SET @Ok = 1
IF @Ok IS NOT NULL SELECT @OkRef = REPLACE(Descripcion, '<BR>', '') FROM MensajeLista  WITH (NOLOCK) WHERE Mensaje = @Ok
IF @Ok IS NULL
BEGIN
IF (SELECT Estatus FROM TMA WITH (NOLOCK)  WHERE ID = @IDDestino) = 'SINAFECTAR'
SELECT @Ok = -1
ELSE
SELECT @OkRef = RTRIM(Mov)+' '+RTRIM(MovID) FROM TMA WITH (NOLOCK)  WHERE ID = @IDDestino
END
