...
@MapaLatitud            float,
@MapaLongitud           float
SELECT @POSMovID = MovID FROM POSL WITH(NOLOCK) WHERE ID = @IDText
SET @Contador = 1
DECLARE crDatos CURSOR LOCAL FOR
SELECT Ubicacion, MapaLatitud, MapaLongitud
FROM POSLVentaEmbarques WITH(NOLOCK)
...
JOIN POSLVentaEmbarques e WITH(NOLOCK) ON e.ID = pmv.ID AND e.Renglon = pmv.Renglon AND e.RenglonID = pmv.RenglonID AND e.Articulo = pmv.Articulo
