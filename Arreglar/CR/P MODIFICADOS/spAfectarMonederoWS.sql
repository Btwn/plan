...
BEGIN
UPDATE Monedero WITH(ROWLOCK) SET MovID = @MovID WHERE ID = @ID
IF EXISTS(SELECT * FROM MonederoD WITH(NOLOCK) WHERE ID = @ID)
BEGIN
IF NOT EXISTS(SELECT *
FROM Monedero M WITH(NOLOCK)
JOIN MonederoD D WITH(NOLOCK) ON M.ID = D.ID
JOIN TarjetaMonedero T WITH(NOLOCK) ON M.Empresa = T.Empresa AND D.Serie = T.Serie
WHERE M.ID = @ID
AND T.Estatus IN ('ACTIVA'))
BEGIN
UPDATE TarjetaMonedero WITH(ROWLOCK)
...
FROM Monedero WITH(NOLOCK)
WHERE ID = @ID
IF NOT EXISTS(SELECT * FROM MovTiempo WITH(NOLOCK) WHERE Modulo = @Modulo AND ID = @ID AND Estatus = @EstatusN)
INSERT MovTiempo
