...
IF ISNULL((SELECT ecm.TransferenciaManual FROM EmpresaCfgMES ecm WITH(NOLOCK) WHERE ecm.Empresa = @Empresa), 0) = 0
BEGIN
SELECT 'Desactivado'
RETURN
END
IF (SELECT SeriesLotesAutoOrden FROM EmpresaCfg ec WITH(NOLOCK) WHERE ec.Empresa = @Empresa) = 'No'
BEGIN
SELECT @Articulo = MIN(m.Hijo)
FROM MesInvTransfer m WITH(NOLOCK)
JOIN Art a WITH(NOLOCK) ON m.Hijo = a.Articulo
LEFT OUTER JOIN MesInvTransferSerieLote sl WITH(NOLOCK) ON sl.Usuario = m.Usuario AND sl.ID = m.ID
...
SELECT @Moneda = MonedaCosteo FROM EmpresaCfg WITH(NOLOCK) WHERE Empresa = @Empresa
...
FROM MesInvTransfer mit WITH(NOLOCK)
...
FROM MesInvTransfer mit WITH(NOLOCK)
JOIN Art a WITH(NOLOCK) ON mit.Hijo = a.Articulo
WHERE mit.Usuario = @Usuario
AND mit.Lanzamiento = @Lanzamiento
AND ISNULL(mit.CantidadA, 0) > 0
...
FROM MesInvTransferSerieLote sl WITH(NOLOCK)
...
IF @Ok IS NULL
BEGIN
UPDATE PrevisionesConsumoMES WITH(ROWLOCK) SET EstatusIntelIMES = CASE WHEN ISNULL(PrevisionesConsumoMES.Transferido, 0)+ CASE a.AlmacenROP WHEN mit.AlmacenD THEN ISNULL(mit.CantidadA, 0) ELSE 0 END >= ISNULL(mit.Cantidad, 0) THEN 2 ELSE 1 END,
...
ELSE
BEGIN
SELECT @Mensaje = ml.Descripcion FROM MensajeLista ml WITH(NOLOCK) WHERE ml.Mensaje = @Ok
