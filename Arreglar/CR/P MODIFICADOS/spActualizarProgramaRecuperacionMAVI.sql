...
IF @MovTipo = 'CXC.FAC'
BEGIN
SELECT @Mayor12Meses = Mayor12Meses, @EsCredilana = EsCredilana FROM Cxc WITH(NOLOCK) WHERE Mov = @MovAplica AND MovID = @MovAplicaID AND Estatus IN('Pendiente', 'Concluido')
UPDATE Cxc WITH(ROWLOCK) SET Mayor12Meses = @Mayor12Meses, EsCredilana = @EsCredilana WHERE ID = @ID
END
ELSE
BEGIN
SELECT @Mayor12Meses = Mayor12Meses FROM Cxc WITH(NOLOCK) WHERE ID = @ID
...
UPDATE Cxc WITH(ROWLOCK) SET Mayor12Meses = 1 WHERE ID = @ID
END
ELSE
BEGIN
SELECT @Mayor12Meses = 0
UPDATE Cxc WITH(ROWLOCK) SET Mayor12Meses = 0 WHERE ID = @ID
END
END
ELSE
BEGIN
...
BEGIN
UPDATE Cxc WITH(ROWLOCK) SET Mayor12Meses = @Mayor12Meses, EsCredilana = @EsCredilana WHERE Mov = 'Documento' AND OrigenTipo = 'CXC' AND Origen = @Mov AND OrigenID = @MovID AND Estatus IN('PENDIENTE','CONCLUIDO')
END
END
END
END
...
SELECT @TotalDoctos = COUNT(*) FROM CxcD WITH(NOLOCK) WHERE ID = @ID
IF @TotalDoctos > 0
BEGIN
DECLARE DoctosACobrar CURSOR FOR SELECT C.Renglon, C.Importe, C.Aplica, C.AplicaID, C.DescuentoRecargos FROM CxcD C WITH(NOLOCK) WHERE ID = @ID
OPEN DoctosACobrar
SET @Cont = 0
...
SELECT @IDPuente = ID FROM Cxc WITH(NOLOCK) WHERE Mov = @CxcMov AND MovID = @CxcMovID AND Estatus IN('Pendiente', 'Concluido')
SELECT TOP 1 @CxcMov = Aplica, @CxcMovID = AplicaID FROM CxcD WITH(NOLOCK) WHERE ID = @IDPuente
END 
ELSE
BEGIN 
...
BEGIN
SELECT @CxcID = ID FROM Cxc WITH(NOLOCK) WHERE Mov = @CxcMov AND MovID = @CxcMovID
IF EXISTS(SELECT ID FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID)
BEGIN 
IF EXISTS(SELECT ID FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID AND Periodo = @PeriodoTmp AND Ejercicio = @EjercicioTmp) 
BEGIN
SELECT @Creditos = ISNULL(Creditos, 0.0), @Cobro = ISNULL(Cobros, 0.0), @PorcentajeFinanciamiento = ISNULL(PorcentajeFinanciamiento, 0.0), @Renglon = ISNULL(Renglon, 0) FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID AND Periodo = @PeriodoTmp AND Ejercicio = @EjercicioTmp
IF @EsNC = 1
...
IF @EsNC = 1
BEGIN
UPDATE  RecuperacionCredilanasPPMAVI WITH(ROWLOCK) SET Creditos = @Creditos, PagoFinanciamiento = @PagoFinanciamiento WHERE ID = @CxcID AND Periodo = @PeriodoTmp AND Ejercicio = @EjercicioTmp
...
BEGIN
DELETE RecuperacionCredilanasPPMAVID WHERE ID = @CxcID AND CobroID = @ID AND Renglon = @Renglon AND CxcMov = (CASE @EsPuente WHEN 1 THEN @MovPuente ELSE @CxcMov END) AND CxcMovID = (CASE @EsPuente WHEN 1 THEN @MovIDPuente ELSE @CxcMovID END) 
UPDATE  RecuperacionCredilanasPPMAVI WITH(ROWLOCK) SET Cobros = @Cobro, PagoFinanciamiento = @PagoFinanciamiento WHERE ID = @CxcID AND Periodo = @PeriodoTmp AND Ejercicio = @EjercicioTmp
...
ELSE
BEGIN 
SELECT @PorcentajeFinanciamiento = ISNULL(PorcentajeFinanciamiento, 0.0), @PorcentajeCapital = ISNULL(PorcentajeCapital, 0.0) FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID
...
END
SELECT @Renglon = ISNULL(Renglon, 0) FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID AND Periodo = @PeriodoTmp AND Ejercicio = @EjercicioTmp
DELETE RecuperacionCredilanasPPMAVID WHERE ID = @CxcID AND CobroID = @ID AND Renglon = @Renglon AND CxcMov = (CASE @EsPuente WHEN 1 THEN @MovPuente ELSE @CxcMov END) AND CxcMovID = (CASE @EsPuente WHEN 1 THEN @MovIDPuente ELSE @CxcMovID END) 
IF @EsNC = 1
BEGIN
...
BEGIN
SELECT @CxcOrigenMov = Origen, @CxcOrigenMovID = OrigenID FROM Cxc WITH(NOLOCK) WHERE Mov = @CxcMov AND MovID = @CxcMovID 
IF @CxcOrigenMov IN('Credilana','Prestamo Personal')
BEGIN 
SELECT @CxcID = ID FROM Cxc WITH(NOLOCK) WHERE Mov = @CxcOrigenMov AND MovID = @CxcOrigenMovID 
IF EXISTS(SELECT ID FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID)
BEGIN 
IF EXISTS(SELECT ID FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID AND Periodo = @PeriodoTmp AND Ejercicio = @EjercicioTmp) 
BEGIN
SELECT @Cobro = ISNULL(Cobros, 0.0), @Creditos = ISNULL(Creditos, 0.0), @PorcentajeFinanciamiento = ISNULL(PorcentajeFinanciamiento, 0.0), @Renglon = ISNULL(Renglon, 0) FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID AND Periodo = @PeriodoTmp AND Ejercicio = @EjercicioTmp
IF @EsNC = 1
SELECT @Creditos = (@Creditos + ISNULL(@CxcImporte, 0.0))
ELSE
SELECT @Cobro = (@Cobro + ISNULL(@CxcImporte, 0.0))
...
IF @CxcMov = 'Endoso'
BEGIN
SELECT @CxcOrigenMov = MovAplica, @CxcOrigenMovID = MovAplicaID FROM Cxc WITH(NOLOCK) WHERE Mov = @CxcMov AND MovID = @CxcMovID AND Estatus IN('PENDIENTE', 'CONCLUIDO')
IF @CxcOrigenMov IN('Credilana','Prestamo Personal')  
BEGIN
SELECT @CxcID = ID FROM Cxc WITH(NOLOCK) WHERE Mov = @CxcOrigenMov AND MovID = @CxcOrigenMovID AND Estatus IN('CONCLUIDO','PENDIENTE') 
IF EXISTS(SELECT ID FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID)
BEGIN 
IF EXISTS(SELECT ID FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID AND Periodo = @PeriodoTmp AND Ejercicio = @EjercicioTmp) 
BEGIN
...
BEGIN
SELECT @CxcOrigenMovTmp = Origen, @CxcOrigenMovIDTmp = OrigenID FROM Cxc WITH(NOLOCK) WHERE Mov = @CxcOrigenMov AND MovID = @CxcOrigenMovID AND Estatus IN('CONCLUIDO','PENDIENTE') 
IF @CxcOrigenMovTmp IN('Credilana','Prestamo Personal')
BEGIN 
SELECT @CxcID = ID FROM Cxc WITH(NOLOCK) WHERE Mov = @CxcOrigenMovTmp AND MovID = @CxcOrigenMovIDTmp AND Estatus IN('PENDIENTE','CONCLUIDO') 
IF EXISTS(SELECT ID FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID)
BEGIN 
IF EXISTS(SELECT ID FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID AND Periodo = @PeriodoTmp AND Ejercicio = @EjercicioTmp) 
BEGIN
...
BEGIN
SELECT @CxcOrigenMovTmp = MovAplica, @CxcOrigenMovIDTmp = MovAplicaID FROM Cxc WITH(NOLOCK) WHERE Mov = @CxcOrigenMov AND MovID = @CxcOrigenMovID AND Estatus IN('CONCLUIDO','PENDIENTE') 
WHILE @CxcOrigenMovTmp = 'Endoso'  
BEGIN
SELECT @CxcOrigenMovTmp = MovAplica, @CxcOrigenMovIDTmp = MovAplicaID FROM Cxc WITH(NOLOCK) WHERE Mov = @CxcOrigenMovTmp AND MovID = @CxcOrigenMovIDTmp AND Estatus IN('CONCLUIDO','PENDIENTE')
END
IF @CxcOrigenMovTmp IN('Credilana','Prestamo Personal')  
BEGIN
SELECT @CxcID = ID FROM Cxc WITH(NOLOCK) WHERE Mov = @CxcOrigenMovTmp AND MovID = @CxcOrigenMovIDTmp AND Estatus IN('CONCLUIDO','PENDIENTE') 
IF EXISTS(SELECT ID FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID)
BEGIN 
IF EXISTS(SELECT ID FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID AND Periodo = @PeriodoTmp AND Ejercicio = @EjercicioTmp) 
BEGIN
...
BEGIN
SELECT @CxcOrigenMovTmp2 = Origen, @CxcOrigenMovIDTmp2 = OrigenID FROM Cxc WITH(NOLOCK) WHERE Mov = @CxcOrigenMovTmp AND MovID = @CxcOrigenMovIDTmp AND Estatus IN('CONCLUIDO','PENDIENTE') 
IF @CxcOrigenMovTmp2 IN('Credilana','Prestamo Personal')
BEGIN 
SELECT @CxcID = ID FROM Cxc WITH(NOLOCK) WHERE Mov = @CxcOrigenMovTmp2 AND MovID = @CxcOrigenMovIDTmp2 AND Estatus IN('PENDIENTE','CONCLUIDO') 
IF EXISTS(SELECT ID FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID)
BEGIN 
IF EXISTS(SELECT ID FROM RecuperacionCredilanasPPMAVI WITH(NOLOCK) WHERE ID = @CxcID AND Periodo = @PeriodoTmp AND Ejercicio = @EjercicioTmp) 
BEGIN
