...
FROM ListaSt a WITH (NOLOCK) JOIN  POSArtSucursalTemp t WITH (NOLOCK) ON a.Estacion = t.Estacion
JOIN Sucursal s WITH (NOLOCK) ON s.Sucursal = t.Sucursal
...
FROM ListaSt a WITH (NOLOCK) JOIN  POSArtSucursalTemp t WITH (NOLOCK) ON a.Estacion = t.Estacion
JOIN Sucursal s WITH (NOLOCK) ON s.Sucursal = t.Sucursal
JOIN CB c WITH (NOLOCK) ON c.Cuenta = a.Clave AND c.TipoCuenta = 'Articulos'
WHERE a.Estacion = @Estacion
EXCEPT
SELECT Sucursal,   Codigo,   Articulo,  SubCuenta,    Host, Cantidad,   Unidad
FROM POSArtCodigoSucursal WITH (NOLOCK)
...
art.TipoRetencion3
FROM @Tabla a JOIN Art art WITH (NOLOCK) ON a.Articulo = art.Articulo
