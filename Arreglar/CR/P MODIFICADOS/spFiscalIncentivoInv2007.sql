...
FROM CompraTCalc c WITH (NOLOCK)
JOIN MovTipo mt WITH (NOLOCK) ON mt.Modulo = 'COMS' AND mt.Mov = c.Mov AND mt.Clave IN ('COMS.F', 'COMS.FL', 'COMS.EG', 'COMS.EI')
...
FROM VentaTCalc c WITH (NOLOCK)
JOIN MovTipo mt WITH (NOLOCK) ON mt.Modulo = 'VTAS' AND mt.Mov = c.Mov AND mt.Clave IN ('VTAS.F','VTAS.FAR', 'VTAS.D', 'VTAS.DF', 'VTAS.B')
...
FROM #Reporte r
JOIN Cxp c WITH (NOLOCK) ON c.Empresa = @Empresa AND c.Estatus IN ('PENDIENTE', 'CONCLUIDO')
...
FROM #Reporte r
LEFT OUTER JOIN Cte c WITH (NOLOCK) ON c.Cliente = r.Cliente
LEFT OUTER JOIN Prov p WITH (NOLOCK) ON p.Proveedor = r.Proveedor
LEFT OUTER JOIN Art a WITH (NOLOCK) ON a.Articulo = r.Articulo
ORDER BY r.ID
RETURN
END
