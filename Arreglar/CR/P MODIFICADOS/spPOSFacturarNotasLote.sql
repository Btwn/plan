...
IF NOT EXISTS(SELECT * FROM ListaSt l WITH(NOLOCK) JOIN POSL p WITH(NOLOCK) ON p.Mov+' '+p.MovID = l.Clave AND p.Empresa = @Empresa AND p.Facturado = 0 WHERE l.Estacion = @Estacion )
...
FROM POSL WITH(NOLOCK)
WHERE Empresa = @Empresa AND Mov+' '+MovID IN(SELECT Clave FROM ListaSt WITH(NOLOCK) WHERE Estacion = @Estacion)
...
SELECT p.ID
FROM ListaSt l WITH(NOLOCK) JOIN POSL p ON p.Mov+' '+p.MovID = l.Clave AND p.Empresa = @Empresa
...
ORDER BY Orden
END
IF EXISTS(SELECT * FROM POSLCobro WITH(NOLOCK) WHERE ID = @IDPOS)
BEGIN
...
UPDATE POSLVenta WITH(ROWLOCK) SET Articulo = @ArtTarjetaServicio, RenglonTipo = 'N'
...
UPDATE POSL WITH(ROWLOCK) SET Mov =  @MovDev,
MovID =  @MovIDDev,
...
@EsMonedero
IF @@ERROR <> 0 SET @Ok = 1
IF @Ok IS NULL AND EXISTS(SELECT * FROM POSLSerieLote WITH(NOLOCK) WHERE ID = @IDPOS AND RenglonID = @RenglonIDD)
BEGIN
...
FETCH NEXT FROM crDetalle WITH(NOLOCK) INTO  @RenglonD, @RenglonIDD, @RenglonTipo, @Cantidad, @CantidadObsequio, @Articulo, @SubCuenta, @Precio,
@PrecioSugerido, @DescuentoLinea, @Impuesto1, @Impuesto2, @Impuesto3, @Unidad, @Factor,  @Puntos,
@PrecioImpuestoInc, @Almacen, @Codigo, @EsMonedero
END
CLOSE crDetalle
...
FROM @VentaDetalle
IF @@ERROR <> 0 SET @Ok = 1
IF EXISTS(SELECT * FROM POSLSerieLote WITH(NOLOCK) WHERE ID = @IDPOS)
INSERT POSLSerieLote(ID, RenglonID, Articulo, SubCuenta, SerieLote)
...
UPDATE POSL WITH(ROWLOCK) SET MovID =  @MovIDFact,
Prefijo = ISNULL(Prefijo, @Prefijo),
Consecutivo = ISNULL(Consecutivo, @Consecutivo),
noAprobacion = ISNULL(noAprobacion, @NoAprobacion),
fechaAprobacion = ISNULL(fechaAprobacion, @fechaAprobacion),
...
FROM POSLVenta pmv WITH(NOLOCK)
JOIN POSL p WITH(NOLOCK) ON pmv.ID = p.ID
WHERE pmv.ID = @IDFact
AND ISNULL(pmv.precio,0.0) <> 0.0
IF @@ERROR <> 0 SET @Ok = 1
EXEC spPOSInsertarPOSVentaCobro @IDFact, @Empresa,@Sucursal, @Ok OUTPUT, @OkRef OUTPUT
IF EXISTS(SELECT 1 FROM POSLSerieLote pls WITH(NOLOCK) WHERE pls.ID = @IDFact)
...
FROM POSLSerieLote pls WITH(NOLOCK)
WHERE pls.ID = @IDFact
...
IF @Ok IS NULL AND EXISTS(SELECT * FROM CFD WITH(NOLOCK) WHERE Modulo = 'VTAS' AND ModuloID = @IDNuevo)
BEGIN
...
FROM CFD cfd WITH(NOLOCK)
WHERE cfd.Modulo = 'VTAS'
AND cfd.ModuloID = @IDNuevo
IF @Ok IS NULL
UPDATE POSL WITH(ROWLOCK) SET CadenaOriginal = @CadenaOriginal,
Sello = @Sello,
...
END
IF @Ok IS NULL
UPDATE POSL WITH(ROWLOCK) SET Facturado = 1 WHERE Empresa = @Empresa AND Mov+' '+MovID IN(SELECT Clave FROM ListaSt WHERE Estacion = @Estacion)
IF @Ok IS NULL
BEGIN
COMMIT TRANSACTION
