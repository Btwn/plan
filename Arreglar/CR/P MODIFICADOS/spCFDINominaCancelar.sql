...
SELECT @Estatus = Estatus, @Empresa = Empresa, @Sucursal = Sucursal, @Mov = Mov, @MovID = MovID FROM Nomina WITH (NOLOCK) WHERE ID = @ID
SELECT @Timbrado = ISNULL(Timbrado, 'No Timbrado') FROM CFDINominaRecibo WITH (NOLOCK) WHERE ID = @ID AND Personal = @Personal
...
IF @Ok IS NOT NULL
UPDATE CFDINominaRecibo WITH (ROWLOCK) SET OkCancela = @Ok, OkRefCancela = @OkRef WHERE ID = @ID AND Personal = @Personal
ELSE IF @Ok IS NULL
BEGIN
IF /*@FirmaCancelacionSAT IS NOT NULL AND */@Ok IS NULL
BEGIN
UPDATE CFDNomina WITH (ROWLOCK)
SET Cancelado		= 1,
...
IF NOT EXISTS(SELECT * FROM AnexoMov WITH (NOLOCK) WHERE Rama = 'NOM' AND ID = @ID AND CFD = 1 AND Direccion = @AlmacenarRuta)
...
UPDATE CFDINominaRecibo WITH (ROWLOCK) SET Timbrado = 'No Timbrado', OkCancela = NULL, OkRefCancela = NULL WHERE ID = @ID AND Personal = @Personal
...
UPDATE Nomina WITH (ROWLOCK) SET CFDTimbrado = 0 WHERE ID = @ID
END
END
END
