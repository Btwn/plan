...
SELECT @EsEcuador = EsEcuador FROM Empresa WITH(NOLOCK) WHERE Empresa = @Empresa
...
IF @Modulo = 'VTAS'
BEGIN
SELECT @Modulo = ConsecutivoModulo FROM MovTipo WITH(NOLOCK) WHERE Mov = @Mov AND Modulo = @Modulo
SELECT TOP 1
@PuntoEmision = PuntoEmision,
@AutorizacionSRI = Autorizacion,
@VigenteA = Vigencia,
@Establecimiento = Establecimiento
FROM EcuadorAutorizacion WITH(NOLOCK)
 WHERE Modulo = @Modulo
AND Mov = @Mov
