...
vd.DescripcionExtra
INTO #Pedido
FROM Venta v1 WITH(NOLOCK) JOIN Venta v2 WITH(NOLOCK) ON  v1.Origen=v2.Mov AND v1.OrigenID=v2.MovID
JOIN ( SELECT ID,Aplica,AplicaID
FROM VentaD a WITH(NOLOCK) WHERE ID=(
SELECT b.ID  FROM Venta a WITH(NOLOCK) JOIN Venta b WITH(NOLOCK) ON  a.Origen=b.Mov AND a.OrigenID=b.MovID
WHERE a.Mov IN('Factura','Factura CFD') AND a.ID =@ID
)
GROUP BY ID,Aplica,AplicaID
) vd1  ON vd1.ID =v2.ID
JOIN Venta  v3 WITH(NOLOCK) ON v3.Mov=vd1.Aplica  AND v3.MovID=Vd1.AplicaID
JOIN VentaD vd WITH(NOLOCK) ON vd.ID=v3.ID
LEFT OUTER JOIN cb WITH(NOLOCK) ON vd.articulo = cb.cuenta
LEFT OUTER JOIN artunidad au WITH(NOLOCK) ON au.Articulo =vd.Articulo
WHERE  v1.ID =@ID
AND v1.Referencia =@Referencia
AND v1.Mov IN  ('Factura','Factura CFD')
AND v3.estatus <>'CANCELADO'
AND au.Unidad  = 'Empaque'
AND vd.Articulo IN (SELECT Articulo FROM VentaD WITH(NOLOCK) WHERE ID=@ID)
ORDER BY v3.MovID
