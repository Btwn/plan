...
SELECT @Mov = 'Campaña'
IF @ID IS NOT NULL AND EXISTS (SELECT 1 FROM Campana WITH(NOLOCK) WHERE ID = @ID AND Estatus = 'SINAFECTAR')
UPDATE Campana WITH(ROWLOCK) SET
...
WHERE ID = @ID
ELSE IF NOT EXISTS (SELECT 1 FROM Campana WITH(NOLOCK) WHERE ID = @ID)
