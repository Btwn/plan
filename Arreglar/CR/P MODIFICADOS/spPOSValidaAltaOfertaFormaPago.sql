...
BEGIN
IF EXISTS(SELECT * FROM  POSOfertaFormaPago WITH (NOLOCK) WHERE ID <> @ID AND Estatus = 'ACTIVA' AND  FormaPago = @FormaPago  AND FechaD <= @FechaA AND FechaA >= @FechaD)
BEGIN
IF EXISTS(SELECT * FROM  POSOfertaFormaPago WITH (NOLOCK) WHERE ID <> @ID AND Estatus = 'ACTIVA'
