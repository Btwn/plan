...
JOIN NominaD WITH(NOLOCK) ON NominaD.Id = Nomina.Id
AND NominaD.Personal = @Personal
JOIN Empresa WITH(NOLOCK) ON Empresa.Empresa = Nomina.Empresa
...
AND Nomina.Mov IN (SELECT Mov FROM MovTipo WITH(NOLOCK) WHERE Modulo = 'NOM' AND RTRIM(UPPER(Clave)) IN ('NOM.N')) /*, 'NOM.NE', 'NOM.NC', 'COMS.B', 'COMS.CC', 'COMS.FL', 'COMS.CP', 'COMS.OP', 'COMS.O', 'COMS.C', 'COMS.DC', 'COMS.D','COMS.DG', 'COMS.F','COMS.EG', 'COMS.EI', 'COMS.EST', 'COMS.CA', 'COMS.GX', 'COMS.IG', 'COMS.R', 'COMS.OI', 'COMS.OD', 'COMS.OG', 'COMS.PR', 'COMS.R', 'COMS.F', 'COMS.D')) */
...
FROM NominaConsulta WITH(NOLOCK) WHERE ID = @Id AND Personal = @Personal AND Empresa = @ClaveEmpresa AND MovID = @MovId
UPDATE #ConcNom
...
IF @@Rowcount = 0
BEGIN
SELECT @NombreAnexoXML = Nombre FROM AnexoMov WITH(NOLOCK) WHERE Rama = 'NOM' AND id = @Id AND Nombre Like '%' + ltrim(rtrim(@Personal)) + '%.xml%'
SELECT @NombreAnexoPDF = Nombre FROM AnexoMov WITH(NOLOCK) WHERE Rama = 'NOM' AND id = @Id AND Nombre Like '%' + ltrim(rtrim(@Personal)) + '%.pdf%'
