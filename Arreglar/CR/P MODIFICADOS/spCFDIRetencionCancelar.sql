...
SELECT @Estatus = Estatus, @Empresa = Empresa, @Sucursal = Sucursal, @Mov = Mov, @MovID = MovID, @Proveedor = Proveedor FROM Cxp WITH (NOLOCK) WHERE ID = @ID
SELECT @ConceptoSAT = ConceptoSAT, @UUID = UUID FROM CFDRetencion WITH (NOLOCK) WHERE Modulo = 'CXP' AND ModuloID = @ID
...
UPDATE CFDRetencion WITH (ROWLOCK)
SET Cancelado		= 1,
...
IF NOT EXISTS(SELECT * FROM AnexoMov WITH (NOLOCK) WHERE Rama = 'CXP' AND ID = @ID AND CFD = 1 AND Direccion = @AlmacenarRuta)
...
ELSE IF @Modulo = 'DIN'
UPDATE Dinero WITH (ROWLOCK) SET CFDRetencionTimbrado = 0 WHERE ID = @ModuloID
IF NOT EXISTS(SELECT * FROM AnexoMov WITH (NOLOCK) WHERE Rama = @Modulo AND ID = @ModuloID AND CFD = 1 AND Direccion = @AlmacenarRuta)
INSERT AnexoMov ( Sucursal,  Rama,    ID,        Nombre,         Direccion,     Tipo,      Icono, CFD)
