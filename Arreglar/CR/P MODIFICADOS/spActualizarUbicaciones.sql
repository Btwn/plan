...
FROM invArtUbicaciones WITH(NOLOCK)
...
FROM ExistenciaAlternaPosicionSerieLote el WITH(NOLOCK)
inner join invd id WITH(NOLOCK) on EL.ARTICULO=ID.ARTICULO AND ISNULL(el.SUBCUENTA,'')= ISNULL(id.subcuenta,'')
...
FROM invArtUbicaciones WITH(NOLOCK) WHERE id=@ID and renglon =@Renglon AND Articulo =@Articulo AND Almacen=@Almacen
OPEN crUbicaciones
...
UPDATE invArtUbicaciones WITH(ROWLOCK) SET CantidadA=@CantidadA,PosicionDestino=@PosicionDestino
WHERE Articulo=@Articulo AND SerieLote=@SerieLote AND Unidad=@Unidad AND  Posicion=@Posicion AND Almacen=@Almacen
...
FROM InvArtUbicacionD WITH(NOLOCK)
WHERE rid=@RID
