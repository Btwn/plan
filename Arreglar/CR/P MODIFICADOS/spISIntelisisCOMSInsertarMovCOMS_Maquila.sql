...
SELECT @Estacion = 1 ,@Usuario2 = Usuario FROM Acceso WITH(NOLOCK) WHERE ID = @IDAcceso
IF @Ok IS NULL
BEGIN
...
IF NOT EXISTS(SELECT * FROM MovTipo WITH(NOLOCK) WHERE Modulo = 'COMS' AND Mov = @Mov ) SELECT @Ok = 14055
...
Moneda       = m.Moneda,
TipoCambio      = m.TipoCambio
FROM Compra WITH(NOLOCK), Prov p WITH(NOLOCK), Mon m WITH(NOLOCK)
...
SELECT @AlmacenDestino = Almacen FROM Prov WITH(NOLOCK) WHERE Proveedor = @Proveedor
SELECT @DefPosicionSurtido = DefPosicionSurtido FROM Alm WITH(NOLOCK) WHERE Almacen = @AlmacenMatPrima
SELECT  @DefPosicionRecibo = DefPosicionRecibo FROM Alm WITH(NOLOCK) WHERE Almacen = @AlmacenDestino
SELECT TOP 1 @MovTransferencia = Mov FROM MovTipo WITH(NOLOCK) WHERE Modulo = 'INV' AND Clave = 'INV.OT'
SELECT TOP 1 @MovEntrada = Mov FROM MovTipo WITH(NOLOCK) WHERE Modulo = 'INV' AND Clave = 'INV.E'
IF EXISTS(SELECT * FROM @Temp a JOIN ArtMaterial m WITH(NOLOCK) ON a.Articulo = m.Articulo) AND @Transforma = 0
BEGIN
...
END
ELSE
IF NOT EXISTS(SELECT * FROM @Temp a JOIN ArtMaterial m WITH(NOLOCK) ON a.Articulo = m.Articulo) AND @Transforma = 1
BEGIN
INSERT INTO Inv (Empresa, Mov,  MovID, FechaEmision,   Concepto, Proyecto, Actividad,   UEN,   Moneda,   TipoCambio,   Usuario, Referencia, DocFuente, Observaciones, Estatus, Situacion, SituacionFecha, SituacionUsuario,   SituacionNota,   Prioridad, Directo, RenglonID,    Almacen,          AlmacenDestino,   FechaRequerida,   Condicion,   Vencimiento, FormaEnvio, OrigenTipo, Origen, OrigenID, Poliza,   PolizaID,   GenerarPoliza, ContID, Ejercicio, Periodo,   FechaRegistro,   FechaConclusion,   FechaCancelacion,   Peso,  Volumen, Paquetes, FechaEntrega, EmbarqueEstado,   Sucursal,   Importe,    VerDestino, EstaImpreso,    SubModulo,   SucursalOrigen, SucursalDestino)
VALUES      (   @Empresa, @MovTransferencia, NULL, @FechaEmision,  @Concepto, @Proyecto, @Actividad, @UEN, @Moneda, @TipoCambio, @Usuario,  @Referencia, @DocFuente, @Observaciones, @Estatus, @Situacion, @SituacionFecha, @SituacionUsuario, @SituacionNota, @Prioridad, @Directo, @RenglonID, @AlmacenMatPrima, @AlmacenDestino, @FechaRequerida, @Condicion, @Vencimiento, @FormaEnvio, @OrigenTipo, @Origen, @OrigenID, @Poliza, @PolizaID, @GenerarPoliza, @ContID, @Ejercicio, @Periodo, @FechaRegistro, @FechaConclusion, @FechaCancelacion, @Peso, @Volumen, @Paquetes, @FechaEntrega, @EmbarqueEstado, @Sucursal, @Importe,  @VerDestino, @EstaImpreso,  'INV', @SucursalOrigen, @SucursalDestino)
IF @@ERROR <> 0 SET @Ok = 1
...
IF NOT EXISTS(SELECT * FROM Art WITH(NOLOCK) WHERE Articulo = @Articulo ) SELECT @Ok = 10530
...
FROM Art WITH(NOLOCK) WHERE Articulo = @Articulo
...
SELECT @ArtTipo = Tipo, @Unidad = UnidadCompra FROM Art WITH(NOLOCK) WHERE Articulo = @Articulo
SELECT @UnidadMaterial = Unidad FROM Art WITH(NOLOCK) WHERE Articulo = @ArtMaterial
EXEC spRenglonTipo @ArtTipo, @Subcuenta, @RenglonTipo OUTPUT
SELECT @MovMoneda = Moneda, @MovTipoCambio = TipoCambio, @MovZonaImpuesto = ZonaImpuesto, @Proveedor= Proveedor FROM Compra WITH(NOLOCK) WHERE ID = @IDGenerar
...
IF EXISTS(SELECT * FROM Compra c WITH(NOLOCK) JOIN CompraD d WITH(NOLOCK) ON c.ID = d.ID JOIN MovTipo mt WITH(NOLOCK) ON mt.Mov = c.Mov AND mt.Modulo = 'COMS' WHERE mt.Clave = 'COMS.C' AND c.Estatus = 'CONFIRMAR' AND c.Proveedor = @Proveedor AND d.Articulo = @Articulo)
BEGIN
SELECT TOP 1 @Costo = (d.Costo/d.Cantidad) FROM Compra c WITH(NOLOCK) JOIN CompraD d WITH(NOLOCK) ON c.ID = d.ID JOIN MovTipo mt WITH(NOLOCK) ON mt.Mov = c.Mov AND mt.Modulo = 'COMS' WHERE mt.Clave = 'COMS.C' AND c.Estatus = 'CONFIRMAR' AND c.Proveedor = @Proveedor AND d.Articulo = @Articulo ORDER BY c.FechaEmision DESC
END
...
IF @@ERROR <> 0 SET @Ok = 1
SELECT @RenglonInv = MAX(Renglon) FROM InvD WITH(NOLOCK) WHERE ID = @IDGenerarInv
...
IF @Ok IS NULL
UPDATE InvD WITH(ROWLOCK) SET Posicion = @DefPosicionSurtido, PosicionDestino = @DefPosicionRecibo, Almacen = @AlmacenMatPrima
...
END
IF EXISTS (SELECT * FROM CompraD WITH(NOLOCK) WHERE ID = @IDGenerar AND NULLIF(Costo,0) IS NULL)
SELECT @Ok = 20101
BEGIN TRANSACTION
