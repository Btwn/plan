...
END
SELECT @MovID = t.MovID,@Mov=t.Mov FROM TMA t WITH (NOLOCK) JOIN TMAD d WITH (NOLOCK)  ON t.ID = d.ID WHERE t.Empresa = @Empresa AND t.Sucursal = @Sucursal AND d.Tarima = @Tarima
...
IF NOT EXISTS(SELECT * FROM TMA t WITH (NOLOCK) JOIN TMAD d WITH (NOLOCK) ON t.ID = d. ID  WHERE  d.Tarima = @Tarima  AND t.Empresa = @Empresa AND t.Sucursal = @Sucursal AND t.Mov=@Mov AND t.MovID= @MovID) SELECT @Ok = 13110
IF NOT EXISTS(SELECT * FROM TMA t WITH (NOLOCK) JOIN TMAD d WITH (NOLOCK) ON t.ID = d. ID  WHERE  d.Tarima = @Tarima AND d.PosicionDestino = @PosicionDestino AND t.Empresa = @Empresa AND t.Sucursal = @Sucursal AND t.Mov=@Mov AND t.MovID= @MovID) SELECT @Ok = 13035
END
IF @Ok IS NULL
BEGIN
SELECT @Tipo = Tipo FROM AlmPos WITH (NOLOCK) WHERE Posicion = @PosicionDestino
...
SELECT @Modificar = ModificarPosicionSugeridaWMS FROM Usuario WITH (NOLOCK) WHERE Usuario = @Usuario
UPDATE TMAD WITH (ROWLOCK) 
SET CantidadA = 1
WHERE Tarima = @Tarima  AND ID = @ID2
IF @Ok IS NULL AND @Estatus = 'PENDIENTE' AND @MovTipo = 'TMA.TSUR'
BEGIN
SELECT @Movimiento = TMASurtido FROM EmpresaCfgMovWMS WITH (NOLOCK)  WHERE Empresa = @Empresa
...
WHERE ID = @ID
SELECT @Mov2 = Mov , @MovID2 = MovID  FROM TMA WITH (NOLOCK) WHERE ID = @IDGenerar
SELECT @PosicionDGenerar = PosicionDestino  FROM TMAD WITH (NOLOCK) WHERE ID = @IDGenerar
SELECT @Descripcion = Descripcion FROM AlmPos WITH (NOLOCK) WHERE Posicion = @PosicionDGenerar
END
IF @Ok IS NULL
BEGIN
COMMIT TRANSACTION
