...
SELECT @Timbrado = Timbrado FROM CFDINominaRecibo WITH (NOLOCK) WHERE ID = @ID AND Personal = @Personal
SELECT @NoTimbrado = MAX(NoTimbrado) FROM CFDNominaCancelado WITH (NOLOCK) WHERE Modulo = 'NOM' AND ModuloID = @ID AND Personal = @Personal
...
IF @Ok IS NULL
UPDATE CFDINominaRecibo WITH (ROWLOCK) SET Documento = @XML WHERE ID = @ID AND Personal = @Personal
...
BEGIN
EXEC spPNetEnviarCorreoPersonal @Personal, 'Generar N�mina', @ParamPeriodoNomina
END
END
END
IF @Ok IS NOT NULL
UPDATE CFDINominaRecibo WITH (ROWLOCK)
SET Ok = @Ok,
OkRef = @OkRef
WHERE ID = @ID
...
BEGIN
UPDATE CFDINominaRecibo WITH (ROWLOCK) SET Timbrado = 'Timbrado', Ok = NULL, OkRef = NULL WHERE ID = @ID AND Personal = @Personal
UPDATE CFDNomina WITH (ROWLOCK) SET Timbrado = 1 WHERE Modulo = 'NOM' AND ModuloID = @ID AND Personal = @Personal
END
END
END
RETURN
