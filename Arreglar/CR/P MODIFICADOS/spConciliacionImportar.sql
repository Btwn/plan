...
GO
ALTER PROCEDURE spConciliacionImportar
@Estacion		Int,
@ID			int
...
@FechaD		datetime,
@FechaA		datetime
SELECT @Empresa = Empresa, @CtaDinero = CtaDinero FROM Conciliacion WITH (NOLOCK)  WHERE ID = @ID
SELECT @Moneda = Moneda, @NumeroCta = NumeroCta, @Institucion = Institucion FROM CtaDinero  WITH (NOLOCK) WHERE CtaDinero = @CtaDinero
SELECT @Layout = UPPER(Layout) FROM InstitucionFin WITH (NOLOCK)  WHERE Institucion = @Institucion
...
WHERE Concepto IN (SELECT ConceptoBanco FROM InstitucionFinConcepto WITH (NOLOCK) WHERE Institucion = @Institucion AND TipoMovimiento = 'N/A')
