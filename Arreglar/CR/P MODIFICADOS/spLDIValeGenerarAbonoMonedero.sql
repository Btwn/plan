...
@Monedero           varchar(50),
@Importe            float
SELECT @Articulo = Articulo FROM Vale WITH (NOLOCK)   WHERE ID = @ID
SELECT TOP 1 @Monedero = Serie, @Importe = Importe FROM ValeD  WITH (NOLOCK)  WHERE ID = @ID
SELECT @Servicio = 'MON ABONO'
SELECT @ServicioInverso = ServicioInverso FROM LDIServicio WITH (NOLOCK)   WHERE Servicio = @Servicio
SELECT @ServicioInversoInverso = ServicioInverso FROM LDIServicio WITH (NOLOCK)   WHERE Servicio = @ServicioInverso
IF @Importe <0.0
