...
SELECT @ContabilidadParalela = ISNULL(ContabilidadParalela, 0) FROM Version WITH(NOLOCK)
SELECT @SPID = @@SPID
...
@OkTipo 	= NULL,
@VolverAfectar = 0
SELECT @Empresa = Empresa, @Sucursal = Sucursal FROM Inv WITH(NOLOCK) WHERE ID = @ID
...
BEGIN
SELECT @FechaRegistro = Fecha FROM CFD WITH(NOLOCK) WHERE Modulo = @Modulo AND ModuloID = @ID
...
END
IF @Accion = 'GENERAR'
IF NOT EXISTS(SELECT * FROM MovTipo WITH(NOLOCK) WHERE Modulo = @Modulo AND Mov = @GenerarMov)
...
IF @Ok IS NULL AND (SELECT IntelMESInterfase FROM EmpresaCfg WITH(NOLOCK) WHERE Empresa = @Empresa) = 1
...
BEGIN
SELECT @Empresa = Empresa FROM Mov WITH(NOLOCK) WHERE Modulo = @Modulo AND ID = @ID
SELECT @Notificacion = ISNULL(Notificacion,0) FROM EmpresaGral WITH(NOLOCK) WHERE Empresa = @Empresa
IF @Notificacion = 1
BEGIN
