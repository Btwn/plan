...
FROM POSCfg WITH(NOLOCK) WHERE Empresa = @Empresa
...
IF @Accion <> 'CANCELAR'
BEGIN
IF NOT EXISTS(SELECT * FROM Venta v WITH(NOLOCK), VentaOrigen o WITH(NOLOCK) WHERE v.ID = o.OrigenID AND o.ID = @ID)
...
FROM Venta v WITH(NOLOCK), VentaD d WITH(NOLOCK), VentaOrigen o WITH(NOLOCK)
...
IF (SELECT ROUND(SUM(Cantidad), 0) FROM @ChecarNotas) <> 0 SELECT @Ok = 20380
END
IF @Ok = 20380 AND EXISTS(SELECT * FROM Venta v WITH(NOLOCK), VentaOrigen o WITH(NOLOCK), MovTipo mt WITH(NOLOCK) WHERE v.ID = o.OrigenID AND o.ID = @ID AND mt.Modulo = 'VTAS' AND mt.Mov = v.Mov AND mt.Clave = 'VTAS.NO')
SELECT @Ok = NULL
END
END
IF @Ok IS NULL
...
FROM Venta v WITH(NOLOCK), VentaOrigen o WITH(NOLOCK), MovTipo mt WITH(NOLOCK)
...
IF @Accion <> 'CANCELAR'
BEGIN
UPDATE VentaD WITH(ROWLOCK) SET ProcesadoID = vpt.ID
FROM  VentaProcesarNotas vpt JOIN VentaD d WITH(NOLOCK) ON d.ID = vpt.IDOrigen AND d.Renglon = vpt.Renglon AND d.RenglonSub = vpt.RenglonSub
WHERE  vpt.IDOrigen = @IDNota AND d.ID = @IDNota AND vpt.ID = @ID
IF EXISTS (SELECT * FROM VentaD WITH(NOLOCK) WHERE ID = @IDNota AND  ProcesadoID  IS NULL)
...
IF @Accion = 'CANCELAR'
BEGIN
UPDATE VentaD WITH(ROWLOCK) SET ProcesadoID = NULL
