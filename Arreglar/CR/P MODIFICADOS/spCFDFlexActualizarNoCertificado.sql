...
IF @Ok IS NULL
IF @Tipo = 'Sucursal'
UPDATE Sucursal WITH (ROWLOCK) SET NoCertificado = @NoCertificado WHERE Sucursal = @Sucursal
ELSE
UPDATE EmpresaCFD WITH (ROWLOCK) SET NoCertificado = @NoCertificado WHERE Empresa = @Empresa
END
END
