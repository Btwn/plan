...
IF NOT EXISTS (SELECT * FROM POSSerieTarjetaMovM WITH (NOLOCK) WHERE Mov = @Mov AND MovID = @MovID AND Posicion = @Posicion AND Empresa = @Empresa AND Serie = @Serie AND Sucursal = @Sucursal AND Referencia = 'APROBADA')
