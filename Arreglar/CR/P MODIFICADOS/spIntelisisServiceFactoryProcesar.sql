...
SELECT @EstatusD = Estatus FROM IntelisisService WITH(NOLOCK) WHERE ID = @RID
...
UPDATE IntelisisServiceFactory  WITH(ROWLOCK) SET RID = @RID , Estatus = @EstatusD,Modulo = @Modulo, ModuloID = @ModuloID
WHERE ID = @ID
END
END
