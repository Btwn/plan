...
/DANumeroDocumentos,
@DAPeriodo = DAPeriodo
FROM  POSL WITH (NOLOCK) JOIN POSLVenta WITH (NOLOCK) ON POSL.ID=POSLVenta.ID
JOIN Cte WITH (NOLOCK) ON PosL.Cliente=Cte.Cliente
JOIN Condicion WITH (NOLOCK) ON POSL.Condicion=Condicion.Condicion
WHERE POSL.ID=@ID
...
SELECT @Cadena=@Cadena+('DOC '+ Convert(varchar,ROW_NUMBER() OVER(ORDER BY Fecha ASC)) +' Vencimiento :' + CONVERT(VARCHAR,Fecha,103)+  ' Importe : ' + dbo.fnFormatoMoneda(@Importe,'FREY'))+'<BR>' FROM ProrrateoFecha WITH (NOLOCK) WHERE Estacion = @@spid
...
FROM ProrrateoFecha WITH (NOLOCK)
WHERE Estacion = @@spid
GROUP BY Fecha
END
RETURN
