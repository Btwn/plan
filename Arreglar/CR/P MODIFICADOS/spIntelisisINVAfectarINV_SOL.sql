...
SELECT @ID3 = f.DID
FROM MovFlujo f
 WITH(NOLOCK) JOIN MovTipo t
 WITH(NOLOCK) ON f.DMov = t.Mov AND f.DModulo = t.Modulo
...
IF @OK is NULL
BEGIN
SELECT @Partidas = count(ID) FROM InvD WITH(NOLOCK) WHERE ID = @ID3
IF @Partidas = 0
SET @OK = 60010 
END
IF @Ok IS NULL
BEGIN
SELECT @Mov2 = InvEntarimado FROM EmpresaCfgMov WITH(NOLOCK) WHERE Empresa = @Empresa
...
UPDATE Inv  WITH(ROWLOCK) SET Referencia = @Referencia
Where ID = @IDGenerar
IF @@ERROR <> 0 SET @Ok = 1
