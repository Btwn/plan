...
SELECT @Mensaje = EnviarMensajeNomina,@EnviarXML=EnviarXMLNomina,@EnviarPDF=EnviarPDFNomina,@Enviar=EnviarNomina,@RutaIntelisisCFDI = RutaIntelisisCFDI  FROM EmpresaCFD WITH(NOLOCK) WHERE Empresa = @Empresa
...
SELECT @Usuario=DireccionCorreo, @Pasword= ContrasenaCorreo,@Smtp=SmtpCorreo,@PuertoCorreo=PuertoCorreo FROM EmpresaCFDNomina WITH (NOLOCK) where Empresa=@Empresa
...
UPDATE CFDINominaRecibo WITH (ROWLOCK) SET EnviarMailSql= CASE WHEN  @Estatus='Envio Exitoso' THEN 'Enviado'   ELSE 'Error en el envio' END WHERE RID =@ID
SELECT ''
