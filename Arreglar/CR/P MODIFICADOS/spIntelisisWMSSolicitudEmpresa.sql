...
WHERE Campo = 'Usuario'
IF NOT EXISTS (SELECT * FROM UsuarioD WITH(NOLOCK) WHERE Usuario = @Usuario)
BEGIN
SELECT @Texto = (SELECT Empresa FROM Empresa WITH(NOLOCK) WHERE Estatus = 'ALTA'
FOR XML AUTO)
END
IF EXISTS (SELECT * FROM UsuarioD WITH(NOLOCK) WHERE Usuario = @Usuario)
BEGIN
SELECT @Texto = (SELECT u.Empresa
FROM UsuarioD u
 WITH(NOLOCK) JOIN Empresa e  WITH(NOLOCK) ON (u.Empresa = e.Empresa)
