...
@OrigenID			varchar(20),
@StrSQL             nvarchar(max)
SELECT @GenerarMovTipo = Clave FROM MovTipo WITH(NOLOCK) WHERE Modulo = @Modulo AND Mov = @GenerarMov
DELETE ListaIDOk WHERE Estacion = @Estacion
SELECT v.OrigenTipo, v.Origen, v.OrigenID
INTO #Origen
FROM ListaID l, Compra v, MovTipo mt
WITH(NOLOCK) WHERE l.Estacion = @Estacion AND v.ID = l.ID AND v.Empresa = @Empresa AND mt.Modulo = @Modulo AND mt.Mov = v.Mov
...
UPDATE Compra  WITH(ROWLOCK) SET OrigenTipo = @OrigenTipo, Origen = @Origen, OrigenID = @OrigenID WHERE ID = @IDGenerar
IF @Ok = 80030
BEGIN
