...
IF @ACCION ='AGREGAR'
BEGIN
IF NOT EXISTS(SELECT ID FROM WebPaginaSuscripcion WITH(NOLOCK) WHERE Pagina = @Pagina  AND Usuario = @Usuario AND eMail = @Correo AND Estatus='ALTA')
BEGIN
...
IF @ACCION ='CANCELAR'
BEGIN
IF EXISTS(SELECT ID FROM WebPaginaSuscripcion WITH(NOLOCK) WHERE Pagina = @Pagina  AND Usuario = @Usuario AND eMail = @Correo AND Estatus='ALTA')
BEGIN
UPDATE WebPaginaSuscripcion WITH(ROWLOCK) SET Estatus = 'BAJA' WHERE Pagina = @Pagina  AND Usuario = @Usuario AND eMail = @Correo
SELECT '0000' 
END
ELSE
SELECT '2222' 
...
select eMail FROM WebPaginaSuscripcion WITH(NOLOCK) WHERE Pagina = @Pagina AND Estatus = 'ALTA'
open CURSORITO
