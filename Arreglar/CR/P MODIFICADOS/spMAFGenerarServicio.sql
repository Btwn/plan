...
IF @ID IS NOT NULL
SELECT @AFArticulo = AFArticulo, @AFSerie = AFSerie, @Origen = Mov, @OrigenID = MovID FROM Gestion WITH(NOLOCK) WHERE ID = @ID
SELECT
...
SELECT @Tipo = Tipo, @AF = ID FROM ActivoF WITH(NOLOCK) WHERE Articulo = @AFArticulo AND Serie = @AFSerie
SELECT
...
FROM ActivoFTipoServicio WITH(NOLOCK)
WHERE Tipo = @Tipo
AND Servicio = @Servicio
SELECT @Moneda = NULLIF(DefMoneda,'') FROM Cte WITH(NOLOCK) WHERE Cliente = @MAFServicioCliente
...
FROM ServicioTipoPlantilla stp WITH(NOLOCK) JOIN Art a WITH(NOLOCK)
...
BEGIN
SELECT @MovIDGenerar = MovID FROM Venta WITH(NOLOCK) WHERE ID = @IDGenerar
...
ON mt.Mov = mf.DMov AND mt.Modulo = mf.DModulo JOIN Venta v WITH(NOLOCK)
ON v.ID = mf.DID
WHERE mf.OID = @ID
AND mf.OModulo = 'GES'
AND mt.Modulo = 'VTAS'
