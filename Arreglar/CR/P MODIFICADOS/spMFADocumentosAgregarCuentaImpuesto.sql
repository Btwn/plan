...
UPDATE MFAPolizaImpuesto WITH (ROWLOCK)
...
JOIN MFAPolizaImpuesto mpc  WITH (NOLOCK)ON mpc.ModuloID = CONVERT(int,ld.origen_id) AND mpc.Modulo = ld.origen_modulo AND mpc.ImpuestoTipo = 'ISR'
UPDATE layout_documentos WITH (ROWLOCK)
SET iva_cuenta_contable = mpc.Cuenta
FROM layout_documentos ld  
JOIN MFAPolizaImpuesto mpc  WITH (NOLOCK)ON mpc.ModuloID = CONVERT(int,ld.origen_id) AND mpc.Modulo = ld.origen_modulo AND mpc.ImpuestoTipo = 'IVA'
UPDATE layout_documentos WITH (ROWLOCK)
SET retencion_iva_cuenta_contable = mpc.Cuenta
FROM layout_documentos ld  
JOIN MFAPolizaImpuesto mpc  WITH (NOLOCK)ON mpc.ModuloID = CONVERT(int,ld.origen_id) AND mpc.Modulo = ld.origen_modulo AND mpc.ImpuestoTipo = 'Retencion IVA'
UPDATE layout_documentos WITH (ROWLOCK)
SET ieps_cuenta_contable = mpc.Cuenta
FROM layout_documentos ld  
JOIN MFAPolizaImpuesto mpc WITH (NOLOCK) ON mpc.ModuloID = CONVERT(int,ld.origen_id) AND mpc.Modulo = ld.origen_modulo AND mpc.ImpuestoTipo = 'IEPS'
UPDATE layout_documentos WITH (ROWLOCK)
SET isan_cuenta_contable = mpc.Cuenta
FROM layout_documentos ld  
JOIN MFAPolizaImpuesto mpc WITH (NOLOCK) ON mpc.ModuloID = CONVERT(int,ld.origen_id) AND mpc.Modulo = ld.origen_modulo AND mpc.ImpuestoTipo = 'ISAN'
UPDATE layout_documentos WITH (ROWLOCK)
