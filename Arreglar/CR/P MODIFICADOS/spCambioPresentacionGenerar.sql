...
FROM EmpresaCfgMov cfg WITH (NOLOCK)
...
IF @Ok IS NULL
BEGIN
SELECT @DestinoTipo = DestinoTipo, @Destino = Destino, @DestinoID = DestinoID FROM InvD WITH (NOLOCK) WHERE ID = @ID AND Articulo = @NuevaPresentacion
IF @DestinoTipo = 'VTAS'
BEGIN
EXEC spMovEnID @DestinoTipo, @Empresa, @Destino, @DestinoID, @IDDestino OUTPUT, @DestinoMoneda OUTPUT, @Ok OUTPUT
UPDATE VentaD WITH (ROWLOCK) SET DescripcionExtra = @Articulo WHERE ID = @IDDestino AND Articulo = @NuevaPresentacion AND NULLIF(CantidadOrdenada, 0) IS NOT NULL
END
END
IF @Ok IS NULL
