...
SELECT @RFCEmisor = RFC FROM Empresa WITH (NOLOCK) WHERE Empresa = @Empresa
...
BEGIN
EXEC (@SQL)
UPDATE EmpresaCFD WITH (ROWLOCK) SET ContatoFirmadoAspel = (SELECT ArchivoXML FROM #CFDFlexLeerArchivo) WHERE Empresa = @Empresa
END
ELSE
BEGIN
