...
SELECT @RetencionAlPago = RetencionAlPago FROM EmpresaCfg2 WITH (NOLOCK) WHERE Empresa = @Empresa
SELECT @Concepto = Cxp.Concepto
FROM #Pagos p
JOIN Cxp WITH (NOLOCK) ON p.Empresa = Cxp.Empresa AND p.Aplica = Cxp.Mov AND p.AplicaID = Cxp.MovID
JOIN Prov WITH (NOLOCK) ON Cxp.Proveedor = Prov.Proveedor
LEFT OUTER JOIN Pais WITH (NOLOCK) ON Prov.Pais = Pais.Pais
...
FROM #Pagos p
JOIN Cxp WITH (NOLOCK) ON p.Empresa = Cxp.Empresa AND p.Aplica = Cxp.Mov AND p.AplicaID = Cxp.MovID 
LEFT OUTER JOIN Cxp O WITH (NOLOCK) ON Cxp.Empresa = O.Empresa AND Cxp.Origen = O.Mov AND Cxp.OrigenID = O.MovID
LEFT OUTER JOIN CxpD WITH (NOLOCK) ON O.ID = CxpD.ID
LEFT OUTER JOIN Cxp oo WITH (NOLOCK) ON O.Origen = oo.Mov AND O.OrigenID = oo.MovID
JOIN Prov WITH (NOLOCK) ON Cxp.Proveedor = Prov.Proveedor
LEFT OUTER JOIN Pais WITH (NOLOCK) ON Prov.Pais = Pais.Pais
JOIN Version ver WITH (NOLOCK) ON 1 = 1
WHERE Cxp.Empresa = @Empresa
...
JOIN #Retenciones r WITH (NOLOCK) ON g.Mov = r.Origen AND g.MovID = r.OrigenID 
JOIN #Pagos p WITH (NOLOCK) ON p.Aplica = r.Mov AND p.AplicaID = r.MovID
JOIN Version ver WITH (NOLOCK) ON 1=1
JOIN Concepto WITH (NOLOCK) ON d.Concepto = Concepto.Concepto AND Concepto.Modulo IN('GAS', 'COMSG')
...
FROM Compra g WITH (NOLOCK)
JOIN CompraD d WITH (NOLOCK) ON g.ID = d.ID
JOIN CompraTCalc ctc WITH (NOLOCK) ON ctc.RenglonSub = d.RenglonSub AND ctc.Renglon = d.Renglon AND ctc.ID = d.ID
JOIN #Retenciones r WITH (NOLOCK) ON g.Mov = r.Origen AND g.MovID = r.OrigenID 
JOIN #Pagos p WITH (NOLOCK) ON p.Aplica = r.Mov AND p.AplicaID = r.MovID
JOIN Version ver WITH (NOLOCK) ON 1=1
