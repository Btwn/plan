...
IF @Modulo = 'NOM' SELECT @Empresa = Empresa, @Sucursal = Sucursal, @Mov = RTRIM(Mov), @MovID = RTRIM(MovID), @FechaEmision = FechaEmision, @FechaRegistro = FechaRegistro FROM Nomina WITH (NOLOCK) WHERE ID = @ID
SELECT @Reporte = NULLIF(Reporte,'') FROM CFDINominaMov WITH (NOLOCK) WHERE Mov = @Mov
...
SELECT @EnviarPara = email FROM Personal WITH (NOLOCK) WHERE Personal = @Personal
RETURN
END
