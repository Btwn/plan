...
FROM Cte c WITH(NOLOCK), Mon m WITH(NOLOCK)
...
LEFT OUTER JOIN AnexoCta WITH(NOLOCK) ON Art.Articulo = AnexoCta.Cuenta AND (AnexoCta.Nombre = 'Imagen' OR AnexoCta.Nombre IS NULL) AND (AnexoCta.Tipo = 'Imagen' OR AnexoCta.Tipo IS NULL)
WHERE Art.Rama = @Rama
