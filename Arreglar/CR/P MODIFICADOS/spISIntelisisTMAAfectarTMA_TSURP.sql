...
SELECT @Sucursal = Sucursal FROM Sucursal  WITH (NOLOCK) WHERE Nombre = @RefSucursalDes
SELECT @Origen = TMAOrdenSurtidoPCK FROM EmpresaCfgMovWMS  WITH (NOLOCK)WHERE Empresa = @Empresa
...
JOIN MovTipo mt WITH (NOLOCK)  ON t.Mov = mt.Mov AND mt.Modulo='TMA'
