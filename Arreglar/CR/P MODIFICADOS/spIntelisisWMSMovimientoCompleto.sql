...
IF @Modulo = 'TMA' SELECT @ModuloID = ID FROM TMA WITH(NOLOCK) WHERE Empresa = @Empresa AND Mov = @Mov AND MovID = @MovID
...
SELECT ISNULL(SUM(CantidadPendiente),0), 0 FROM TMAD WITH(NOLOCK) WHERE ID = @ModuloID
