...
@Notificacion				bit, 
@Corte						bit
SELECT @EsEcuador = ISNULL(EsEcuador,0) FROM Empresa WITH (NOLOCK) WHERE Empresa = @Empresa
...
END
IF EXISTS(SELECT * FROM CtaDinero c WITH (NOLOCK), CtaDineroCajero j WITH (NOLOCK) WHERE c.CtaDinero = j.CtaDinero AND c.Sucursal = @Sucursal AND j.Cajero IS NOT NULL) AND @Ok IS NULL
SELECT @Ok = 22010
IF EXISTS(SELECT * FROM Venta WITH (NOLOCK) WHERE Estatus = 'PROCESAR' AND Sucursal = @Sucursal) AND @Ok IS NULL
SELECT @Ok = 22020
IF @Ok IS NULL
...
SELECT @FechaTrabajo = FechaTrabajo FROM FechaTrabajo WITH (NOLOCK) WHERE Empresa = @Empresa AND Sucursal = @Sucursal
...
IF @Ok IS NULL
BEGIN
UPDATE FechaTrabajo WITH (ROWLOCK)
