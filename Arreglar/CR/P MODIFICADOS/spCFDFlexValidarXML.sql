...
@Clave				varchar(50)
SELECT @Clave = XSD FROM MovTipoCFDFlex WITH (NOLOCK) WHERE Modulo = @Modulo AND Mov = @Mov AND ISNULL(NULLIF(ISNULL(NULLIF(Contacto,''),'(Todos)'),'(Todos)'),@Contacto) = @Contacto
SET @IDSQL = @Clave
