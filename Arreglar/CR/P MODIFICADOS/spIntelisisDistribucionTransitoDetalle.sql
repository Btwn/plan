...
GO
ALTER PROCEDURE spIntelisisDistribucionTransitoDetalle
@ID                                int,
@iSolicitud                        int,
@Version                           float,
...
ISNULL(Factor,0.0)
FROM InvD
WITH(NOLOCK) WHERE ID = @TransitoID
UPDATE @Tabla
...
DECLARE TransitoDetalle_cursor CURSOR FOR SELECT IDR, Articulo, SubCuenta, Unidad FROM @Tabla
OPEN TransitoDetalle_cursor
FETCH NEXT FROM TransitoDetalle_cursor INTO @IDR, @Articulo, @Subcuenta, @Unidad
...
FETCH NEXT FROM TransitoDetalle_cursor INTO @IDR, @Articulo, @Subcuenta, @Unidad
END
CLOSE TransitoDetalle_cursor
DEALLOCATE TransitoDetalle_cursor
