...
IF EXISTS(SELECT * FROM EmpresaValidarFechaAplicacion WITH(NOLOCK) WHERE Empresa = @Empresa AND Modulo = @Modulo AND Mov = @Mov AND AplicaModulo = @AplicaModulo AND AplicaMov = @Aplica)
BEGIN
