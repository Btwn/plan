...
WITH(NOLOCK) WHERE c.Estatus = 'CONCLUIDO' AND c.ComisionCorte IS NULL AND c.FechaConclusion <= @FechaCorte
...
IF @Ok IS NULL
UPDATE Cxc  WITH(ROWLOCK) SET ComisionCorte = @FechaEmision WHERE CURRENT OF crComisionLote
END
FETCH NEXT FROM crComisionLote  INTO @ID, @Mov, @MovID, @MovTipo, @Moneda, @TipoCambio, @Agente, @Comision, @Observaciones
END  
