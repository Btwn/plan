...
IF EXISTS(SELECT * FROM Venta pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'VTAS' AND mt.Clave IN ('VTAS.F', 'VTAS.N','VTA.FM') WHERE LTRIM(RTRIM(pl.Mov)) + ' ' + LTRIM(RTRIM(pl.MovId)) = @ReferenciaOrigen AND pl.Importe >= 0)
SELECT @IDPOS = ReferenciaOrdenCompra FROM Venta pl WITH (NOLOCK) INNER JOIN MovTipo mt WITH (NOLOCK) ON pl.Mov = mt.Mov AND mt.Modulo = 'VTAS' AND mt.Clave IN ('VTAS.F', 'VTAS.N','VTA.FM') WHERE LTRIM(RTRIM(pl.Mov)) + ' ' + LTRIM(RTRIM(pl.MovId)) = @ReferenciaOrigen AND pl.Importe >= 0
IF EXISTS(SELECT * FROM POSLCB cb WITH (NOLOCK) JOIN POSL pl WITH (NOLOCK) ON cb.ID = pl.ID WHERE cb.IDCB = @ReferenciaOrigen AND pl.Importe >= 0) AND @IDPOS IS NULL
SELECT @IDPOS = cb.ID FROM  POSLCB cb WITH (NOLOCK) JOIN POSL pl WITH (NOLOCK) ON cb.ID = pl.ID WHERE cb.IDCB = @ReferenciaOrigen AND pl.Importe >= 0
...
LEFT JOIN SerieLoteMov SerieLoteMov WITH (NOLOCK) ON SerieLoteMov.ID = Venta.ID AND SerieLoteMov.Empresa = Venta.Empresa AND SerieLoteMov.Modulo = 'VTAS' AND  ISNULL(SerieLoteMov.SubCuenta, '') = ISNULL(VentaD.Subcuenta, '') AND SerieLoteMov.Articulo = VentaD.Articulo
LEFT JOIN Oferta Oferta WITH (NOLOCK) ON VentaD.OfertaID = Oferta.ID
LEFT JOIN OfertaTipo OfertaTipo WITH (NOLOCK) ON OfertaTipo.Tipo = Oferta.Tipo
