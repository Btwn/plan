...
SELECT @Saldo = ISNULL(Saldo,0.00) FROM POSSaldoPMon WITH (NOLOCK) WHERE Empresa = @Empresa AND Cuenta = @Monedero AND Moneda = @Moneda
...
FROM POSSerieTarjetaMovMTemp WITH (NOLOCK)
