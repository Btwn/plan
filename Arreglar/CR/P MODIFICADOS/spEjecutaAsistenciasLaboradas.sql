...
FROM Asiste A
 WITH(NOLOCK) JOIN AsisteD B
 WITH(NOLOCK) ON A.ID = B.ID
JOIN MovTipo C
 WITH(NOLOCK) ON A.Mov = C.Mov
...
SELECT @Jornada = Jornada FROM Personal WITH(NOLOCK) WHERE Personal = @cPersonal
...
SELECT @Fecha = Fecha FROM ASISTED WITH(NOLOCK) WHERE ID = @rID AND Personal = @cPersonal AND Fecha = @cDia
...
ELSE
BEGIN
SELECT @TipoMov = Tipo FROM Asisted WITH(NOLOCK) WHERE Id = @rID AND Personal = @cPersonal AND Fecha = @cDia
...
FROM Asiste A
LEFT JOIN MovTipo MT  WITH(NOLOCK) ON MT.Mov = A.Mov
WHERE A.ID = @rID
AND A.Empresa = @Empresa
