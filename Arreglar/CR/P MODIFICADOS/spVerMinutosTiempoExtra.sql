...
@cFechaA        datetime,
@IdDias			int
IF EXISTS(SELECT * FROM AuxAsistenciaHE WITH(NOLOCK) WHERE ID = @ID AND Estatus <> 'SINAFECTAR')
BEGIN
RETURN
END
IF @BorraTabla = 0
...
JOIN AsisteD B WITH(NOLOCK)
ON A.ID = B.ID
JOIN MovTipo C WITH(NOLOCK)
ON A.Mov = C.Mov
...
FROM VerJornadaTiempo WITH(NOLOCK)
WHERE Jornada = @Jornada
AND DATEPART(YEAR,@cDia) = Ano
AND DATEPART(MONTH,@cDia) = Mes
AND DATEPART(DAY,@cDia) = Dia
GROUP BY Ano, Mes, Dia
SELECT @MinutosAs = ISNULL(SUM(Cantidad),0) FROM ASISTED WITH(NOLOCK) WHERE ID = @cID AND Personal = @cPersonal AND Fecha = @cDia AND Tipo = 'Minutos Asistencia'
SELECT @MinutosAu = ISNULL(SUM(Cantidad),0) FROM ASISTED WITH(NOLOCK) WHERE ID = @cID AND Personal = @cPersonal AND Fecha = @cDia AND Tipo = 'Minutos Ausencia'
