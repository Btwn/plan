...
GO
ALTER PROCEDURE spInforTransferenciaVTAS_F
@ID                                        int,
@iSolicitud                           int,
@Version                              float,
...
WITH (ID varchar(255))
SELECT @Almacen = Almacen ,@Mov =Mov ,@Origen = Origen ,@MovIDP = OrigenID FROM Venta with(nolock) WHERE ID = @IDO
SELECT @Serie = SerieMES FROM MovTipo WITH(NOLOCK) WHERE Clave = 'VTAS.P' AND Modulo ='VTAS' AND Mov = @Origen
...
FROM @Tabla2 a LEFT OUTER JOIN Art b  WITH(NOLOCK) ON a.CodigoArticulo = b.Articulo LEFT OUTER JOIN Alm c  WITH(NOLOCK) ON c.Almacen= b.INFORAlmacenProd
