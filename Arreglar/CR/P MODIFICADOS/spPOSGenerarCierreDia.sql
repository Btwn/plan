...
FROM Venta v WITH(NOLOCK) JOIN POSCierreDia p WITH(NOLOCK) ON v.Sucursal = p.Sucursal AND v.FechaEmision = p.FechaEmision AND v.Empresa = p.Empresa
AND v.Almacen = p.Almacen AND v.Moneda = p.Moneda
WHERE p.ID IN (SELECT ID FROM @Tabla ) AND p.Sucursal = @Sucursal
IF EXISTS(SELECT * FROM ListaID WITH(NOLOCK) WHERE Estacion = @Estacion)
