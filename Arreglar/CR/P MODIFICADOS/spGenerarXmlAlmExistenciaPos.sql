...
FROM    ExistenciaAlternaPosicionSerieLote e  WITH(NOLOCK) JOIN AlmPos a  WITH(NOLOCK) ON e.Almacen = a.Almacen AND e.Posicion = a.Posicion
JOIN Art ar  WITH(NOLOCK) ON e.Articulo = ar.Articulo
WHERE e.Almacen = @Almacen
AND e.Empresa = @Empresa
AND e.Existencia <> 0.0
SELECT @Xml1 = (SELECT * FROM AlmPos  AlmacenPosicion with(nolock) WHERE AlmacenPosicion.Almacen = @Almacen FOR XML AUTO)
