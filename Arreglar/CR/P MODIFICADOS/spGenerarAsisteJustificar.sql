...
WITH(NOLOCK) WHERE Empresa = @Empresa AND Personal = @Personal AND Fecha = @Fecha AND Ausencia > 0 AND Permiso IS NOT NULL AND PermisoID IS NOT NULL
...
FROM PersonalAsisteDifMin
WITH(NOLOCK) WHERE Empresa = @Empresa AND Personal = @Personal AND Fecha = @Fecha AND Extra > 0 AND Permiso IS NOT NULL AND PermisoID IS NOT NULL AND FechaHoraD >= @FechaHoraD AND @FechaHoraA BETWEEN FechaHoraD AND FechaHoraA
GROUP BY PermisoConcepto, FechaHoraD
ORDER BY PermisoConcepto, FechaHoraD
OPEN crPermisos
FETCH NEXT FROM crPermisos  INTO @Concepto, @Permiso
