...
IF @Modulo = 'COMS'
BEGIN
SELECT @OMov = Mov, @MonedaD = Moneda, @TipoCambioD = TipoCambio, @Proveedor = Proveedor, @Referencia = @Referencia FROM Compra WITH(NOLOCK) WHERE ID = @OID
SELECT @DMov = Mov FROM Compra WITH(NOLOCK) WHERE ID = @DID
IF @OMov = @DMov
UPDATE CompraD WITH(ROWLOCK) SET Tarima = NULL WHERE ID = @DID
SELECT @SubClave = Subclave FROM MovTipo WITH(NOLOCK) WHERE Modulo = @Modulo AND Mov = @DMov
...
AplicaRenglon	float NULL)
IF (SELECT CompraConcentrarEntrada FROM EmpresaCfg WITH(NOLOCK) WHERE Empresa = @Empresa) = 1
INSERT
INTO #CompraAplica
...
FROM CompraD d WITH(NOLOCK), #CompraAplica a
WHERE d.ID = @DID
AND d.Almacen = a.Almacen
AND d.Aplica = a.Aplica
AND d.AplicaID = a.AplicaID
...
IF @Modulo = 'INV'
BEGIN
SELECT @OMov = Mov FROM Inv WITH(NOLOCK) WHERE ID = @OID
SELECT @DMov = Mov FROM Inv WITH(NOLOCK) WHERE ID = @DID
IF @OMov = @DMov
UPDATE InvD WITH(ROWLOCK) SET Tarima = NULL WHERE ID = @DID
IF @GenerarDirecto = 1 SELECT @OrigenMov = NULL, @OrigenMovID = NULL
...
BEGIN
DELETE FROM SerieLoteMov WITH(NOLOCK) WHERE Modulo = @Modulo AND ID = @DID
INSERT SerieLoteMov(Empresa, Modulo, ID, RenglonID, Articulo, SubCuenta, SerieLote, Cantidad, CantidadAlterna,
Propiedades, Ubicacion, Cliente, Localizacion, Sucursal, ArtCostoInv, Tarima, AsignacionUbicacion)
...
FROM InvD d WITH(NOLOCK), #InvAplica a
WHERE d.ID = @DID
AND ISNULL(d.ProdSerieLote, '') = ISNULL(a.ProdSerieLote, '')
AND ISNULL(d.Producto, '') = ISNULL(a.Producto, '')
AND ISNULL(d.SubProducto, '') = ISNULL(a.SubProducto, '')
...
IF @Modulo = 'TMA'
BEGIN
SELECT @OMov = Mov FROM WITH(NOLOCK) TMA WHERE ID = @OID
SELECT @DMov = Mov FROM WITH(NOLOCK) TMA WHERE ID = @DID
IF @OMov = @DMov
UPDATE TMAD WITH(ROWLOCK) SET Tarima = NULL WHERE ID = @DID
END
IF @Modulo = 'SAUX'
BEGIN
IF @GenerarDirecto = 1 SELECT @OrigenMov = NULL, @OrigenMovID = NULL
...
/*********************************************************     BUG 7029     *********************************************************************/
IF @Modulo = 'INV'
SELECT @SubClave = B.SubClave FROM Inv A WITH(NOLOCK) JOIN MovTipo B WITH(NOLOCK) ON B.Modulo = @Modulo AND A.Mov = B.Mov WHERE A.ID = @OID
IF @Modulo = 'INV' AND @MovTipo = 'INV.T' 
BEGIN
DECLARE crArticuloTarima CURSOR FOR
SELECT Articulo, isnull(Subcuenta,''), Tarima, Almacen, SUM(Cantidad), RenglonID  FROM InvD WITH(NOLOCK) WHERE ID = @OID GROUP BY Articulo, SubCuenta, Tarima, Almacen, RenglonID
...
BEGIN
DECLARE crArticuloTarima CURSOR FOR
SELECT Articulo, isnull(Subcuenta,''), Tarima, Almacen, SUM(Cantidad), RenglonID FROM VentaD WITH(NOLOCK) WHERE ID = @DID GROUP BY Articulo, SubCuenta , Tarima, Almacen, RenglonID
END
IF @Modulo = 'INV'
BEGIN
DELETE FROM SerieLoteMOV WHERE ID = @DID AND Modulo = @Modulo
DECLARE crArticuloTarima CURSOR FOR
SELECT Articulo, isnull(Subcuenta,''), Tarima, Almacen, SUM(Cantidad), RenglonID  FROM InvD WITH(NOLOCK) WHERE ID = @DID GROUP BY Articulo, SubCuenta, Tarima, Almacen, RenglonID
END
IF @Modulo = 'COMS'
DECLARE crArticuloTarima CURSOR FOR
SELECT Articulo, isnull(Subcuenta,''), Tarima, Almacen, SUM(Cantidad), RenglonID  FROM CompraD WITH(NOLOCK) WHERE ID = @DID GROUP BY Articulo, SubCuenta, Tarima, Almacen, RenglonID
OPEN crArticuloTarima
FETCH NEXT FROM crArticuloTarima INTO @Articulo, @Subcuenta, @Tarima, @Almacen, @CantidadP, @cRenglonID
WHILE @@FETCH_STATUS = 0 AND @CantidadP > 0
BEGIN
