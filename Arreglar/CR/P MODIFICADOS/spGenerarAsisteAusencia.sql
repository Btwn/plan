...
GO
ALTER PROCEDURE spGenerarAsisteAusencia
@Empresa		char(5),
...
IF NOT EXISTS(SELECT * FROM JornadaTiempo WITH(NOLOCK) WHERE Jornada = @Jornada AND Entrada > @FechaFinal)
BEGIN
SELECT @Ok = 55260
RETURN
END
...
FROM JornadaTiempo WITH(NOLOCK)
...
IF EXISTS(SELECT * FROM PersonalAsiste WITH(NOLOCK) WHERE Empresa = @Empresa AND Personal = @Personal/* NES Para correcci�n de entradas y salidas en diferentes dias AND Fecha = @FechaEntrada*/
AND DAY(Entrada) = DAY(@FechaEntrada) AND MONTH(Entrada) = MONTH(@FechaEntrada) AND YEAR(Entrada) = YEAR(@FechaEntrada))
BEGIN
SELECT @ID = NULL
SELECT @ID = MIN(ID)
FROM PersonalAsiste
WITH(NOLOCK) WHERE Empresa = @Empresa AND Personal = @Personal AND Fecha = @FechaEntrada AND ProcesarAusencia = 1 AND ID > @UltID
IF @ID IS NULL
BEGIN
...
END ELSE
BEGIN
IF NOT EXISTS(SELECT * FROM PersonalAsisteDifDia WITH(NOLOCK) WHERE Empresa = @Empresa AND Personal = @Personal AND Fecha = @FechaEntrada)
INSERT PersonalAsisteDifDia (Empresa, Personal, Fecha, Ausencia) VALUES (@Empresa, @Personal, @FechaEntrada, 1)
END
UPDATE PersonalAsiste  WITH(ROWLOCK) SET ProcesarAusencia = 0 WHERE ID = @ID
END
FETCH NEXT FROM crJornadaTiempo  INTO @Entrada, @Salida
END  
