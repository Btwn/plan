...
WHERE ID = @IDPOS
END
IF EXISTS(SELECT * FROM POSLVenta WITH (NOLOCK) WHERE ID = @IDDev AND ISNULL(EsMonedero,0) = 1)
BEGIN
SELECT @RenglonIDMonederoDev = RenglonID
FROM POSLVenta WITH (NOLOCK)
WHERE  ID = @IDDev AND ISNULL(EsMonedero,0) = 1
UPDATE POSLVenta WITH (ROWLOCK) SET Articulo = @ArticuloTarjetaServico, RenglonTipo = 'N'
WHERE  ID = @IDDev AND ISNULL(EsMonedero,0) = 1
DELETE POSLSerieLote WHERE ID = @IDDev AND RenglonID = @RenglonIDMonederoDev
END
SELECT @MovDev = Mov FROM POSL WITH (NOLOCK) WHERE ID = @IDDev
IF @Ok IS NULL
BEGIN
...
UPDATE
POSL WITH (ROWLOCK) SET MovID =  @MovIDDev,
...
WHERE ID = @IDPOS
END
IF EXISTS(SELECT * FROM POSLVenta WITH (NOLOCK) WHERE ID = @IDFact AND ISNULL(EsMonedero,0) = 1)
BEGIN
...
UPDATE
POSL WITH (ROWLOCK) SET MovID =  @MovIDFact,
Prefijo = ISNULL(Prefijo, @Prefijo),
Consecutivo = ISNULL(Consecutivo, @Consecutivo),
noAprobacion = ISNULL(noAprobacion, @NoAprobacion),
fechaAprobacion = ISNULL(fechaAprobacion, @fechaAprobacion),
...
FROM POSLVenta pmv WITH (NOLOCK) JOIN POSL p WITH (NOLOCK) ON pmv.ID = p.ID
WHERE pmv.ID = @IDFact
IF @@ERROR <> 0
SET @Ok = 1
...
END
IF @Ok IS NULL
UPDATE POSL WITH (ROWLOCK) SET Facturado = 1 WHERE ID = @IDPOS
IF @Ok IS NULL
BEGIN
COMMIT TRANSACTION
