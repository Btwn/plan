...
END
SELECT @MovTipo = Clave,@SubClave = SubClave FROM MovTipo WITH (NOLOCK) WHERE Mov = @Mov AND Modulo = 'TMA'
...
SELECT @Agente = DefAgente, @Almacen=DefAlmacen FROM Usuario WITH (NOLOCK) WHERE Usuario = @Usuario  
...
IF @Ok IS NULL
BEGIN
SELECT @ID2 = ID FROM TMA WITH (NOLOCK) WHERE Empresa = @Empresa AND Mov = @Mov AND MovID = @MovID
SELECT @Estatus = Estatus FROM TMA WITH (NOLOCK) WHERE ID = @ID2
SELECT @Procesado = ISNULL(Procesado,0),
@Renglon = Renglon
FROM TMAD WITH (NOLOCK)
WHERE ID = @ID2
AND Tarima = @Tarima
...
BEGIN
SELECT @Mov3 = TMASurtidoTransitoPCK FROM EmpresaCfgMovWMS WITH (NOLOCK) WHERE Empresa = @Empresa
