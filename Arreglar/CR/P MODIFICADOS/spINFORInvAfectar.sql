...
SELECT @IDP =ID
FROM Compra
WITH(NOLOCK) WHERE Mov = @MovO AND MovID = @MovIDO AND Empresa = @Empresa AND Sucursal = @Sucursal
SELECT @Aplica= Aplica ,@AplicaID = AplicaID
FROM VentaD
WITH(NOLOCK) WHERE ID = @ID
SELECT @IDV =ID
FROM Venta
WITH(NOLOCK) WHERE Mov = @Aplica AND MovID = @AplicaID AND Empresa = @Empresa AND Sucursal = @Sucursal
SELECT @MovIntelisisMES = MovIntelisisMES FROM Compra WITH(NOLOCK) WHERE ID = @IDP
SELECT @MovIntelisisMES1 = MovIntelisisMES FROM Compra WITH(NOLOCK) WHERE ID = @ID
...
IF    EXISTS(SELECT * FROM MapeoMovimientosIntelisisInfor WITH(NOLOCK) WHERE Movimiento = @Mov AND Modulo = @Modulo)
BEGIN
SET @Datos = NULL
IF @Modulo ='VTAS'
BEGIN
IF @MovTipo = 'VTAS.P'  AND @EstatusNuevo = 'PENDIENTE' AND @Accion IN('AFECTAR','RESERVARPARCIAL')
AND EXISTS(SELECT * FROM VentaD d  WITH(NOLOCK) JOIN Art a  WITH(NOLOCK) ON d.Articulo = a.Articulo JOIN Alm al  WITH(NOLOCK) ON a.INFORAlmacenProd = al.Almacen AND ISNULL(al.EsFactory,0) = 1 WHERE ISNULL(a.SeProduce,0) = 1 AND ISNULL(a.EsFactory,0) = 1 AND d.ID = @ID)
