...
SELECT @OrdenPCKTarima = TMAOrdenPCKTarima FROM EmpresaCfgMovWMS WITH(NOLOCK) WHERE Empresa = @Empresa
...
IF @Completo = 3
SELECT @Mov = TMASurtidoTransito FROM EmpresaCfgMovWMS WITH(NOLOCK) WHERE Empresa = @Empresa
...
JOIN ArtDisponibleTarima a  WITH(NOLOCK) ON d.Tarima = a.Tarima AND t.Empresa = a.Empresa AND a.Almacen = d.Almacen
...
JOIN MovTipo mt  WITH(NOLOCK) ON mt.Mov = t.Mov AND Modulo = 'TMA'
...
FROM CB
WITH(NOLOCK) WHERE Cuenta = @Articulo
...
SELECT @Tipo = Tipo
FROM AlmPos
WITH(NOLOCK) WHERE Posicion = @Posicion
IF @Ok IS NULL
