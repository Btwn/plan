...
BEGIN
IF EXISTS(SELECT 1 FROM Prospecto WITH(NOLOCK) WHERE Prospecto = ISNULL(@Prospecto,''))
UPDATE Prospecto WITH(ROWLOCK) SET
Nombre              = @Nombre,
Direccion           = @Direccion,
...
BEGIN
IF @ID IS NOT NULL AND EXISTS(SELECT 1 FROM Campana WITH(NOLOCK) WHERE ID = @ID AND Estatus IN ('SINAFECTAR', 'PENDIENTE'))
BEGIN
IF NOT EXISTS(SELECT 1 FROM CampanaD WITH(NOLOCK) WHERE ID = @ID AND ContactoTipo = 'Prospecto' AND Contacto = @Prospecto)
