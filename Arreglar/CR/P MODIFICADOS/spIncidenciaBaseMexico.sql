...
LEFT OUTER JOIN Jornada j  WITH(NOLOCK) ON j.Jornada = p.Jornada
LEFT OUTER JOIN PeriodoTipo pt  WITH(NOLOCK) ON pt.PeriodoTipo = p.PeriodoTipo
...
WITH(NOLOCK) WHERE PeriodoTipo = @PeriodoTipo AND Abierto = 1
SELECT @SMZ = SueldoMinimo FROM ZonaEconomica WITH(NOLOCK) WHERE Zona = @ZonaEconomica
SELECT @SMDF = SueldoMinimo FROM ZonaEconomica WITH(NOLOCK) WHERE Zona = 'A'
