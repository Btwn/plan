...
SELECT @Empresa = Empresa FROM Compra WITH(NOLOCK) WHERE ID = @ID
SELECT @ProdInterfazINFOR = ISNULL(ProdInterfazINFOR,0) FROM EmpresaGral WHERE Empresa = @Empresa
SELECT @MovClave = mt.Clave FROM Compra c  WITH(NOLOCK) JOIN MovTipo mt  WITH(NOLOCK) ON c.Mov = mt.Mov AND mt.Modulo = 'COMS' WHERE c.ID = @ID
SELECT @Origen = Origen, @OrigenID = OrigenID FROM Compra WHERE ID =  @ID
SELECT @OrigenTipo = OrigenTipo FROM Compra WITH(NOLOCK) WHERE Empresa = @Empresa AND Mov = @Origen AND MovID = @OrigenID
...
BEGIN
IF @OK IS NULL  AND EXISTS(SELECT * FROM Inv i  WITH(NOLOCK) JOIN InvD d  WITH(NOLOCK) ON d.ID = i.ID JOIN MovTipo m  WITH(NOLOCK) ON m.Mov = i.Mov AND m.Modulo = 'INV' JOIN Compra c  WITH(NOLOCK) ON c.ID = @ID JOIN CompraD cd  WITH(NOLOCK) ON c.ID = cd.ID AND cd.Articulo = d.Articulo AND ISNULL(cd.SubCuenta,'') = ISNULL(d.SubCuenta,'') AND ISNULL(cd.Almacen,c.Almacen) = ISNULL(d.Almacen,i.Almacen) WHERE m.Clave = 'INV.SOL' AND i.Estatus = 'PENDIENTE' AND i.OrigenTipo = 'I:MES' AND ISNULL(d.CantidadPendiente,0.0) > 0 )
BEGIN
DECLARE crArt CURSOR LOCAL FAST_FORWARD FOR
SELECT d.Articulo, d.Cantidad, d.SubCuenta, d.Unidad, ISNULL(d.Almacen,c.Almacen)
FROM Compra c  WITH(NOLOCK) JOIN CompraD d  WITH(NOLOCK) ON c.ID = d.ID
WHERE c.ID = @ID
OPEN crArt
...
BEGIN
UPDATE InvD  WITH(ROWLOCK) SET CantidadA = CantidadPendiente WHERE ID = @IDSol AND Renglon = @RenglonD
...
BEGIN
UPDATE InvD
 WITH(ROWLOCK) SET CantidadA = @CantidadReservar
