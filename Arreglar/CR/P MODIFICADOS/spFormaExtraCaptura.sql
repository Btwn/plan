...
LEFT OUTER JOIN FormaExtraD fv  WITH(NOLOCK) ON fv.FormaTipo = @FormaTipo AND fv.ID = @AplicaID AND fv.Campo = fc.Campo
...
CASE WHEN fc.TipoDato IN ('Fecha', 'Hora', 'Fecha/Hora') THEN CONVERT(datetime, fv.Valor, 126) END
FROM FormaExtraCampo fc
LEFT OUTER JOIN FormaExtraValor fv  WITH(NOLOCK) ON fv.FormaTipo = @FormaTipo AND fv.Aplica = @Aplica AND fv.AplicaClave = @AplicaClave AND fv.Campo = fc.Campo
WHERE fc.FormaTipo = @FormaTipo
END ELSE
BEGIN
IF @Campo IS NOT NULL
UPDATE tempFormaCaptura
 WITH(ROWLOCK) SET ValorTexto = @ValorTexto, ValorNumerico = @ValorNumerico, ValorFlotante = @ValorFlotante, ValorMonetario = @ValorMonetario, ValorLogico = @ValorLogico, ValorFechaHora = @ValorFechaHora, Tiempo = @Tiempo
...
FROM tempFormaCaptura WITH(NOLOCK)
...
BEGIN
SELECT @TipoDato = TipoDato FROM FormaExtraCampo WITH(NOLOCK) WHERE FormaTipo = @FormaTipo AND Campo = @Campo
SELECT @Valor = NULL
...
BEGIN
UPDATE FormaExtraD
 WITH(ROWLOCK) SET Valor = NULLIF(RTRIM(@Valor), ''),
...
END ELSE
BEGIN
SELECT @OrdenEspecial = OrdenEspecial FROM FormaTipo WITH(NOLOCK) WHERE FormaTipo = @FormaTipo
IF @OrdenEspecial = 1
BEGIN
