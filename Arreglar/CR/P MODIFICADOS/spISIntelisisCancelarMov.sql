...
SET @Cantidad=0
SELECT @Estacion = EstacionTrabajo ,@Usuario2 = Usuario FROM Acceso WITH(NOLOCK) WHERE ID = @IDAcceso
...
BEGIN
IF(SELECT Estatus FROM Compra WITH(NOLOCK) WHERE ID =  @IDO)='CONCLUIDO' SET @Ok=46090
IF EXISTS(SELECT * FROM CompraD WITH(NOLOCK) WHERE ID = @IDO AND  ISNULL(CantidadPendiente,0.0) <> ISNULL(Cantidad,0.0))
BEGIN
...
WHILE @@FETCH_STATUS = 0
BEGIN
IF NOT EXISTS(SELECT * FROM Inv WITH(NOLOCK) WHERE ID = @IDI AND ESTATUS  <> 'CANCELADO')SET @Ok =14055
...
SELECT @MovInv = Mov FROM Inv WITH(NOLOCK) WHERE ID = @IDI
SELECT @SubClave = SubClave FROM MovTipo WITH(NOLOCK) WHERE Mov = @MovInv AND Modulo = 'INV'
SELECT @CfgReservar = PedidosReservar FROM EmpresaCfg WITH(NOLOCK) WHERE Empresa = @Empresa
IF @Ok IS NULL AND @CfgReservar = 1 AND @SubClave NOT IN('INV.ER')
BEGIN
SELECT @Serie = Serie ,@PedidoMES = PedidoMES FROM Inv WITH(NOLOCK) WHERE ID = @IDI
SELECT @MovVenta = Mov FROM MovTipo WITH(NOLOCK) WHERE SerieMES = @Serie
SELECT @IDVenta = ID FROM Venta WITH(NOLOCK) WHERE Mov = @MovVenta AND MovID =@PedidoMES AND Empresa=@Empresa
SELECT @CantidadVenta=CantidadPendiente FROM VentaD WITH(NOLOCK) WHERE ID = @IDVenta AND Articulo = @Articulo AND SubCuenta = @SubCuenta
...
BEGIN
SELECT @Cantidad = Cantidad FROM InvD WITH(NOLOCK) WHERE ID = @IDI AND Articulo = @Articulo AND SubCuenta= @SubCuenta
