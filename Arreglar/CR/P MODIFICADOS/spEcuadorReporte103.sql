...
GO
ALTER PROCEDURE spEcuadorReporte103 @Estacion int
...
FROM TipoImpuesto  WITH(NOLOCK) JOIN CompraD
 WITH(NOLOCK) ON TipoImpuesto.TipoImpuesto = CompraD.TipoImpuesto3 JOIN Compra
 WITH(NOLOCK) ON CompraD.ID = Compra.ID JOIN MovTipo
 WITH(NOLOCK) ON Compra.Mov = MovTipo.Mov
...
FROM TipoImpuesto  WITH(NOLOCK) JOIN GastoD
 WITH(NOLOCK) ON TipoImpuesto.TipoImpuesto = GastoD.TipoImpuesto3 JOIN Gasto
 WITH(NOLOCK) ON GastoD.ID = Gasto.ID JOIN MovTipo
 WITH(NOLOCK) ON Gasto.Mov = MovTipo.Mov
