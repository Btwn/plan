...
FROM AuxAsistenciaHE WITH(NOLOCK)
WHERE Generar = 1
...
IF EXISTS (SELECT HorasExtrasCompletas FROM EMPRESACFG WITH(NOLOCK) WHERE EMPRESA = @Empresa AND HorasExtrasCompletas = 1)
BEGIN
...
UPDATE Nomina  WITH(ROWLOCK) SET Estatus = 'PROCESAR' WHERE ID = @IDNomina
SELECT @DMovID = MovID FROM NOMINA WITH(NOLOCK) WHERE ID = @IDNomina
...
CLOSE crHES2D7
DEALLOCATE crHES2D7
UPDATE AuxAsistenciaHE  WITH(ROWLOCK) SET ESTATUS = 'CONCLUIDO' WHERE ID = @ID
