...
SELECT @AgruparConceptoSAT = ISNULL(AgruparConceptoSATRetenciones,0) FROM EmpresaCfg2 WITH (NOLOCK) WHERE Empresa = @Empresa
...
JOIN CFDIRetencionImpuestoTipo WITH (NOLOCK) ON CFDIRetencionImpuestoTipo.Retencion = 'ISR'
JOIN GastoD WITH (NOLOCK) ON CFDIRetencion.ModuloID = GastoD.id
JOIN cfdiretsatretencion WITH (NOLOCK) ON CFDIRetencion.ConceptoSAT = cfdiretsatretencion.Clave
WHERE Proveedor = @Proveedor
AND EstacionTrabajo = @Estacion
AND ConceptoSAT = @ConceptoSAT
AND GastoD.Concepto in(Select Concepto From CFDIRetencionConcepto WITH (NOLOCK) Where Modulo = 'GAS' AND CFDIRetClave = @ConceptoSAT)
AND Empresa = @Empresa
...
FROM CFDIRetencion WITH (NOLOCK)
JOIN CFDIRetencionImpuestoTipo WITH (NOLOCK) ON CFDIRetencionImpuestoTipo.Retencion = 'IVA'
JOIN GastoD WITH (NOLOCK) ON CFDIRetencion.ModuloID = GastoD.id
JOIN cfdiretsatretencion WITH (NOLOCK) ON CFDIRetencion.ConceptoSAT = cfdiretsatretencion.Clave
WHERE Proveedor = @Proveedor
AND EstacionTrabajo = @Estacion
...
FROM CFDIRetencion WITH (NOLOCK)
JOIN CFDIRetencionImpuestoTipo WITH (NOLOCK) ON CFDIRetencionImpuestoTipo.Retencion = 'IEPS'
JOIN GastoD WITH (NOLOCK) ON CFDIRetencion.ModuloID = GastoD.id
JOIN cfdiretsatretencion WITH (NOLOCK) ON CFDIRetencion.ConceptoSAT = cfdiretsatretencion.Clave
WHERE Proveedor = @Proveedor
AND EstacionTrabajo = @Estacion
...
TipoPagoRet
FROM CFDIRetencionImpuestoD WITH (NOLOCK)
WHERE ModuloID = @ID
AND Modulo = 'CXP'
