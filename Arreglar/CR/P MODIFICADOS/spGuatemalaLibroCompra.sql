...
cg.TotalExcento
FROM CompraGuatemala cg WITH(NOLOCK) LEFT OUTER JOIN Compra c
 WITH(NOLOCK) ON cg.ID = c.ID AND cg.Modulo = 'COMS' LEFT OUTER JOIN MovTipo mt
 WITH(NOLOCK) ON c.Mov = mt.Mov AND mt.Modulo = 'COMS' LEFT OUTER JOIN Gasto g
 WITH(NOLOCK) ON cg.ID = g.ID AND cg.Modulo = 'GAS' LEFT OUTER JOIN Prov p
 WITH(NOLOCK) ON p.Proveedor = g.Acreedor LEFT OUTER JOIN FiscalRegimen fr
 WITH(NOLOCK) ON p.FiscalRegimen = fr.FiscalRegimen
...
FROM Compra c  WITH(NOLOCK) JOIN MovTipo mt
 WITH(NOLOCK) ON mt.Mov = ISNULL(c.Mov,'') AND mt.Modulo = 'COMS' JOIN Prov p
 WITH(NOLOCK) ON p.Proveedor = c.Proveedor
...
FROM Gasto g  WITH(NOLOCK) JOIN MovTipo mt
 WITH(NOLOCK) ON mt.Mov = ISNULL(g.Mov,'') AND mt.Modulo = 'GAS' JOIN Prov p
 WITH(NOLOCK) ON p.Proveedor = g.Acreedor
