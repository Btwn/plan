...
@SQLAncho = NULL,
@SQLSeparador = ' '
SELECT @Proveedor = Proveedor FROM Cxp WITH (NOLOCK) WHERE ID = @ID
...
SELECT @Para =ISNULL (eMail1,'' ) FROM Prov WITH (NOLOCK) WHERE Proveedor = @Proveedor
IF (@EnviarXML=1	AND @Enviar=1)
SELECT @Anexos = ArchivoXML FROM CFDRetencion WITH (NOLOCK) WHERE Modulo = 'CXP' AND ModuloID = @ID
IF (@EnviarPDF =1 AND @Enviar =1 )
BEGIN
IF LEN(@Anexos)>0
SET @Anexos=@Anexos+';'
SELECT @Anexos=(ISNULL(@Anexos,'')+ArchivoPDF) FROM CFDRetencion WITH (NOLOCK) WHERE Modulo = 'CXP' AND ModuloID = @ID
END
