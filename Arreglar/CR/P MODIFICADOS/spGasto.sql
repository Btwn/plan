...
@Verificar			bit*/
SELECT @MovFactor = Mov FROM Gasto  WITH (NOLOCK) WHERE ID = @ID
SELECT @Factor = Factor FROM MovTipo  WITH (NOLOCK) WHERE Modulo = @Modulo AND Mov = @MovFactor
...
SELECT @OrigenMovTipo = Clave FROM MovTipo  WITH (NOLOCK) WHERE Modulo = @OrigenTipo AND Mov = @Origen
...
BEGIN
IF @CfgActividadDetalle = 0 UPDATE GastoD WITH(ROWLOCK) SET Actividad  = @Actividad  WHERE ID = @ID AND Actividad <> @Actividad
IF @CfgActividadDetalle = 1 UPDATE GastoD WITH(ROWLOCK) SET Actividad  = @Actividad  WHERE ID = @ID AND NULLIF(RTRIM(Actividad), '') IS NULL
IF @CfgProyectoDetalle  = 0 UPDATE GastoD WITH(ROWLOCK) SET Proyecto   = @Proyecto   WHERE ID = @ID AND Proyecto  <> @Proyecto
IF @CfgProyectoDetalle  = 1 UPDATE GastoD WITH(ROWLOCK) SET Proyecto   = @Proyecto   WHERE ID = @ID AND NULLIF(RTRIM(Proyecto), '') IS NULL
IF @CfgUENDetalle       = 0 UPDATE GastoD WITH(ROWLOCK) SET UEN        = @UEN        WHERE ID = @ID AND UEN <> @UEN
IF @CfgUENDetalle       = 1 UPDATE GastoD WITH(ROWLOCK) SET UEN        = @UEN        WHERE ID = @ID AND UEN IS NULL
IF @CfgAFDetalle        = 0 UPDATE GastoD WITH(ROWLOCK) SET AFArticulo = @AFArticulo, AFSerie = @AFSerie WHERE ID = @ID AND (AFArticulo <> @AFArticulo OR AFSerie <> @AFSerie)
IF @CfgAFDetalle        = 1 UPDATE GastoD WITH(ROWLOCK) SET AFArticulo = @AFArticulo, AFSerie = @AFSerie WHERE ID = @ID AND (NULLIF(RTRIM(AFArticulo), '') IS NULL OR NULLIF(RTRIM(AFSerie), '') IS NULL)
END
