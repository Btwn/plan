...
SELECT @GastoRetencionConcepto = GastoRetencionConcepto, @GastoRetencion2Concepto = GastoRetencion2Concepto FROM EmpresaCfg2 WITH (NOLOCK) WHERE Empresa = @Empresa
IF YEAR(@FechaD)  = 1899 SELECT @FechaD = NULL
IF YEAR(@FechaA) = 1899  SELECT @FechaA = NULL
IF @Proveedor IS NOT NULL AND NOT EXISTS(SELECT Proveedor FROM Prov WITH (NOLOCK) WHERE Proveedor = @Proveedor)
SELECT @Ok = 26050
...
SELECT @Ok = 1, @OkRef = 'Solo se puede presentar la Informaci�n de un Ejercicio'
IF @Ok IS NULL AND @GastoRetencionConcepto NOT LIKE 'ISR%'
SELECT @Ok = 1, @OkRef = 'El Concepto de Retenci�n ISR debe ser "ISR - (Concepto Gasto)"<BR>Configuraci�n de M�dulos de la Empresa'
IF @Ok IS NULL AND @GastoRetencion2Concepto NOT LIKE 'IVA%'
SELECT @Ok = 1, @OkRef = 'El Concepto de Retenci�n IVA debe ser "IVA - (Concepto Gasto)"<BR>Configuraci�n de M�dulos de la Empresa'
IF @Ok IS NULL
BEGIN
...
SELECT @OkRef
ELSE
SELECT Descripcion FROM MensajeLista WITH (NOLOCK) WHERE Mensaje = @Ok
END
