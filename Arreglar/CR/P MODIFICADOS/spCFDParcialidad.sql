...
IF @Modulo = 'CXC'
BEGIN
IF (SELECT Count(ID) FROM CxcD c WITH (NOLOCK) JOIN MovTipo mt WITH (NOLOCK) ON mt.Modulo = 'CXC' AND c.Aplica = mt.Mov WHERE c.ID = @ID AND mt.Clave IN ('CXC.D','CXC.F')) > 2
...
SELECT @OrigenDAMov = d.Mov, @OrigenDAMovID = d.MovID FROM CXC c WITH (NOLOCK)
JOIN DocAuto d WITH (NOLOCK) ON c.Empresa = d.Empresa AND c.Origen = d.Mov AND c.OrigenID = d.MovID
WHERE c.ID = @IDOrigen
IF @OrigenDAMovID IS NOT NULL
BEGIN
SELECT @IDFacturaVta = ID FROM Venta WITH (NOLOCK) WHERE Empresa = @Empresa AND Mov = @OrigenDAMov AND MovID = @OrigenDAMovID
...
FROM CFD WITH (NOLOCK) WHERE Modulo = 'VTAS' AND ModuloID = @IDFacturaVta
...
BEGIN
SELECT @AplicaMov = Aplica, @AplicaMovID = AplicaID, @AplicaMovTipo = Clave FROM CxcD d WITH (NOLOCK)
JOIN MovTipo mt ON mt.Modulo = 'CXC' AND mt.Mov = d.Aplica
WHERE mt.Clave IN ('CXC.F', 'CXC.D') AND ID = @ID
SELECT @IDAplica = ID FROM Cxc WITH (NOLOCK) WHERE Empresa = @Empresa AND Mov = @AplicaMov AND MovID = @AplicaMovID
IF @AplicaMovTipo = 'CXC.F'
SELECT @IdFacturaVta = OID FROM MovFlujo v WHERE DModulo = 'CXC' AND DID = @IDAplica AND OModulo = 'VTAS'
IF @AplicaMovTipo = 'CXC.D'
BEGIN
SELECT @IDFacturaCXC = mf.OID FROM MovFlujo mf WITH (NOLOCK)
...
SELECT @IDFacturaVta = mf.OID FROM MovFlujo mf WITH (NOLOCK)
...
@MontoFolioFiscalOrig = ISNULL(Importe,0.0)+ISNULL(Impuesto1,0.0)+ISNULL(Impuesto2,0.0)-ISNULL(Retencion1,0.0)-ISNULL(Retencion2,0.0)
FROM CFD WITH (NOLOCK) WHERE Modulo = 'VTAS' AND ModuloID = @IDFacturaVta
IF (SELECT NULLIF(ParcialidadNumero,'') FROM CFD WITH (NOLOCK) WHERE Modulo = @Modulo AND ModuloID = @ID) IS NULL
BEGIN
SELECT @Parcialidad = MAX(ParcialidadNumero) FROM CFD WITH (NOLOCK) WHERE Empresa = @Empresa AND @Modulo ='CXC' AND OrigenSerie = @SerieFolioFiscalOrig AND OrigenUUID = @FolioFiscalOrig
SELECT @Parcialidad = ISNULL(@Parcialidad,0) + 1
END ELSE
SELECT @Parcialidad = NULLIF(ParcialidadNumero,'') FROM CFD WITH (NOLOCK) WHERE Modulo = @Modulo AND ModuloID = @ID
