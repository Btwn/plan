...
SET @Cadena = ''
IF NOT EXISTS( SELECT * FROM eDocInRutaD WITH(NOLOCK)  WHERE eDocIn = @eDocIn AND Ruta = @Ruta AND Tipo = @Tipo)
SELECT @Cadena = '(1)'
ELSE
BEGIN
...
WITH(NOLOCK) WHERE eDocIn = @eDocIn AND Ruta = @Ruta AND GUID = @GUID
