...
IF NOT EXISTS(SELECT * FROM Agente WITH (NOLOCK)  WHERE Agente = @Agente)
SELECT @Ok = 26090
IF @Ok IS NOT NULL
SET @OkRef = ISNULL(@OkRef,'')+(SELECT Descripcion FROM MensajeLista WITH (NOLOCK)  WHERE Mensaje = @Ok)
...
FROM Art Art WITH (NOLOCK)  JOIN Agenteart aa WITH (NOLOCK)  ON Art.Articulo = aa.Articulo
LEFT JOIN ArtUnidad ArtUnidad WITH (NOLOCK)  ON Art.Articulo = ArtUnidad.Articulo
