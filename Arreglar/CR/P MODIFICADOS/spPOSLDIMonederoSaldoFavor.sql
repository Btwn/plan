...
DECLARE
@Mensaje	varchar(255)
IF EXISTS(SELECT * FROM POSLDIMonederoSaldoFavor plmsf WITH (NOLOCK) WHERE plmsf.ID = @ID)
UPDATE POSLDIMonederoSaldoFavor WITH (ROWLOCK) SET Monedero = @Monedero WHERE ID = @ID
ELSE
