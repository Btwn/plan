...
IF @LlamadaExterna = 0
BEGIN
IF @Modulo = 'VTAS' SELECT @Contacto = Cliente,   @Mov = Mov, @MovID = MovID, @FechaEmision = FechaEmision, @FechaRegistro = FechaRegistro, @Usuario = Usuario, @TipoCambio = TipoCambio, @Sucursal = Sucursal FROM Venta WITH (NOLOCK)   WHERE ID = @ID ELSE
...
IF @Modulo = 'COMS' SELECT @Contacto = Proveedor, @Mov = Mov, @MovID = MovID, @FechaEmision = FechaEmision, @FechaRegistro = FechaRegistro, @Usuario = Usuario, @TipoCambio = TipoCambio, @Sucursal = Sucursal FROM Compra WITH (NOLOCK)  WHERE ID = @ID ELSE
IF @Modulo = 'CXP'  SELECT @Contacto = Proveedor, @Mov = Mov, @MovID = MovID, @FechaEmision = FechaEmision, @FechaRegistro = FechaRegistro, @Usuario = Usuario, @TipoCambio = TipoCambio, @Sucursal = Sucursal FROM Cxp    WITH (NOLOCK)  WHERE ID = @ID ELSE
IF @Modulo = 'GAS'  SELECT @Contacto = Acreedor,  @Mov = Mov, @MovID = MovID, @FechaEmision = FechaEmision, @FechaRegistro = FechaRegistro, @Usuario = Usuario, @TipoCambio = TipoCambio, @Sucursal = Sucursal FROM Gasto  WITH (NOLOCK)  WHERE ID = @ID ELSE
IF @Modulo = 'CORTE'SELECT @Contacto = CorteCuentaDe,  @Mov = Mov, @MovID = MovID, @FechaEmision = FechaEmision, @FechaRegistro = FechaRegistro, @Usuario = Usuario, @TipoCambio = TipoCambio, @Sucursal = Sucursal FROM Corte WITH (NOLOCK) WHERE ID = @ID
END ELSE
BEGIN
IF NULLIF(@Contacto,'') IS NULL SET @Contacto = '(Todos)'
...
IF @OK IS NULL
BEGIN
IF EXISTS(SELECT 1 FROM CFD WITH (NOLOCK) WHERE ModuloID = @ID AND Modulo = @Modulo AND ISNULL(Timbrado,0) = 1 AND @CFDI = 1) 
SET @Continuar = 0
ELSE
IF EXISTS(SELECT 1 FROM CFD WITH (NOLOCK) WHERE ModuloID = @ID AND Modulo = @Modulo AND Documento IS NOT NULL AND @CFDI = 0) 
SET @Continuar = 0
END
...
IF @Modulo = 'CXC' AND ('CXC.D' IN(SELECT Clave FROM MovTipo WITH (NOLOCK) WHERE Mov = @MovOrigen AND Modulo = @Modulo) AND @NoValidarOrigenDocumento = 1)
BEGIN
SELECT @CFDEsParcialidad = ISNULL(CFDEsParcialidad,0) FROM MovTipo WITH (NOLOCK) WHERE Modulo = @Modulo AND Mov = @Mov
...
IF(SELECT COUNT(1) FROM CxcD WITH (NOLOCK) WHERE ID = @ID AND NULLIF(AplicaID,'') IS NOT NULL) > 1 SELECT @Ok = 60210 
IF @Modulo = 'CORTE'
BEGIN
...
END ELSE
IF @Estatus NOT IN (SELECT NULLIF(Estatus,'') FROM MovTipoCFDFlexEstatus WITH (NOLOCK) WHERE Modulo = @Modulo AND Mov = @Mov AND Contacto = @Contacto)
BEGIN
IF @Estatus NOT IN (SELECT NULLIF(Estatus,'') FROM MovTipoCFDFlexEstatus WITH (NOLOCK) WHERE Modulo = @Modulo AND Mov = @Mov AND ISNULL(NULLIF(ISNULL(NULLIF(Contacto,''),'(Todos)'),'(Todos)'),@Contacto) = @Contacto) SET @Continuar = 0
END
IF @OK IS NULL
EXEC spCFDFlexValidarPlantillaConfiguracion @Comprobante, @Modulo, @CFDI, @Ok OUTPUT, @OkRef OUTPUT
IF @ModificarModulo = 1
BEGIN
IF @Modulo = 'VTAS' UPDATE Venta WITH (ROWLOCK) SET CFDFlexEstatus = @CFDFlexEstatus WHERE ID = @ID ELSE
IF @Modulo = 'CXC' UPDATE Cxc WITH (ROWLOCK) SET CFDFlexEstatus = @CFDFlexEstatus WHERE ID = @ID ELSE
IF @Modulo = 'COMS' UPDATE Compra WITH (ROWLOCK) SET CFDFlexEstatus = @CFDFlexEstatus WHERE ID = @ID ELSE
IF @Modulo = 'CXP' UPDATE Cxp WITH (ROWLOCK) SET CFDFlexEstatus = @CFDFlexEstatus WHERE ID = @ID ELSE
IF @Modulo = 'GAS' UPDATE Gasto WITH (ROWLOCK) SET CFDFlexEstatus = @CFDFlexEstatus WHERE ID = @ID     ELSE
IF @Modulo = 'CORTE' UPDATE Corte WITH (ROWLOCK) SET CFDFlexEstatus = @CFDFlexEstatus WHERE ID = @ID
END
RETURN
END
