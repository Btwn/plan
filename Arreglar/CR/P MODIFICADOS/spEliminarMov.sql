...
IF @Modulo = 'CONT'  SELECT @MovID = MovID, @Estatus = Estatus FROM Cont  	     WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'VTAS'  SELECT @MovID = MovID, @Estatus = Estatus FROM Venta        WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'PROD'  SELECT @MovID = MovID, @Estatus = Estatus FROM Prod         WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'COMS'  SELECT @MovID = MovID, @Estatus = Estatus FROM Compra       WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'INV'   SELECT @MovID = MovID, @Estatus = Estatus FROM Inv          WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'CXC'   SELECT @MovID = MovID, @Estatus = Estatus FROM Cxc          WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'CXP'   SELECT @MovID = MovID, @Estatus = Estatus FROM Cxp          WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'AGENT' SELECT @MovID = MovID, @Estatus = Estatus FROM Agent        WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'GAS'   SELECT @MovID = MovID, @Estatus = Estatus FROM Gasto        WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'DIN'   SELECT @MovID = MovID, @Estatus = Estatus FROM Dinero       WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'EMB'   SELECT @MovID = MovID, @Estatus = Estatus FROM Embarque     WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'NOM'   SELECT @MovID = MovID, @Estatus = Estatus FROM Nomina       WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'RH'    SELECT @MovID = MovID, @Estatus = Estatus FROM RH      	 WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'ASIS'  SELECT @MovID = MovID, @Estatus = Estatus FROM Asiste       WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'AF'    SELECT @MovID = MovID, @Estatus = Estatus FROM ActivoFijo   WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'PC'    SELECT @MovID = MovID, @Estatus = Estatus FROM PC           WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'OFER'  SELECT @MovID = MovID, @Estatus = Estatus FROM Oferta       WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'VALE'  SELECT @MovID = MovID, @Estatus = Estatus FROM Vale         WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'CR'    SELECT @MovID = MovID, @Estatus = Estatus FROM CR           WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'ST'    SELECT @MovID = MovID, @Estatus = Estatus FROM Soporte      WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'CAP'   SELECT @MovID = MovID, @Estatus = Estatus FROM Capital      WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'INC'   SELECT @MovID = MovID, @Estatus = Estatus FROM Incidencia   WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'CONC'  SELECT @MovID = MovID, @Estatus = Estatus FROM Conciliacion WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'PPTO'  SELECT @MovID = MovID, @Estatus = Estatus FROM Presup       WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'CREDI' SELECT @MovID = MovID, @Estatus = Estatus FROM Credito	     WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'TMA'   SELECT @MovID = MovID, @Estatus = Estatus FROM TMA          WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'RSS'   SELECT @MovID = MovID, @Estatus = Estatus FROM RSS          WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'CMP'   SELECT @MovID = MovID, @Estatus = Estatus FROM Campana      WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'FIS'   SELECT @MovID = MovID, @Estatus = Estatus FROM Fiscal       WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'CONTP' SELECT @MovID = MovID, @Estatus = Estatus FROM ContParalela WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'OPORT' SELECT @MovID = MovID, @Estatus = Estatus FROM Oportunidad  WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'CORTE' SELECT @MovID = MovID, @Estatus = Estatus FROM Corte        WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'FRM'   SELECT @MovID = MovID, @Estatus = Estatus FROM FormaExtra   WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'CAPT'  SELECT @MovID = MovID, @Estatus = Estatus FROM Captura      WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'GES'   SELECT @MovID = MovID, @Estatus = Estatus FROM Gestion      WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'CP'    SELECT @MovID = MovID, @Estatus = Estatus FROM CP           WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'PCP'   SELECT @MovID = MovID, @Estatus = Estatus FROM PCP          WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'PROY'  SELECT @MovID = MovID, @Estatus = Estatus FROM Proyecto     WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'ORG'   SELECT @MovID = MovID, @Estatus = Estatus FROM Organiza     WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'RE'    SELECT @MovID = MovID, @Estatus = Estatus FROM Recluta      WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'ISL'   SELECT @MovID = MovID, @Estatus = Estatus FROM ISL          WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'CAM'   SELECT @MovID = MovID, @Estatus = Estatus FROM Cambio       WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'PACTO' SELECT @MovID = MovID, @Estatus = Estatus FROM Contrato     WITH(NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'SAUX'  SELECT @MovID = MovID, @Estatus = Estatus FROM SAUX         WITH(NOLOCK) WHERE ID = @ID
