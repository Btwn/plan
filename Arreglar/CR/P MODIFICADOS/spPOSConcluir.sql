...
SELECT @UsuarioPerfil = NULLIF(POSPerfil,'') FROM Usuario WITH(NOLOCK) WHERE Usuario = @Usuario
...
INNER JOIN MovTipo mt WITH(NOLOCK) ON mt.Mov = p.Mov AND mt.Modulo = 'POS'
WHERE p.ID = @ID
...
BEGIN
SELECT @ImporteVerificar = SUM(ISNULL(Importe,0)) FROM POSLCobro WITH(NOLOCK) WHERE ID = @ID 
...
BEGIN
IF EXISTS(SELECT * FROM POSLVenta p WITH(NOLOCK) JOIN Art a WITH(NOLOCK) ON p.Articulo = a.Articulo
...
FROM POSLVenta p JOIN Art a WITH(NOLOCK) ON a.Articulo = p.Articulo
WHERE p.ID = @ID
...
END
END
IF(SELECT NULLIF(Monedero,'') FROM POSL WITH(NOLOCK) WHERE ID = @ID) IS NULL
UPDATE POSLVenta WITH(ROWLOCK) SET Puntos = NULL  WHERE ID = @ID
IF @ValidarDevolucion = 1 AND @ImporteMov < 0 AND @MovClave IN ('POS.N','POS.NPC')
BEGIN
IF NOT EXISTS(SELECT * FROM POSLValidarDevolucion plv WITH(NOLOCK) WHERE plv.ID = @ID)
BEGIN
...
FROM POSLDIMonederoSaldoFavor WITH(NOLOCK)
WHERE ID = @ID
...
SELECT @ImporteLDI = SUM(ISNULL(PUNTOS,0)) FROM POSLVenta WITH(NOLOCK) WHERE ID = @ID
EXEC spPOSLDI 'MON ABONO', @ID, @MonederoLDI, @Empresa, @Usuario, @Sucursal, NULL, @ImporteLDI, 1, null, @Ok OUTPUT, @OkRef OUTPUT, 'POS'
END
...
SELECT @Ok = 20370
IF NOT EXISTS(SELECT * FROM POSLVenta WITH(NOLOCK) WHERE ID = @ID) AND @MovClave IN ('POS.N', 'POS.F', 'POS.A','POS.P','POS.NPC','POS.INVD', 'POS.INVA')
SELECT @Ok = 60010
END
...
FROM POSUsuarioMov pum WITH(NOLOCK)
...
SELECT @Ok = 30210
END
IF NOT EXISTS(SELECT * FROM POSLCobro WITH(NOLOCK) WHERE ID = @ID) AND @Ok IS NULL
...
IF EXISTS(SELECT * FROM POSCFDFlexPendiente WITH(NOLOCK))
...
BEGIN
IF NOT EXISTS(SELECT * FROM POSEstatusCaja WITH(NOLOCK) WHERE Caja = @CtaDineroDestino)
SELECT @Ok = 30440, @OkRef = @CtaDineroDestino
IF (SELECT Abierto FROM POSEstatusCaja WITH(NOLOCK) WHERE Caja = @Caja) = 0 AND @iSyncNivel = 'Sucursal'
SELECT @Ok =30440, @OkRef = @Caja
END
IF @MovClave IN ('POS.CC','POS.CCM','POS.CAC','POS.CACM') AND @Ok IS NULL
BEGIN
SELECT @EsConcentradora = ISNULL(EsConcentradora,0) FROM CtaDinero WITH(NOLOCK) WHERE CtaDinero = @CtaDinero
...
SELECT @Caja = Caja FROM POSL WITH(NOLOCK) WHERE ID = @ID
IF EXISTS (SELECT * FROM POSLDIRecargaTemp WITH(NOLOCK) WHERE ID = @ID) AND @Ok IS NULL
BEGIN
EXEC spPOSLDIRecargaTelefonica @ID, @Ok OUTPUT, @OkRef OUTPUT
IF @OK IS NULL
SELECT @Mensaje = 'RECARGA EXITOSA  <BR>'
END
IF EXISTS (SELECT * FROM POSLDIPagoServicioTemp WITH(NOLOCK) WHERE ID = @ID) AND @Ok IS NULL
BEGIN
...
UPDATE POSLCobro WITH(ROWLOCK)SET Importe = ImporteRef WHERE ID = @ID AND ImporteRef <> 0 AND Importe = 0
IF @IDDevolucion IS NOT NULL
BEGIN
UPDATE POSL WITH(ROWLOCK) SET Devolucion = 1 WHERE ID = @IDDevolucion
UPDATE POSLVenta WITH(ROWLOCK) SET CantidadSaldo = 0 WHERE ID = @IDDevolucion
END
IF @IDDevolucionP IS NOT NULL
BEGIN
DECLARE @Contador INT, @CantidadSaldoSum float
UPDATE POSL WITH(ROWLOCK) SET DevolucionP = 1 WHERE ID = @IDDevolucionP
...
UPDATE POSLVenta WITH(ROWLOCK) SET CantidadSaldo = @CantidadSaldoSum
WHERE ID = @IDDevolucionP AND Articulo = @Articulo AND NULLIF(SubCuenta,'') = @SubCuenta
END
FETCH NEXT FROM crSE1 INTO @Articulo, @Cantidad, @SubCuenta
END
...
DEALLOCATE crSE1
END
UPDATE POSLVenta WITH(ROWLOCK) SET CantidadSaldo = Cantidad WHERE ID = @ID
END
...
FROM POSL p WITH(NOLOCK)
INNER JOIN POSLVenta plv WITH(NOLOCK) ON p.ID = plv.ID
WHERE plv.ID = @ID
SELECT @Condicion = NULLIF(Condicion,'')
FROM POSL WITH(NOLOCK)
WHERE ID = @ID
UPDATE POSL WITH(ROWLOCK) Set Importe = ROUND(@TotalImporte,@RedondeoMonetarios), Impuestos = ROUND(@TotalImpuesto1,@RedondeoMonetarios)
WHERE ID = @ID
END
IF @Ok IS NULL AND @MovClave = 'POS.N' AND @MovSubClave = 'POS.PEDCONT'
IF NULLIF(@Condicion,'') IS NULL OR @Condicion NOT IN (SELECT Condicion FROM Condicion WITH(NOLOCK) WHERE ControlAnticipos = 'Cobrar Pedido')
SELECT @Ok = 20880
IF @Ok IS NULL AND @MovClave = 'POS.P' AND @MovSubClave = 'POS.DEVCRED'
IF NULLIF(@Condicion,'') IS NULL OR @Condicion NOT IN (SELECT Condicion FROM Condicion WITH(NOLOCK) WHERE TipoCondicion = 'Credito')
SELECT @Ok = 20700
IF @Ok IS NULL AND (@MovClave IN ('POS.F','POS.N')
AND (SELECT ISNULL(AnticipoFacturadoTipoServicio,0) FROM POSL WITH(NOLOCK) WHERE ID = @ID)= 1)
OR (@MovClave  = 'POS.P' AND @MovSubClave = 'POS.FACCRED')OR (@MovClave  = 'POS.P' AND @MovSubClave = 'POS.DEVCRED')
BEGIN
IF @Ok IS NULL AND  @CxcLocal = 0
UPDATE POSL WITH(ROWLOCK) Set Estatus = 'CONFIRMAR'  WHERE ID = @ID
IF @MovClave IN ('POS.F','POS.N', 'POS.P') AND EXISTS(SELECT * FROM POSLCobro WITH(NOLOCK) WHERE ID = @ID)
IF NOT EXISTS(SELECT * FROM POSVentaCobro WITH(NOLOCK) WHERE ID = @ID)
...
AND NOT EXISTS(SELECT * FROM POSVentaCobro WITH(NOLOCK) WHERE ID = @ID) AND EXISTS (SELECT * FROM POSLCobro WITH(NOLOCK) WHERE ID = @ID)
EXEC spPOSInsertarPOSVentaCobro @ID, @Empresa,@Sucursal, @Ok OUTPUT, @OkRef OUTPUT
...
UPDATE POSL WITH(ROWLOCK) Set Estatus = 'CONCLUIDO'  WHERE ID = @ID
IF @MovClave = 'POS.N' AND (SELECT ISNULL(AnticipoFacturadoTipoServicio,0) FROM POSL WITH(NOLOCK) WHERE ID = @ID)= 1
UPDATE POSL WITH(ROWLOCK) Set Estatus = 'TRASPASADO' WHERE ID = @ID
IF @Ok IS NULL AND  @CxcLocal = 1 AND @MovClave IN('POS.N')
UPDATE POSL WITH(ROWLOCK) Set AfectadoLocal = 1  WHERE ID = @ID
...
FROM POSL p WITH(NOLOCK)
INNER JOIN POSLCobro c WITH(NOLOCK) ON p.ID = c.ID
WHERE c.ID = @ID
IF @MovSubClave = 'POS.FACCRED' AND @CxcLocal = 0
UPDATE POSL WITH(ROWLOCK) Set Estatus = 'TRASPASADO' WHERE ID = @ID
UPDATE POSL WITH(ROWLOCK) Set Importe = ROUND(@TotalImporte,@RedondeoMonetarios) WHERE ID = @ID
END
...
WHERE ID = @ID
IF EXISTS(SELECT * FROM POSLPorCobrar WITH(NOLOCK) WHERE ID = @IDR AND Estatus = 'PORCOBRAR')
UPDATE 	POSLPorCobrar WITH(ROWLOCK) SET Estatus = 'TRASPASADO' WHERE ID = @IDR
IF @Ok IS NULL
BEGIN
...
UPDATE POSL WITH(ROWLOCK) SET Estatus = 'TRASPASADO'
FROM POSL p WITH(NOLOCK) JOIN MovTipo m WITH(NOLOCK) ON m.Mov = p.Mov AND m.Modulo = 'POS'
WHERE m.Clave = 'POS.NPC' AND p.Estatus = 'PORCOBRAR' AND p.ID IN (SELECT ID FROM POSLPorCobrar WITH(NOLOCK) WHERE Estatus = 'TRASPASADO')
END
...
JOIN FormaPago f WITH(NOLOCK) ON plc.FormaPago = f.FormaPago
