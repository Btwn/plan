	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				IF NOT EXISTS (SELECT * FROM CobroXPoliticaHistMAVI  WITH (NOLOCK)  WHERE Mov = @Origen AND MovID = @OrigenID AND CONVERT(VARCHAR(8), FechaEmision, 112) = CONVERT(VARCHAR(8), @FechaAplicacion, 112) AND EstatusCobro = 'CONCLUIDO')
				BEGIN
