...
IF NOT EXISTS(SELECT * FROM Jornada WITH(NOLOCK) WHERE Jornada = @Jornada) RETURN
SELECT @Ok = NULL
...
FROM JornadaD d
 WITH(NOLOCK) JOIN Jornada j  WITH(NOLOCK) ON d.Jornada = j.Jornada
...
IF @NoLabora = 0
BEGIN
IF @Fecha IN (SELECT Fecha FROM DiaFestivo WITH(NOLOCK) WHERE EsLaborable = 0)
SELECT @EsFestivoGeneral = 1
IF @Fecha IN (SELECT Fecha FROM JornadaDiaFestivo WITH(NOLOCK) WHERE Jornada = @Jornada AND EsLaborable = 0)
