...
FROM POSLVenta p WITH(NOLOCK)
LEFT OUTER JOIN OfertaD od WITH(NOLOCK) ON  p.Articulo = od.Articulo
LEFT OUTER JOIN Oferta o WITH(NOLOCK) ON od.ID = o.ID
LEFT OUTER JOIN MovTipo mt WITH(NOLOCK) ON  o.Mov  = mt.Mov AND mt.Modulo = 'OFER' AND mt.Clave = 'OFER.OF'
WHERE p.ID = @ID
...
SELECT Articulo
FROM POSCobroCondicionTemp WITH(NOLOCK)
OPEN CrPOSInt
...
LEFT OUTER JOIN OfertaD WITH(NOLOCK) ON Oferta.ID = OfertaD.ID
LEFT OUTER JOIN MovTipo WITH(NOLOCK) ON MovTipo.Mov = Oferta.Mov AND MovTipo.Modulo = 'OFER'
...
BEGIN
IF EXISTS (SELECT * FROM ART WITH(NOLOCK) WHERE Categoria = @Categoria AND Articulo = @Articulo)
BEGIN
INSERT INTO FormaPagoPos (
ID, Articulo, FormaPago)
VALUES(
...
BEGIN
IF EXISTS (SELECT * FROM ART WITH(NOLOCK) WHERE Grupo = @Grupo AND Articulo = @Articulo)
BEGIN
INSERT INTO FormaPagoPos (
ID, Articulo, FormaPago)
VALUES(
...
BEGIN
IF EXISTS (SELECT * FROM ART WITH(NOLOCK) WHERE Familia = @Familia AND Articulo = @Articulo)
BEGIN
INSERT INTO FormaPagoPos (
ID, Articulo, FormaPago)
VALUES(
...
BEGIN
IF EXISTS (SELECT * FROM ART WITH(NOLOCK) WHERE Linea = @Linea AND Articulo = @Articulo)
BEGIN
INSERT INTO FormaPagoPos (
ID, Articulo, FormaPago)
VALUES(
...
BEGIN
IF EXISTS (SELECT * FROM ART WITH(NOLOCK) WHERE Fabricante = @Fabricante AND Articulo = @Articulo)
BEGIN
INSERT INTO FormaPagoPos (
ID, Articulo, FormaPago)
VALUES(
...
CLOSE CrPOSInt
DEALLOCATE CrPOSInt
IF EXISTS (SELECT * FROM POSCobroCondicionTemp WITH(NOLOCK) WHERE ID = @ID)
SELECT  1
ELSE
SELECT 0
END
