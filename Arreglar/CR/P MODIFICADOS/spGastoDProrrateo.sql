...
@Clase		varchar(50),
@SubClase		varchar(50)
SELECT @Clase = Clase, @SubClase = NULLIF(RTRIM(SubClase), '') FROM Gasto WITH (NOLOCK) WHERE ID = @ID
...
IF NOT EXISTS(SELECT * FROM GastoDProrrateo WITH (NOLOCK) WHERE ID = @ID AND Renglon = @Renglon AND RenglonSub = @RenglonSub AND Concepto = @Concepto)
BEGIN
...
END
UPDATE GastoDProrrateo  WITH (ROWLOCK)
...
SELECT @Total = 0.0
SELECT @Total = ROUND(SUM(Porcentaje), @RedondeoMonetarios) FROM GastoDProrrateo WITH (NOLOCK) WHERE ID = @ID AND Renglon = @Renglon AND RenglonSub = @RenglonSub AND Concepto = @Concepto
