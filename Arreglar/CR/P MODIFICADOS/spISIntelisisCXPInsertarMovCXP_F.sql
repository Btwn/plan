...
GO
ALTER PROCEDURE spISIntelisisCXPInsertarMovCXP_F
@ID				int,
@iSolicitud		int,
@Version		float,
...
IF NOT EXISTS(SELECT * FROM Condicion WITH (NOLOCK) WHERE Condicion = @Condicion ) AND NULLIF(@Condicion,'') IS NOT NULL SELECT @Ok = 30030
IF NOT EXISTS(SELECT * FROM Mon WITH (NOLOCK) WHERE Moneda = @ProveedorMoneda ) AND NULLIF(@ProveedorMoneda,'') IS NOT NULL SELECT @Ok = 20196
IF NOT EXISTS(SELECT * FROM Mon WITH (NOLOCK) WHERE Moneda = @Moneda ) AND NULLIF(@Moneda,'') IS NOT NULL SELECT @Ok = 20196
SELECT
