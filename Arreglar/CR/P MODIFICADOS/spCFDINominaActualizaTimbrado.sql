...
AND ModuloID = @ID
AND Personal = @Personal
IF NOT EXISTS(SELECT * FROM CFDNomina WITH (NOLOCK) WHERE Modulo = 'NOM' AND ModuloID = @ID AND ISNULL(Timbrado, 0) = 0)
UPDATE Nomina WITH (ROWLOCK) SET CFDTimbrado = 1 WHERE ID = @ID
RETURN
END
