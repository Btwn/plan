...
MovID = b.MovID
FROM @TablaPaquete a
JOIN TMA b WITH(NOLOCK) ON(a.IDDestino = b.ID)
UPDATE @TablaPaquete
...
FROM @TablaPaquete a
JOIN Sucursal b WITH(NOLOCK) ON(a.Sucursal = b.Sucursal)
UPDATE @TablaPaquete
SET AlmacenNombre = b.Nombre
FROM @TablaPaquete a
JOIN Alm b WITH(NOLOCK) ON(a.Almacen = b.Almacen)
