...
GO
ALTER PROCEDURE spISIntelisisINVInsertarMovINV_E
@ID                     int,
@iSolicitud       int,
@Version          float,
...
SELECT @CfgReservar = PedidosReservar ,@CfgPosiciones = ISNULL(Posiciones, 0) FROM EmpresaCfg WITH(NOLOCK) WHERE Empresa = @Empresa
...
BEGIN
SELECT @FormaCosteo = FormaCosteo, @TipoCosteo = TipoCosteo FROM EmpresaCfg WITH(NOLOCK) WHERE Empresa = @Empresa
SELECT @INFORTipo = INFORTipo FROM Art WITH(NOLOCK) WHERE Articulo = @Articulo
...
SELECT @Aplica = i.Mov, @AplicaID = i.MovID FROM Inv i WITH(NOLOCK) JOIN MovTipo mt WITH(NOLOCK) ON i.Mov = mt.Mov AND mt.Modulo = 'INV' WHERE i.ReferenciaMES = @ReferenciaMES AND i.OrigenTipo = @OrigenTipo AND mt.Clave= '' AND i.Estatus = 'PENDIENTE'
IF @CfgPosiciones = 1
SELECT @Posicion= DefPosicionRecibo FROM Alm WITH(NOLOCK) WHERE ISNULL(Ubicaciones,0) = 1 AND NULLIF(DefPosicionRecibo,'') IS NOT NULL AND Almacen = @AlmacenD
...
IF NOT EXISTS(SELECT ID FROM InvD WITH(NOLOCK) WHERE ID = @IDGenerar AND Articulo = @Articulo)
BEGIN
...
ELSE
BEGIN
UPDATE InvD WITH(ROWLOCK) SET Cantidad = Cantidad + @Cantidad WHERE ID = @IDGenerar AND Articulo = @Articulo
END
IF @Ok IS NULL
BEGIN
...
BEGIN
SELECT @MovVenta = Mov FROM MovTipo WITH(NOLOCK) WHERE SerieMES = @Serie
SELECT @IDVenta = @PedidoMES 
IF @IDVenta IS NOT NULL
BEGIN
SELECT @RenglonCompra =Renglon,@CantidadVenta=CantidadPendiente FROM VentaD WITH(NOLOCK) WHERE ID = @IDVenta AND Articulo = @Articulo AND SubCuenta = @SubCuenta
...
END
BEGIN TRANSACTION
IF (@Ok IS NULL AND @Estatus = 'CONFIRMAR') 
BEGIN
BEGIN TRY
...
END
IF @OK IS NULL  AND EXISTS(SELECT * FROM Inv i WITH(NOLOCK) JOIN InvD d WITH(NOLOCK) ON d.ID = i.ID JOIN MovTipo m WITH(NOLOCK) ON m.Mov = i.Mov AND m.Modulo = 'INV'  JOIN Inv c WITH(NOLOCK) ON c.ID = @IDGenerar JOIN InvD cd WITH(NOLOCK) ON c.ID = cd.ID AND cd.Articulo = d.Articulo AND ISNULL(cd.SubCuenta,'') = ISNULL(d.SubCuenta,'') AND ISNULL(cd.Almacen,c.Almacen) = ISNULL(d.Almacen,i.Almacen)  WHERE m.Clave = 'INV.SOL' AND i.Estatus = 'PENDIENTE' AND i.OrigenTipo = 'I:MES' AND ISNULL(d.CantidadPendiente,0.0) > 0 )
BEGIN
BEGIN TRANSACTION
...
FROM Inv c WITH(ROWLOCK) JOIN InvD d WITH(ROWLOCK) ON c.ID = d.ID
...
FROM Inv i WITH(NOLOCK) JOIN InvD d WITH(NOLOCK) ON d.ID = i.ID JOIN MovTipo m WITH(NOLOCK) ON m.Mov = i.Mov AND m.Modulo = 'INV'
...
BEGIN
UPDATE InvD WITH(ROWLOCK) SET CantidadA = Cantidad WHERE ID = @IDSol AND Renglon = @RenglonO
...
END
END
IF @OK IS NULL AND @CfgReservar = 1 AND EXISTS(SELECT * FROM Venta v WITH(NOLOCK) JOIN VentaD d WITH(NOLOCK) ON d.ID = v.ID JOIN MovTipo m WITH(NOLOCK) ON m.Mov = v.Mov AND m.Modulo = 'VTAS' JOIN InvD i WITH(NOLOCK) ON i.Articulo = d.Articulo AND ISNULL(i.SubCuenta,'') = ISNULL(d.SubCuenta,'') AND i.ID = @IDGenerar WHERE m.Clave = 'VTAS.P' AND v.Estatus = 'PENDIENTE'  AND d.CantidadPendiente > 0 )
BEGIN
BEGIN TRANSACTION
DECLARE crArt CURSOR LOCAL FAST_FORWARD FOR
SELECT d.Articulo, d.Cantidad, d.SubCuenta, d.Unidad, d.Almacen
FROM Inv i WITH(NOLOCK) JOIN InvD d WITH(NOLOCK) ON i.ID = d.ID WHERE i.ID = @IDGenerar
OPEN crArt
...
FROM Venta v WITH(NOLOCK) JOIN VentaD d WITH(NOLOCK) ON d.ID = v.ID JOIN MovTipo m WITH(NOLOCK) ON m.Mov = v.Mov AND m.Modulo = 'VTAS'
...
BEGIN
/*ARL 26Ene2015 */
UPDATE VentaD WITH(ROWLOCK) SET CantidadA = CantidadPendiente WHERE ID = @IDPedido AND Renglon = @RenglonD
