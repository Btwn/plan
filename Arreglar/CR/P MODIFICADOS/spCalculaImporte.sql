...
IF @Modulo = 'PROD' SELECT @Mov = Mov FROM Prod WITH (NOLOCK) WHERE ID = @ID ELSE
IF @Modulo = 'INV' SELECT @Mov = Mov FROM Inv WITH (NOLOCK) WHERE ID = @ID
SELECT @SubMovTipo = SubClave FROM MovTipo WITH (NOLOCK) WHERE Mov = @Mov AND Modulo = @Modulo
END
SELECT @RedondeoMonetarios = RedondeoMonetarios, @Impuesto2Info = ISNULL(Impuesto2Info, 0), @Impuesto3Info = ISNULL(Impuesto3Info, 0), @Impuesto2BaseImpuesto1 = ISNULL(Impuesto2BaseImpuesto1, 0), @Retencion2BaseImpuesto1 = ISNULL(Retencion2BaseImpuesto1, 0) FROM Version WITH (NOLOCK)
