...
SELECT @Perfil = NULLIF(DBMailPerfil,'') FROM EmpresaGral WITH (NOLOCK) WHERE Empresa = @Empresa
SELECT @MedioEnvio = EnviarMedio FROM EmpresaCFD WITH (NOLOCK) WHERE  Empresa = @Empresa
SELECT @RutaIntelisisCFDI = RutaIntelisisCFDI, @Enviar =  Enviar FROM EmpresaCFD WITH (NOLOCK) WHERE Empresa = @Empresa
