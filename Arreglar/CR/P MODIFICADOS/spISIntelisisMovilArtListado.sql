...
JOIN ListaPreciosD lp WITH(NOLOCK) ON ISNULL(ISNULL(NULLIF(d.ListaPreciosEsp,''),e.ListaPreciosEsp), lp.Lista) = lp.Lista
JOIN Art a WITH(NOLOCK) ON lp.Articulo = a.Articulo AND a.ESTATUS = 'ALTA'
JOIN SaldoU s WITH(NOLOCK) ON a.Articulo = s.Cuenta AND s.Empresa = @Empresa
JOIN Alm al WITH(NOLOCK) ON s.Grupo = al.Almacen AND al.Sucursal = @Sucursal
...
JOIN Art a WITH(NOLOCK) ON pd.Articulo = a.Articulo AND a.ESTATUS = 'ALTA'
...
SELECT Articulo
FROM OfertaMovilTemp WITH(NOLOCK)
WHERE GUID = @@SPID
GROUP BY Articulo
OPEN crArticulos
