...
SELECT @Factor = Factor FROM MovTipo WITH(NOLOCK) WHERE Mov = @Mov AND Clave = @MovTipo AND Modulo = @Modulo
...
FROM GastoD g  WITH(NOLOCK) JOIN Concepto c
 WITH(NOLOCK) ON g.Concepto = c.Concepto AND c.Modulo = 'GAS'
WHERE g.ID = @ID
OPEN crVerificar
...
WITH(NOLOCK) WHERE Concepto = @Concepto
...
FROM Concepto WITH (NOLOCK) WHERE Concepto = @Concepto AND Modulo = @Modulo
