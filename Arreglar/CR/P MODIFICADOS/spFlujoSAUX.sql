...
@EstatusD				varchar(20)
IF @Modulo = 'INV'
SELECT @Estatus = Estatus FROM Inv  WITH (NOLOCK)  WHERE ID = @ID
SELECT TOP 1
...
AND Modulo = @Modulo
AND Clave = @MovTipo
IF @Modulo = 'VTAS' AND NOT EXISTS(SELECT * FROM Art  WITH (NOLOCK)  WHERE Articulo IN (SELECT Articulo FROM VentaD  WITH (NOLOCK) WHERE ID = @ID)AND SAUX = 1 AND Tipo = 'Servicio')
RETURN
ELSE
IF @Modulo = 'COMS' AND NOT EXISTS(SELECT * FROM Art  WITH (NOLOCK)  WHERE Articulo IN (SELECT Articulo FROM CompraD  WITH (NOLOCK) WHERE ID = @ID)AND SAUX = 1 AND Tipo = 'Servicio')
RETURN
ELSE
...
IF EXISTS(SELECT * FROM SAUX  WITH (NOLOCK)  WHERE Origen = @MovSS AND OrigenID = @MovIDSS AND Estatus <> 'CANCELADO') AND @Accion = 'CANCELAR'
SELECT @Ok = 60060
IF @Accion = 'CANCELAR'
BEGIN
IF @Modulo = 'COMS'
SELECT @DID = DID FROM MovFlujo  WITH (NOLOCK)  WHERE OID = @ID AND OModulo = 'COMS' AND DModulo = 'SAUX'
IF @Modulo = 'VTAS'
SELECT @DID = DID FROM MovFlujo  WITH (NOLOCK) WHERE OID = @ID AND OModulo = 'VTAS' AND DModulo = 'SAUX'
SELECT @EstatusD = Estatus FROM SAUX  WITH (NOLOCK)  WHERE ID = @DID
