...
GO
ALTER PROCEDURE spIntelisisDistribucionSalidaTraspasoDetalle
@ID                                int,
@iSolicitud                        int,
@Version                           float,
...
ISNULL(Factor,0.0)
FROM InvD
WITH(NOLOCK) WHERE ID = @SalidaTraspasoID
UPDATE @Tabla
...
DECLARE SalidaTraspasoDetalle_cursor CURSOR FOR SELECT IDR, Articulo, SubCuenta, Unidad FROM @Tabla
OPEN SalidaTraspasoDetalle_cursor
FETCH NEXT FROM SalidaTraspasoDetalle_cursor INTO @IDR, @Articulo, @Subcuenta, @Unidad
...
FETCH NEXT FROM SalidaTraspasoDetalle_cursor INTO @IDR, @Articulo, @Subcuenta, @Unidad
END
CLOSE SalidaTraspasoDetalle_cursor
DEALLOCATE SalidaTraspasoDetalle_cursor
