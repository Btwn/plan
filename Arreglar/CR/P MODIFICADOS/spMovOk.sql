...
@CFD			bit, 
@CFDFlex		bit  
IF (SELECT COUNT(0) FROM Empresa WITH(NOLOCK) WHERE Empresa = ISNULL(@Empresa,'')) = 0 SET @Ok = 26070
IF (SELECT COUNT(0) FROM Sucursal WITH(NOLOCK) WHERE Sucursal = @Sucursal) = 0 SET @Ok = 72060
IF (SELECT COUNT(0) FROM Usuario WITH(NOLOCK) WHERE Usuario = ISNULL(@Usuario,'')) = 0 SET @Ok = 71020
...
BEGIN
SELECT @Ok = 60170
IF EXISTS(SELECT Empresa FROM EmpresaGral WITH(NOLOCK) WHERE Empresa = @Empresa AND (AC = 1 OR Ford = 1 OR Chrysler = 1))
SELECT @Ok = NULL
END
END
IF @ValidarFechas <> 'NO' AND NOT EXISTS(SELECT * FROM EmpresaCfgValidarFechasEx WITH(NOLOCK) WHERE Empresa = @Empresa AND Modulo = @Modulo AND Mov = @Mov) AND @Accion NOT IN ('RESERVAR', 'DESRESERVAR', 'RESERVARPARCIAL', 'ASIGNAR', 'DESASIGNAR')
BEGIN
...
IF @FechaAfectacion <> ISNULL((SELECT FechaTrabajo FROM FechaTrabajo WITH(NOLOCK) WHERE Empresa = @Empresa AND Sucursal = @Sucursal), @FechaRegistro) SELECT @Ok = 60175
END
...
IF EXISTS (SELECT *
FROM CerrarPeriodo WITH(NOLOCK)
WHERE Empresa = @Empresa
AND Rama = @Modulo
AND Ejercicio = @Ejercicio
AND Periodo = @Periodo)
IF @Accion <> 'CANCELAR' OR NOT EXISTS(SELECT * FROM EmpresaCfgCancelarMov WITH(NOLOCK) WHERE Empresa = @Empresa AND Modulo = @Modulo AND Mov = @Mov)
SELECT @Ok = 60110   			
IF @@ERROR <> 0 SELECT @Ok = 1
IF NULLIF(RTRIM(@Proyecto), '') IS NOT NULL AND @Ok IS NULL AND @Accion <> 'CANCELAR'
BEGIN
SELECT @EstatusProyecto = Estatus FROM Proy WITH(NOLOCK) WHERE Proyecto = @Proyecto
IF @EstatusProyecto = 'BLOQUEADO' SELECT @Ok = 26010 ELSE
IF @EstatusProyecto = 'BAJA'      SELECT @Ok = 26020
END
IF @Accion = 'CANCELAR'
IF EXISTS(SELECT * FROM UsuarioMovCancelacionEx WITH(NOLOCK) WHERE Usuario = @Usuario AND Modulo = @Modulo AND Mov = @Mov)
SELECT @Ok = 20766
...
IF @Modulo = 'CXC' AND (SELECT OrigenTipo FROM CXC WITH(NOLOCK) WHERE ID = @ID) = 'VTAS'
SELECT @CFD = 0
