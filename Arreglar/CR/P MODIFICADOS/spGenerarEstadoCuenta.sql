...
FROM CxcCuentaCorriente c WITH(NOLOCK) , Cte
WITH(NOLOCK) WHERE c.Empresa = @Empresa
...
FROM Auxiliar a WITH(NOLOCK), Cxc c WITH(NOLOCK), Cxc ca WITH(NOLOCK), MovTipo mt WITH(NOLOCK), Cte
