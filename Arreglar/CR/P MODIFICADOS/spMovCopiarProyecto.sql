...
FROM ProyectoRecurso WITH(NOLOCK)
WHERE ID = @ID
...
FROM ProyectoDRecurso WITH(NOLOCK)
WHERE ID = @ID
...
FROM ProyectoDArtMaterial WITH(NOLOCK) WHERE ID = @ID
RETURN
END
