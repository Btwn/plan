...
SELECT @MovGenerar = TMASurtidoPerdido FROM EmpresaCfgMovWMS WITH(NOLOCK) WHERE Empresa = @Empresa
...
IF ISNULL(@Ok,0) = 0
BEGIN
UPDATE TMA  WITH(ROWLOCK) SET Montacarga = @Montacarga, Agente = @Agente WHERE ID = @IDGenerar
UPDATE TMAD  WITH(ROWLOCK) SET GeneradoEnMovil = 1 WHERE ID = @IDGenerar
EXEC spAfectar 'TMA', @IDGenerar, 'AFECTAR', 'Todo', NULL, @Usuario, @Estacion, @EnSilencio = 1, @Conexion = 1, @Ok = @Ok OUTPUT, @OkRef = @OkRef OUTPUT
IF @Ok BETWEEN 80030 AND 81000 SELECT @Ok = NULL, @OkRef = NULL
END
IF ISNULL(@Ok,0) = 0
BEGIN
UPDATE TMAD  WITH(ROWLOCK) SET ProcesadoEnMovil = 1 WHERE ID = @IDGenerar
SELECT @MovIDGenerar = LTRIM(RTRIM(ISNULL(MovID,''))) FROM TMA WITH(NOLOCK) WHERE ID = @IDGenerar
INSERT INTO @Tabla (Mensaje)
VALUES (@MovGenerar + ' ' + @MovIDGenerar + ' generado con �xito.' )
