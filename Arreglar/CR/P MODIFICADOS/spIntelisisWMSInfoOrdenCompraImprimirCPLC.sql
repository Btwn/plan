...
IF @OK IS NULL
BEGIN
IF NOT EXISTS(SELECT TOP 1 * FROM Compra WITH(NOLOCK) WHERE Empresa = @Empresa AND Mov = @Mov AND MovID = @MovID)
SET @OK = 14055
END
IF @OK IS NULL
BEGIN
...
FROM Compra c
 WITH(NOLOCK) JOIN CompraD cd  WITH(NOLOCK) ON (c.ID = cd.ID)
