...
inner join */ BasesDeCalculo bc WITH(NOLOCK)
/*on rbc.idBaseDeCalculo=bc.idBaseDeCalculo*/
inner join  BaseCalculoRubroFiscal bct WITH(NOLOCK)
on bc.idBaseDeCalculo=bct.idBaseDeCalculo
inner join  RubrosFiscales ct WITH(NOLOCK)
on bct.rubro=ct.rubro
inner join CuentaRubro cr WITH(NOLOCK)
on ct.Rubro = cr.rubro
inner join  CuentasContables cc WITH(NOLOCK)
on cr.CuentaContable=cc.CuentaContable
left join   SaldosIni si WITH(NOLOCK)
on cc.CuentaContable=si.CuentaContable and si.Empresa=''' + @idempresa + '''  and si.Anio=' + cast(@anio as nvarchar(4))  +'
left join   MovContables mm WITH(NOLOCK)
...
join rubrosfiscales rff WITH(NOLOCK) on tt1.rubro = rff.rubro '
END
ELSE
BEGIN
set @sql='
...
join rubrosfiscales rff WITH(NOLOCK) on t1.rubro = rff.rubro
