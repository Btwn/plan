...
IF NOT EXISTS(SELECT OModulo FROM MovFlujo WITH(NOLOCK) WHERE OModulo = @OModulo AND OID = @OModuloID) OR @Nivel > 20 RETURN
...
IF @Nivel = 1
BEGIN
IF NOT EXISTS(SELECT OModulo FROM MovFlujo WITH(NOLOCK) WHERE OModulo = @OModulo AND OID = @OModuloID) SELECT @EsAcumulativa = 0 ELSE SELECT @EsAcumulativa = 1
SELECT TOP 1 @DModulo = '', @DModuloID = '', @DMov = '', @DMovID = '', @OMov = OMov, @OMovID = OMovID, @Cancelado = Cancelado, @Empresa = Empresa, @Sucursal = ISNULL(Sucursal,-1) FROM MovFlujo WITH(NOLOCK) WHERE OModulo = @OModulo AND OID = @OModuloID
...
IF NOT EXISTS(SELECT Estacion FROM MovPos WITH(NOLOCK) WHERE Estacion = @Estacion AND Modulo = @Modulo AND Tipo = 'DESTINO' AND OModulo = @OModulo AND OID = @OModuloID AND DModulo = @DModulo AND DID = @DModuloID AND ISNULL(Sucursal,-1) = ISNULL(@Sucursal,-1) AND Empresa = @Empresa)
BEGIN
IF NOT EXISTS(SELECT OModulo FROM MovFlujo WITH(NOLOCK) WHERE OModulo = @DModulo AND OID = @DModuloID) SELECT @EsAcumulativa = 0 ELSE SELECT @EsAcumulativa = 1
IF NOT EXISTS(SELECT Estacion FROM MovPos WITH(NOLOCK) WHERE Estacion = @Estacion AND Modulo = @Modulo AND Tipo = 'DESTINO' AND OModulo = @DModulo AND OID = @DModuloID AND DModulo = @OModulo AND DID = @OModuloID AND Sucursal = @Sucursal AND Empresa = @Empresa)
INSERT MovPos (Estacion,  Modulo,  Sucursal,  Empresa,  OEstatus,  OModulo,  OID,        OMov,  OMovID,  DEstatus,  DModulo,  DID,        DMov,  DMovID,  Cancelado,  Clave,                                                          Rama,                                                           EsAcumulativa,  Movimiento,                                                                             Tipo)
