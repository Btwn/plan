...
FROM PlanArtMaxMinCfg WITH(NOLOCK)
...
UPDATE ArtAlm WITH(ROWLOCK) SET Maximo=0,Minimo=0,PuntoOrden=0,PuntoOrdenOrdenar=0,ABC='',VentaPromedio=0
WHERE Almacen NOT IN(SELECT Almacen FROM alm WITH(NOLOCK) WHERE ExcluirPlaneacion=1)
...
JOIN MovTipo t WITH(NOLOCK) ON v.Mov = t.Mov AND t.Modulo = 'VTAS'
JOIN Art a WITH(NOLOCK) ON d.Articulo = a.Articulo
JOIN Alm l WITH(NOLOCK) ON d.Almacen = l.Almacen
LEFT JOIN ArtFam f WITH(NOLOCK) ON a.Familia = f.Familia
LEFT JOIN ArtCat c WITH(NOLOCK) ON a.Categoria = c.Categoria
...
JOIN MovTipo t WITH(NOLOCK) ON v.Mov = t.Mov AND t.Modulo = 'VTAS'
JOIN Art a WITH(NOLOCK) ON d.Articulo = a.Articulo
JOIN ArtFam f WITH(NOLOCK) ON a.Familia = f.Familia
JOIN ArtCat c WITH(NOLOCK) ON a.Categoria = c.Categoria
JOIN Alm l WITH(NOLOCK) ON d.Almacen = l.Almacen
WHERE t.Clave IN ('VTAS.F', 'VTAS.D','VTAS.VP')
AND v.Estatus IN ('CONCLUIDO')
AND UPPER(a.Tipo) <> 'SERVICIO'
...
UPDATE ArtAlm WITH(ROWLOCK) SET Minimo = ISNULL(b.Minimo,0), Maximo = ISNULL(b.Maximo,0), ABC = ISNULL(b.ABC,''), VentaPromedio = ISNULL(b.VentaPromedio,0)
FROM ArtAlm a 
...
FROM #ArtAlm a
LEFT JOIN ArtAlm b WITH(NOLOCK) ON(a.Empresa = b.Empresa AND a.Articulo = b.Articulo AND ISNULL(a.SubCuenta,'') = ISNULL(b.Subcuenta,'') AND a.Almacen = b.Almacen)
