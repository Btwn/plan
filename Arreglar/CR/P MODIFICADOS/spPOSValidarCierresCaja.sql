...
BEGIN
IF EXISTS(SELECT * FROM POSEstatusCajasCierre WITH (NOLOCK) WHERE Sucursal = @Sucursal AND dbo.fnFechaSinHora(Fecha) = @Fecha AND Caja <> @Caja )
SELECT @Ok = 30448, @OkRef = '('+(SELECT TOP 1 Caja FROM POSEstatusCajasCierre WITH (NOLOCK) WHERE Sucursal = @Sucursal AND dbo.fnFechaSinHora(Fecha) = @Fecha AND Caja <> @Caja)+')'
END
