...
SELECT @PermiteRetroceder = ISNULL(PermiteRetroceder, 0) FROM MovSituacion WITH(NOLOCK) WHERE Modulo = @Modulo AND Mov = @Mov AND Estatus = @Estatus AND Situacion = @Situacion 
