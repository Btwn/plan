...
@MovTipo		 varchar(20)
SELECT @MovTipo = Clave  FROM CXC c WITH (NOLOCK) JOIN MovTipo mt WITH (NOLOCK) ON Mt.Modulo = 'CXC' AND mt.Mov = c.Mov WHERE c.ID = @ID
...
SELECT @FormaCobro = ClaveSAT FROM FormaPago WITH (NOLOCK) WHERE FormaPago = @FormaCobro
...
FROM CteEnviarACFD WITH (NOLOCK)
...
FROM CteCFDInfoPagoD WITH (NOLOCK)
LEFT JOIN FormaPago WITH (NOLOCK) ON CteCFDInfoPagoD.FormaPago = FormaPago.FormaPago
...
SELECT @Resultado = ISNULL(NULLIF(SATFormaPago.Clave,''), '') FROM Cxc WITH (NOLOCK) JOIN Condicion WITH (NOLOCK) ON Cxc.Condicion = Condicion.Condicion
LEFT JOIN SATFormaPago WITH (NOLOCK) ON Condicion.CFD_formaDePago = SATFormaPago.Descripcion WHERE Cxc.ID = @ID
IF NULLIF(@Resultado,'') IS NULL
BEGIN
IF @Ref = 1
