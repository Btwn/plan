...
ELSE
BEGIN
SELECT @Impuestos = ISNULL(SUM(ISNULL(Importe,0.0)),0.0)/CASE WHEN @SAT_MN = 1 THEN @TipoCambio ELSE 1 END FROM CFDCXCMovImpuesto WITH (NOLOCK) WHERE ID = @ID
SELECT @Retenciones = ISNULL(SUM(ISNULL(Importe,0.0)),0.0)/CASE WHEN @SAT_MN = 1 THEN @TipoCambio ELSE 1 END FROM CFDCXCMovRetencion WITH (NOLOCK) WHERE ID = @ID
...
@Retenciones = ISNULL(Retencion,0.0)
FROM Cxc WITH (NOLOCK) WHERE ID = @ID
SELECT @Importe = @Importe - @Impuestos + @Retenciones
END
RETURN (@Importe)
END
