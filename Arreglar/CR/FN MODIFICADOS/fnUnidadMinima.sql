...
SELECT @Resultado = ''
IF @Resultado = '' AND EXISTS (SELECT * FROM Unidad WITH(NOLOCK) WHERE Factor = 1 AND Unidad = @Unidad)
SET @Resultado = @Unidad
IF @Resultado = '' AND EXISTS (SELECT * FROM Unidad WITH(NOLOCK) WHERE Factor = 1 AND Unidad = @UnidadCompra)
SET @Resultado = @UnidadCompra
IF @Resultado = '' AND EXISTS (SELECT * FROM Unidad WITH(NOLOCK) WHERE Factor = 1 AND Unidad = @UnidadTraspaso)
