...
@Sucursal	int
SELECT @SucursaleCommerce = SucursaleCommerce FROM WebEnvolturaRegalo WITH(NOLOCK) WHERE ID = @IDEnvoltura
SELECT @Sucursal = Sucursal FROM Sucursal WITH(NOLOCK) WHERE eCommerceSucursal = @SucursaleCommerce
