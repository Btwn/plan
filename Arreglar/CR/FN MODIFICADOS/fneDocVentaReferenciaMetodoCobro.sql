...
IF EXISTS (SELECT * FROM VentaCobro WITH(NOLOCK) WHERE ID = @ID)
...
SELECT @Resultado = ISNULL(NULLIF(SATFormaPago.Clave,''), '') FROM Venta WITH(NOLOCK) JOIN Condicion WITH(NOLOCK) ON Venta.Condicion = Condicion.Condicion
LEFT JOIN SATFormaPago WITH(NOLOCK) ON Condicion.CFD_formaDePago = SATFormaPago.Descripcion WHERE Venta.ID = @ID
IF NULLIF(@Resultado,'') IS NULL
BEGIN
IF @Ref = 1
