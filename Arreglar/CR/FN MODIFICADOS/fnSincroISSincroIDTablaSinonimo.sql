...
SELECT @TablaSinonimo = SinonimoTabla FROM SincroISTablaSinonimo WITH(NOLOCK) WHERE Tabla = @Tabla
IF @TablaSinonimo IS NOT NULL
SELECT @Resultado = CONVERT(int,SincroID) FROM SysTabla WITH(NOLOCK) WHERE SysTabla = @TablaSinonimo
