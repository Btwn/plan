...
SET @Resultado = NULL
SELECT @NotificacionClave = Clave FROM Notificacion WITH(NOLOCK) WHERE LTRIM(RTRIM(Notificacion)) = @Notificacion
SELECT @NumeroPeriodos = CONVERT(int,ISNULL(Valor,1)) FROM NotificacionParam WITH(NOLOCK) WHERE Notificacion = @Notificacion AND Parametro = @ParamNumeroPeriodos
IF @@ERROR <> 0 SET @NumeroPeriodos = 1
SELECT @PeriodoTipo = ISNULL(NULLIF(Valor,''),'DIA') FROM NotificacionParam WITH(NOLOCK) WHERE Notificacion = @Notificacion AND Parametro = @ParamTipoPeriodo
