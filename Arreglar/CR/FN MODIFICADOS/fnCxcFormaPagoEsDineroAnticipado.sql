...
@Valor			bit
SELECT @FormaPago = FormaCobro, @FormaPago1 = FormaCobro1 FROM Cxc WITH (NOLOCK) WHERE ID = @ID
IF EXISTS(SELECT MovTipo.Clave
FROM MovTipo WITH (NOLOCK)
JOIN FormaPago WITH (NOLOCK) ON FormaPago.MovIngresos = MovTipo.Mov AND MovTipo.Modulo = 'DIN'
