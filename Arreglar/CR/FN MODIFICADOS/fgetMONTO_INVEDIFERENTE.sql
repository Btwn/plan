...
GO
ALTER FUNCTION fgetMONTO_INVEDIFERENTE(@MontoRec as Float,
@CantReinv as Float,
...
IF (@CantReinv+@CantRedimPas) > @MontoRec AND @MesesDif <12 AND @_prorroga=0
BEGIN
SET @Monto = @MontoRec - (@CantReinv+@CantRedimPas);
END
IF (@CantReinv+@CantRedimPas) > @MontoRec AND @MesesDif <24 AND @_prorroga=1
BEGIN
SET @Monto = @MontoRec - (@CantReinv+@CantRedimPas);
END
RETURN @Monto
