...
BEGIN
IF(SELECT TOP 1 ISNULL(PermiteAfectacion, 0) FROM MovSituacion WITH(NOLOCK) WHERE Modulo = @Modulo AND Mov = @Mov AND Estatus = @Estatus AND Situacion = @Situacion) = 1
SELECT @Resultado = 1
ELSE
SELECT @Resultado = 0
END
