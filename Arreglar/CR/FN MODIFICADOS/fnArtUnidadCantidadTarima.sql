...
FROM ArtUnidad WITH (NOLOCK) WHERE Articulo = @Articulo AND Unidad = @Unidad
ELSE
IF @CfgMultiUnidadesNivel = 'UNIDAD'
SELECT @Resultado = NULLIF(CantidadTarima, 0.0) FROM Art WITH (NOLOCK) WHERE Articulo = @Articulo
END
ELSE
SELECT @Resultado = NULLIF(CantidadTarima, 0.0) FROM Art WITH (NOLOCK) WHERE Articulo = @Articulo
RETURN(@Resultado)
