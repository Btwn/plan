...
BEGIN
SELECT @Valor = RTRIM(ISNULL(Valor,'')), @Tipo = RTRIM(ISNULL(Tipo,'TEXTO')) FROM eDocInVariableTemp WITH (NOLOCK) WHERE Estacion = @@SPID AND UPPER(LTRIM(RTRIM(Variable))) = UPPER(RTRIM(LTRIM(@Variable)))
