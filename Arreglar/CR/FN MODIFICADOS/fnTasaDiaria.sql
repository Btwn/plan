...
ELSE BEGIN
SELECT @TasaTotal = ISNULL(Porcentaje, 0) FROM TasaD WITH(NOLOCK) WHERE Tasa = @TasaBase AND Fecha = @Fecha
IF @@ROWCOUNT = 0
SELECT @TasaTotal = ISNULL(Porcentaje, 0) FROM Tasa WITH(NOLOCK) WHERE Tasa = @TasaBase
IF @SobreTasaEn = '%'
